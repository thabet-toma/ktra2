...
|---|---|---:|---:|---|
| `logistics` | الاستيراد والمشتريات: صفقة ← شحنة ← تخليص ← نقل ← فاتورة دولية + التكلفة المستوردة | 26,000 | 18,500 | `/api/logistics/` |
...
| `core` | طبقة مشتركة: عزل الشركة، الصلاحيات، التقارير، الوحدات المرخّصة وحدود الخطط **وأسعارها**، الداشبورد، المساعد الذكي | 18,500 | 12,200 | `/api/` (متفرّق) |
| `accounting` | دفتر الأستاذ: شجرة الحسابات، القيود، الشيكات، البنوك، الفترات المالية، العملات، الضريبة | 16,700 | 10,100 | `/api/accounting/` |
| `sales` | دورة البيع (عرض ← طلبية ← فاتورة ← تسليم ← تحصيل) + سندات صرف المورّدين | 14,700 | 13,200 | `/api/sales/` |
