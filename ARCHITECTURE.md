...
| `core` | طبقة مشتركة: عزل الشركة، الصلاحيات، التقارير، الوحدات المرخّصة، الداشبورد، المساعد الذكي | 11,600 | 5,100 | `/api/` (متفرّق) |
| `sales` | دورة البيع (عرض ← طلبية ← فاتورة ← تسليم ← تحصيل) + سندات صرف المورّدين | 10,000 | 5,900 | `/api/sales/` |
...
- **الإنفاذ آلي لا طوعي:** CI تعمل على `main` و`newktra` (كانت على `main` وحده وهو متوقف
  ...
  (2026-08-12): لم يعد في المستودع أي مسار نشر آلي إلى `smart.ktragroup.com`.
