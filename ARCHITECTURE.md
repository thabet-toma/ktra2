...
| `sales` | دورة البيع (عرض ← طلبية ← فاتورة ← تسليم ← تحصيل) + سندات صرف المورّدين | 11,900 | 10,300 | `/api/sales/` |
| `inventory` | المنتجات والمستودعات و`StockMovement` (المصدر الوحيد للرصيد) وطبقات كلفة FIFO (`fifo.py` — والمتوسّط مشتقٌّ منها للعرض) والأرقام التسلسلية وحالة المخزون وحدود التجديد | 8,100 | 7,300 | `/api/inventory/` |
