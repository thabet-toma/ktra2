...
| `sales` | دورة البيع (عرض ← طلبية ← فاتورة ← تسليم ← تحصيل) + سندات صرف المورّدين | 12,600 | 11,600 | `/api/sales/` |
| `inventory` | المنتجات والمستودعات و`StockMovement` (المصدر الوحيد للرصيد) وطبقات كلفة FIFO (`fifo.py` — والمتوسّط مشتقٌّ منها للعرض) والأرقام التسلسلية وحالة المخزون وحدود التجديد | 8,200 | 7,500 | `/api/inventory/` |
