...
Backend: Django 6.1.1 + DRF 3.18 على MySQL 8.4 (إنتاج) / SQLite ذاكرة (اختبار).
...
|---|---|---:|---:|---|
| `platform_ops` | عمليات المنصة: فحص صحة دفاتر الشركة المعتمد، وإسناد موظفي المنصة ونقلهم حسب الطاقة، وإدارة الخدمة واشتراكاتها، ودورة أوامر العمل (طابور ← نقل حالة ← تسليم ← مراجعة) مع كتالوج وحدات خدمة بنسخٍ مؤرَّخة ودفتر استخدامٍ لا يُعِدّ إلا بعد الاعتماد، وسياسة تقييم pilot بمحاور 40/30/20/10 ومحفظة عملياتية (راتب وعمولة اكتساب) بإغلاقٍ شهري idempotent، واجتماعاتُ المنصّة بدفتر حضورٍ مستقلٍّ عن دوام `hr` (دخولٌ ضمن نافذة، واعتذارٌ يبتّ فيه المدير)، و**اجتماعاتُ المتقدّمين قبل التوظيف** بدفترٍ ثالثٍ منفصلٍ عنهما: حاضرٌ متقدّمٌ أو اسمٌ حرّ، وملاحظةٌ على كلّ حاضرٍ في كلّ اجتماع، **وشبكةُ حضورٍ** صفُّها شخصٌ وعمودُها اجتماعٌ تُقرأ نظرةً واحدةً وتُصدَّر CSV، **وصفحةُ متابعةٍ عامّةٌ للمتقدّم** على رابط الإعلان نفسِه يفتحها برقم التتبّع وهاتفه فيقرأ حالتَه واجتماعَه ورسائلَ الفريق ويردّ عليها — **وحدة منصة غير مرخصة للشركات، ولا payroll قانوني فيها** | 24,100 | 25,700 | `/api/platform/ops/` · `/api/my-agent/` |
| `logistics` | الاستيراد والمشتريات: صفقة ← شحنة ← تخليص ← نقل ← فاتورة دولية + التكلفة المستوردة | 24,100 | 17,300 | `/api/logistics/` |
| `core` | طبقة مشتركة: عزل الشركة، الصلاحيات، التقارير، الوحدات المرخّصة وحدود الخطط **وأسعارها**، الداشبورد، المساعد الذكي | 18,400 | 12,200 | `/api/` (متفرّق) |
| `accounting` | دفتر الأستاذ: شجرة الحسابات، القيود، الشيكات، البنوك، الفترات المالية، العملات، الضريبة | 16,400 | 10,100 | `/api/accounting/` |
| `sales` | دورة البيع (عرض ← طلبية ← فاتورة ← تسليم ← تحصيل) + سندات صرف المورّدين | 13,300 | 12,600 | `/api/sales/` |
| `inventory` | المنتجات والمستودعات و`StockMovement` (المصدر الوحيد للرصيد) وطبقات كلفة FIFO (`fifo.py` — والمتوسّط مشتقٌّ منها للعرض) والأرقام التسلسلية وحالة المخزون وحدود التجديد | 8,800 | 8,300 | `/api/inventory/` |
| `hr` | الموظفون والرواتب والحضور الجغرافي والورديات والعقود والطلبات والسلف والخدمة الذاتية | 7,600 | 4,500 | `/api/hr/` |
...
| `docshare` | مشاركة المستند برابط عام: صفحة **بلا مصادقة** يفتحها الزبون **أو المورّد** (‏HTML خادمي بوسوم Open Graph لمعاينة واتساب) + قبول/رفض عرض السعر منها. أربعة عشر نوعاً بجمهورين ومفتاحَي صلاحية | 3,600 | 2,900 | `/s/` · `/api/share/` · `/api/document-shares/` |
...
| `crm` | نواةُ CRM كترا التسويقيّة: زبائنُ كترا المحتمَلون (محلّات) قبل أن يصيروا شركاتٍ على المنصّة — مفتاحُهم رقمُ الهاتف المطبَّع، حارسُ ملكيةٍ يمنع عمل موظّفين على عميلٍ واحد، سجلُّ تواصلٍ لا يُحذَف، ونقلٌ رسميٌّ بسببٍ مكتوب، وستاتستكسُ كلِّ رقمٍ مجمَّعةٌ في الخادم (تواصلٌ فعليٌّ لا كلُّ صفٍّ في السجلّ). **وحدةٌ داخليّة لموظّفي كترا أنفسِهم لا للشركات** | 1,700 | 1,800 | `/api/platform/crm/` |
| `partners` | بطاقة الطرف الموحّدة (عميل/مورّد/…) وحساباتها البنكية وربطها بشجرة الحسابات | 1,600 | 1,200 | `/api/partners/` |
...
| `bridge` | جسر مزامنة Firestore القديم (`FirestoreMirrorDoc`) | 600 | 500 | `/api/mapper/` |
<!-- AUTO:apps_table:END -->
...
platform_ops ──► core · tenants · sales · partners · inventory · hr
```
...
### 3. الترحيل المحاسبي
**كل قيد يمرّ عبر `accounting.services` (`post_journal`)** — هي وحدها تفرض الفترة المفتوحة والتوازن الدقيق والـidempotency وقفل `select_for_update`، و**القفل الضريبي** (A2-1): لا ترحيل ولا فكّ ترحيل بتاريخٍ داخل فترة كشف ض.ق.م معتمد نهائياً (`assert_no_final_vat_statement`)، والمخرج إعادة فتح الكشف للمدير بسببٍ مسجَّل. و**وسمُ الطرف على سطر ذمّته وحده**: وسمٌ على صندوقٍ أو مصروفٍ أو مخزون يُرفض (`foreign_partner_tag_rows`) لأن `partner_posted_balance` يجمع أسطر الطرف بلا فلتر حساب.
...
الفترات المالية تُنشأ شهرية افتراضياً (`create_fiscal_year`) **من تاريخ بدء السنة المالية — أوّل كانون الثاني افتراضاً ولا يلزم أن يكون** (#213-أ)، **ولا تتقاطع** (`assert_no_period_overlap`)، وإعادة فتح فترة مغلقة تشترط سبباً يُحفظ في `AccountingAuditLog` — هي الاستثناء المُعلَن الوحيد من القفل.
...
- **الإنفاذ آلي لا طوعي:** CI تعمل على `main` و`newktra` (كانت على `main` وحده وهو متوقف
  منذ 2026-07-19). وهي تثبّت `requirements.txt` أي Django 6.1.1 على Python 3.12 —
  نسخةَ الإنتاج نفسها. **CI هي البوابة الآلية الوحيدة الآن** بعد حذف سكربت النشر
