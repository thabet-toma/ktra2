...
| `logistics` | الاستيراد والمشتريات: صفقة ← شحنة ← تخليص ← نقل ← فاتورة دولية + التكلفة المستوردة | 21,900 | 15,100 | `/api/logistics/` |
...
| `accounting` | دفتر الأستاذ: شجرة الحسابات، القيود، الشيكات، البنوك، الفترات المالية، العملات، الضريبة | 16,100 | 9,600 | `/api/accounting/` |
...
### 3. الترحيل المحاسبي
**كل قيد يمرّ عبر `accounting.services` (`post_journal`)** — هي وحدها تفرض الفترة المفتوحة والتوازن الدقيق والـidempotency وقفل `select_for_update`، و**القفل الضريبي** (A2-1): لا ترحيل ولا فكّ ترحيل بتاريخٍ داخل فترة كشف ض.ق.م معتمد نهائياً (`assert_no_final_vat_statement`)، والمخرج إعادة فتح الكشف للمدير بسببٍ مسجَّل.
