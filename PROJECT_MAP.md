...
- **T2-FIX-01 (HIGH):** the owner's real shipments page is `/shipments` → `ShipmentManagement` → `ShipmentList.tsx:231-232`, which uses bespoke `shipmentName||agentShipmentNumber||shipmentNumber` (camelCase `Shipment`), NOT the shared `buildShipmentOptionLabel`. T2-02's util only reached `SqlShipmentsPage` (secondary `sql-shipments` view) + clearance. Owner's complaint unresolved on the primary page. Needs a camelCase→`ShipmentLabelInput` adapter + wiring.
- **T2-FIX-02 (MED):** `LocalShippingPage.tsx:848` save button `disabled={!canSubmit||submitting}`, `canSubmit` needs `amount>0` → `submit()` (and the T2-03 `validatePaymentInput` guard inside it) is unreachable for empty/zero amount; unified message never shows. Drop `amount>0` from `canSubmit`.
