...
## [PLANNED — T-EXTACCT: بوابة المحاسب القانوني الخارجي متعدد الشركات, 2026-08-03]

**الحالة: خطة معتمدة الولاية، لم يُكتب لها كود بعد.** الولاية الضريبية المُقرَّة:
...
**فجوات مؤكَّدة يعالجها التنفيذ:**
- `_ACCOUNTANT` في `core/access.py:140` يمنح `inventory.doc.post/unpost` و
  `inventory.cost.view` و`import.doc.unpost` — فلا يصلح للمحاسب الخارجي.
- `core/tenant_utils.py:106` يتحقق من العضوية بـ`.exists()` فقط — بلا مفهوم
  «علاقة نشطة»؛ يلزمه حارس إضافي لدور `legal_accountant`.
- `tenants/views.py:229` `_can_create_company` يمنع محاسباً عضويتُه الوحيدة
  `legal_accountant` من إنشاء مكتبه الخاص (يشترط عضوية `manager`).
- `POST /api/hr/auth/signup/` (`hr/auth_api.py:190`) بلا تحقق بريد ولا throttling
  وكلمة مرور 6 أحرف ويكشف تسجيل البريد مسبقاً.
- `VatStatement` (`sales/models.py:1363`) حالتان فقط (draft/final) بلا فترة مرتبطة
  ولا مُعتمِد ولا قفل، وشاشتُه محروسة بـ`accounting.report.view` وحدها.
