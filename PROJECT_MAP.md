...
> ⚠️ **تذكير — أول ما نشتغل على الاستيراد:** وحّد تكلفة فواتير الاستيراد (GR/IR مع
> التخليص) مع نموذج «تكلفة المنتجات» الجديد. حالياً مسار الاستيراد
> (`logistics/views.py::_post_grn_and_receive`) يبقى على **المتوسط المتحرك بتكلفة
> landed** (يحتسب مصاريف التخليص في تكلفة المخزون عبر `goods_clearing_amt`)، ولم
> يُربط بـ `set_avg_cost_from_purchases`. عند توحيده: يجب أن يضمّن `product_cost_breakdown`
> مصاريف التخليص (landed) في تكلفة الفاتورة كي يبقى avg_cost = رصيد حساب المخزون في GL،
> ثم استدعاء `set_avg_cost_from_purchases` في مسار GR/IR + إلغاء ترحيله (كما في المحلي).
...
- `frontend_v2/components/inventory/StocktakePage.tsx`: الصنف المختار يُعرض كاسم ثابت
  ...
  «أظهر الفروقات فقط» + زر **طباعة** ورقة النتيجة (يحترم الفلتر). **حرج:** السطر بلا عدّ
  (خانة فارغة) يُستثنى من الترحيل ولا يُصفِّر الصنف (جرد جزئي آمن). الفرق محسوب بالواجهة.
