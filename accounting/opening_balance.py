...
وبضاعة أول المدة (كمية × تكلفة وحدة لكل صنف/مستودع). ترحيله ذرّيّ:
        ...
        if stock_lines is not None:
            ...
            for raw in stock_lines:
                ...
                if key in seen_stock:
                    raise ValidationError(
                        f"الصنف «{product}» مكرّر في المستودع «{warehouse.name}»."
                    )
                seen_stock.add(key)
                ...
                if quantity <= 0:
                    raise ValidationError(
                        f"كمية الصنف «{product}» يجب أن تكون أكبر من صفر."
                    )
                if unit_cost < 0:
                    raise ValidationError(
                        f"تكلفة وحدة الصنف «{product}» لا تكون سالبة."
                    )
...
def _serial_items(tenant, stock_lines: list) -> list[dict]:
    """أصناف الافتتاح المتتبَّعة تسلسلياً: المُسجَّل مقابل المطلوب — إرشاد لا حارس.
    ...
    التجميع بالصنف لا بالبند: `ProductSerial` بلا مستودع، فوحدةٌ مُرقَّمة تُحسب
    للصنف كلّه — وصنفٌ في مستودعين صفٌّ واحد بمجموع كميته.

    و«المُسجَّل» كل وحدات الصنف مهما كانت حالتها لا «في المخزن» وحدها: بيعُ وحدةٍ
    مُرقَّمة لا يجوز أن يُعيد الصنف ناقصَ الترقيم فيُطالب المستخدم بترقيمٍ أتمّه.
    الرقم الكامل لِما بقي بلا ترقيم (رصيد المخزن مقابل الوحدات) يعرضه كرت الصنف
...
def opening_balance_summary(tenant) -> dict:
    ...
    و`serial_items` أصناف بضاعة الافتتاح المتتبَّعة تسلسلياً وحالة ترقيمها
