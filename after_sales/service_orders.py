...
3. **القطعة المفوترة تمرّ في فاتورة البيع القائمة** بمسار `SALE` المجرَّب — لا
   مسار صرفٍ موازٍ. وبند الأجرة صنف خدمة فيقع إيراده في «إيرادات الخدمات»
...
def resolve_labour_product(tenant_id: int):
    """صنف خدمة «أجرة صيانة» الافتراضي — يُنشأ مرة ويُثبَّت في الإعدادات.
...
def post_covered_parts(order: ServiceOrder, *, user=None) -> dict:
    ...
    كلفةٌ صفرية (صنف بمتوسط صفر) تُسجَّل حركةً بلا قيد: البضاعة خرجت فعلاً
