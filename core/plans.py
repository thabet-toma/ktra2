...
PLAN_LABELS = {
    ...
    "Pro": "المتقدمة",
    "Enterprise": "المتخصصة",
    # التجريبيّةُ مخفيّةٌ عن **العرض** لا عن الرسائل: `public_plan_rows` تمرّ على
    # `PUBLIC_PLAN_ORDER` لا على هذا الجدول، فوجودُها هنا لا يعرضها للبيع —
    # وغيابُها كان يُظهر كلمة `Trial` عاريةً في منتصف جملةٍ عربيّةٍ يقرؤها زبون.
    "Trial": "التجريبية",
}
...
def tenant_usage_rows(tenant) -> list[dict]:
    """صفوفُ «خطّتي» لعضوٍ عاديٍّ في الشركة: الحدُّ **الفعّال** واستهلاكُه.

    إسقاطٌ من `limit_rows` لا نسخةٌ ثانيةٌ منها — الحدودُ تُقرأ من `LIMITS` مرّةً
    واحدةً في المستودع كلِّه، فلا تُعرَض على المستخدم حدودٌ لا يفرضها الخادم.

    **والحدُّ هنا الفعّالُ لا افتراضُ الخطّة**: بطاقةُ «خطّتي» كانت تقرأ حدودَها
    من حمولة الأسعار العامّة، وتلك تعرف `PLAN_DEFAULTS` وحدَها — فشركةٌ رُفع لها
    حدُّ الفواتير بـ`TenantLimit` كانت ترى رقمَ الخطّة لا رقمَها هي، ثمّ تُمنَع
    عند رقمٍ غيرِ الذي قرأته أو تُمنَح فوقَه.

    **ولا `plan_default` ولا `has_override` في الإسقاط**: التجاوزُ اتّفاقٌ بين
    المنصّة والشركة يخصّ لوحةَ المنصّة، وعرضُه لكلّ عضوٍ في الشركة كشفٌ لا يلزم
    لقراءة «كم بقي لي». الفعّالُ وحدَه هو ما يُفرَض فهو وحدَه ما يُعرَض.
    """
    return [
        {
            "key": row["key"],
            ...
            # عددٌ = الحدّ · `None` = بلا حدّ · `0` = غير متاح.
            "limit": row["effective"],
            "usage": row["usage"],
        }
        for row in limit_rows(tenant)
    ]
...
def limit_exceeded_message(tenant, key: str, limit: int) -> str:
    """نصُّ المنع كما يقرؤه **مستخدمُ الشركة** — لا موظّفُ المنصّة.

    ليست سطرَ سجلّ: `enforce_limits` يرفعها في `plan_limit` فتظهر منذ 211-P في
    حوارٍ كامل أمام من مُنع من الإنشاء، وهي آخرُ ما يقرؤه قبل أن يتوقّف.

    فعلان مقصودان هنا:

    - **الاسمُ عربيٌّ** من `PLAN_LABELS` لا مفتاحُ القاعدة: `Basic` قيمةُ عمودٍ
      لا اسمُ خطّة، وهي معروضةٌ له «الأساسية» في صفحة الأسعار وفي «خطّتي» —
      فطباعةُ المفتاح تُريه اسمين لخطّةٍ واحدة.
    - **والمخرجُ بابٌ يملكه**: كان النصُّ يقول «ارفع الحدّ من لوحة المنصة»، وتلك
      شاشةُ كترا نفسِها خلفَ `IsPlatformAdmin` — لا يفتحها زبونٌ أبداً. يبحث
      عنها في حسابه فلا يجدها، فيقرأ العجزَ عيباً في فهمه. والترقيةُ فعلٌ يملكه،
      وحوارُ 211-P يحمل زرَّها فعلاً.
    """
    spec = LIMITS[key]
    plan = _plan_of(tenant)
    return (
        f"بلغت شركتك حدّ خطة «{PLAN_LABELS.get(plan, plan)}» من {spec.label}: "
        f"{limit} {spec.unit} {PERIOD_LABELS[spec.period]}. "
        "رقِّ خطّتك لرفع الحدّ."
    )
