...
بحث منفصلة (المنتجات · الوحدات المُرقَّمة · الأجهزة الحسّاسة · استقبال الصيانة)،
وكلٌّ يسأل المستخدم أن **يعرف نوع ما في يده قبل أن يبحث عنه** — وهو عكس ما يفعله
الموظف على الطاولة: الزبون يناوله علبةً، فيمسح الملصق ولا يدري أهو باركود منتجٍ
...
الوحدات والمنتجات بـ`inventory.item.view`، والأجهزة الحسّاسة بترخيص وحدتها
...
#: يريد قائمةً يفتح شاشة المنتجات، وزرّ «اعرض كل النتائج» يقوده إليها.
FUZZY_LIMIT = 8

#: سقف المطابقات التامّة لكل مصدر. `unique_together` هو (شركة، منتج، رقم) فقد
#: يحمل منتجان الرقم نفسه مشروعاً — نعرضهما ويختار المستخدم، ولا نخمّن.
EXACT_LIMIT = 5
...
def scan_scope(user, tenant) -> dict:
    ...
    return {
        # الوحدة المُرقَّمة جزءٌ من المنتج لا كيانٌ مستقل — صلاحيتها صلاحيته.
        "units": inventory,
        "products": inventory,
        "devices": devices,
...
def _unit_card(unit, row: dict, *, warranty, orders) -> dict:
    ...
    بطاقة المنتج وشاشة الوحدات، فأي تعديل عليه يصل هنا مجّاناً — والعكس، لو
...
def _product_row(product, *, matched_on: str) -> dict:
    return {
        ...
        # لماذا طابق: الموظف يرى أن الرقم باركودٌ لا رمزُ منتج فيثق بالنتيجة.
        "matched_on": matched_on,
    }


def _products(tenant_id: int, term: str, scope: dict) -> list[dict]:
    """المنتجات: التطابق التامّ (باركود ثم رمز) أولاً، ثم المطابقة الجزئية.

    الترتيب مقصود ومسبوقٌ إليه — «خيار البحث عن الباركود المقروء في رقم المنتج
...
def scan_lookup(request):
    ...
    if not any(scope.values()):
        raise PermissionDenied(
            "صلاحية «عرض المنتجات والمخزون» غير ممنوحة لدورك — لا يمكن التعرّف على الأرقام."
        )
