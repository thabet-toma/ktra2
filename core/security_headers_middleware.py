...
الواجهة الأمامية (SPA) ملفّات ساكنة يخدمها nginx على `ktra-pro.tech`، وDjango خلفه
يخدم الـAPI (`/api/`) وواجهة الإدارة وDRF التصفّحية وصفحات الأخطاء فقط — أي أن هذه
الترويسة لا تلحق بصفحة الـSPA أصلاً رغم أنهما على نفس الأصل، فوضع CSP هنا يحمي أسطح
Django دون أن يمسّ تصييرها. رؤوس الأمان الأخرى (nosniff/referrer/xframe/HSTS) تُضبط
عبر إعدادات Django القياسية في settings.py.
