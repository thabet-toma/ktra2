...
class MyPlanUsageTest(APITestCase):
    ...
    def test_the_whole_card_costs_a_fixed_number_of_queries(self):
        ...
        صيغةٌ على `len(LIMITS)`: الصيغةُ تفترض عدّاً واحداً لكلّ حدٍّ وهي مخالِفةٌ
        للواقع، فتتحرّك مع الكود بدل أن تمسكه.

        وكان الرقمُ ستّةَ عشرَ: عدّا البيعِ والشراء يقعان **مرّتين** — مرّةً
        لحدَّيهما ومرّةً لحدّ المجموع — لأنّ `limit_rows` كانت تنادي
        `current_usage` في حلقةٍ على الحدود. صارت تقرأ `bulk_usage` التي تعدّ كلَّ
        مصدرٍ مرّةً واحدةً وتشتقّ المركَّبَ من `LimitSpec.sums`، فسقط الاستعلامان
        الزائدان من كلّ فتحٍ لهذه البطاقة ومن كلّ قراءةٍ لصفوف لوحة المنصّة.
