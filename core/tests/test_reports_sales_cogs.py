"""THA-60/م2: تكلفة المبيع في «حسب الصنف» و«حسب الماركة» تاريخية لا لحظية.
...
ثلاثة عقود:
  ...
  B تكافؤ — Σ ربح الأصناف == ربح «أرباح الفواتير» على الفواتير نفسها.
...
class SalesCogsHistoricalTest(TestCase):
    ...
    def _product(self, sku, *, avg_cost="10", brand="ماركة أ", is_service=False):
        return Product.objects.create(
            tenant=self.tenant, sku=sku, name_ar=f"صنف {sku}", brand=brand,
