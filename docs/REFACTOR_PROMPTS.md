...
> (المنفّذ مشطوب؛ **التالي = المرحلة 5**. التفاصيل الكاملة في «الحالة الآن» أدناه.)
>
...
**المُنجَز:** المراحل 0، 1، 2، 3، 4 + جلسة معالجة ديون المرحلة 2 (`3358bf7`) + **الجلسة الأمنية** (P0-2/3/4/8). الفرع المعتمد لكل العمل: **`newktra`**.

**المهمة التالية: المرحلة 5 — تنفيذ بنود P0 الأداء.** انسخ برومت المرحلة 5 أدناه في جلسة جديدة. قبلها اقرأ:
1. «نتائج المرحلة 4» أدناه + `docs/SCALABILITY_AUDIT.md` — **بنود P0 الأمنية الأربعة (P0-2/3/4/8) نُفّذت**؛ تبقى للمرحلة 5 بنود الأداء فقط: P0-1 (نشر gunicorn)، P0-5 (فرض الترقيم — endpoint-by-endpoint مع مستهلك الواجهة)، P0-6 (Redis)، P0-7 (throttle عام user/anon — البنية موجودة، معدّلان مضافان بالفعل: `agent_query`/`media_upload`)، P0-9/10/11 (استعلامات وفهارس)، P0-12 (واجهة الأصناف). وبند المرحلة 5 رقم 3 (`CONN_MAX_AGE`) مشطوب.
...
**ديون جانبية مفتوحة (أي وقت، جلسة قصيرة):** (أ) 8 اختبارات pytest حمراء سابقة للمرحلة 2 في `sales/tests/test_quotation_{create,convert,pricing_link}.py`. (ب) بنود P1 الأمنية الأخف (P1-6/7/8 في `SCALABILITY_AUDIT.md`).
...
# 3. تحقق الخط الأساسي (~3.5 دقيقة، المتوقع: 1,040 OK + skipped=2):
