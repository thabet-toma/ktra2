...
**المُنجَز:** المراحل 0، 1، 2، 4 + جلسة معالجة ديون المرحلة 2 (`3358bf7` — انظر «الديون المعالَجة» أدناه). الفرع المعتمد لكل العمل: **`newktra`**.

**المهمة التالية — بقرار صاحب المشروع 2026-08-11 (التوصية معتمدة، لم يعد قراراً معلّقاً): جلسة مستقلة تغلق ثغرات عزل الـtenant الثلاث** (P0-2/P0-3/P0-4 في `docs/SCALABILITY_AUDIT.md`: SQL خام بلا مصادقة في `core/agent_db_view.py`، مجموعات bridge مشتركة بين الشركات، تبنّي وثائق يتيمة — ومعها رفع الوسائط المجهول بلا throttle `core/media_views.py:44`). خطورتها أمنية لا أدائية فتتقدّم على ترتيب المراحل. اقرأ قبلها بنود P0 الأمنية في `docs/SCALABILITY_AUDIT.md` كاملة.

**بعدها: المرحلة 3 — تفكيك الملفات العملاقة.** انسخ برومت المرحلة 3 أدناه كاملاً في جلسة جديدة. قبلها اقرأ:
...
| أمن — ثغرات عزل الـtenant | ⬜ **التالية** (معتمدة 2026-08-11) | — | P0-2/P0-3/P0-4 + media throttle من `SCALABILITY_AUDIT.md` |
| 3 — تفكيك الملفات العملاقة | ⬜ بعد جلسة الأمن | — | — |
