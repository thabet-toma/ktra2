...
**المُنجَز:** المراحل **0، 1، 2، 3، 4، 5 كاملةً** + جلسة معالجة ديون المرحلة 2 (`3358bf7`) + الجلسة الأمنية (P0-2/4/8) + **جلسة إكمال المرحلة 5 والديون** (2026-08-11): P0-5 كاملاً على 9 نقاط، P0-13، الاختبارات الثمانية الحمراء، P1-6/7/8. الفرع المعتمد لكل العمل: **`newktra`**.

**المهمة التالية: المرحلة 6 — اختبار الحمل وقرار Celery.** هي المرحلة الوحيدة المتبقية. انسخ برومتها أدناه في جلسة جديدة، **بشرط بيئة تشبه الإنتاج** (gunicorn multi-worker + MySQL) — حاوية الوكيل الاعتيادية (SQLite بلا خادم MySQL) لا تصلح لها، وبرومت المرحلة نفسه يمنع تشغيلها على غير ذلك. شرطها المسبق (بنود P0 منفّذة) متحقق — راجع «مراجعة تحقق الأهداف» أدناه.
...
**ديون جانبية مفتوحة (أي وقت، جلسة قصيرة):** (أ) **P0-3** — تقييد مجموعات bridge العالمية؛ يحتاج هجرة نسبة `userId ↔ tenant` أولاً (جلسة بيانات مستقلة، راجع «درس مراجعة الكود»). (ب) بنود P1 غير الأمنية وP2 في `SCALABILITY_AUDIT.md` — مرتّبة هناك بالأثر.
...
| 6 — اختبار حمل + قرار Celery | ⬜ **الوحيدة المتبقية** | تتطلب بيئة gunicorn+MySQL | — |
...
**بنود ما زالت مفتوحة عمداً (خارج أهداف الخطة الأصلية أو مؤجلة بقرار):**
- **P0-3** (bridge global collections) — تحتاج هجرة نسبة `userId ↔ tenant` (جلسة بيانات مستقلة).
- **P1** غير الأمنية (P1-1..5، P1-9، بقايا P1-10، P1-11..15) — برومت المرحلة 5 يمنع تنفيذ P1 نصاً؛ ما نُفّذ منها (P1-6/7/8 و14) كان بتكليف «النواقص» أو شبه مجاني.
- **P2** كلها.
