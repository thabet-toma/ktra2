...
>
> **الترتيب:** ~~0~~ ← ~~1~~ ← ~~4~~ ← ~~2~~ ← ~~أمن~~ ← ~~3~~ ← **5 (جارية — 6 بنود من 8)** ← 6
> (المنفّذ مشطوب؛ **التالي = إكمال المرحلة 5: البند P0-5 وحده**. التفاصيل الكاملة في «الحالة الآن» أدناه.)
>
...
**المُنجَز:** المراحل 0، 1، 2، 3، 4 + جلسة معالجة ديون المرحلة 2 (`3358bf7`) + **الجلسة الأمنية** (P0-2/3/4/8) + **المرحلة 5 جزئياً: 6 بنود من 8** (P0-1، P0-6، P0-7، P0-9، P0-10، P0-11، P0-12). الفرع المعتمد لكل العمل: **`newktra`**.

**المهمة التالية: إكمال المرحلة 5 — البند P0-5 وحده (فرض الترقيم).** هو آخر بنود P0 المتبقّية، وأكبرها (تقدير التدقيق: 2-3 أيام). **لا تنسخ برومت المرحلة 5 كاملاً** — بنوده الأخرى منفّذة؛ اقرأ «نتائج المرحلة 5» أدناه ثم نفّذ P0-5 وحده بنمط endpoint = commit.

قبلها اقرأ:
1. «نتائج المرحلة 5» أدناه — فيها **خريطة مستهلكي P0-5** المبنيّة من قراءة الكود، وهي المدخل العملي للبند.
...
**ديون جانبية مفتوحة (أي وقت، جلسة قصيرة):** (أ) 8 اختبارات pytest حمراء سابقة للمرحلة 2 في `sales/tests/test_quotation_{create,convert,pricing_link}.py`. (ب) بنود P1 الأمنية الأخف (P1-6/7/8 في `SCALABILITY_AUDIT.md`). (ج) **P0-13 الجديد** — نقطة تسجيل الدخول خارج سلسلة الـthrottle كلياً (`SCALABILITY_AUDIT.md` §1.7)؛ أمنية، ساعتان، ومستقلة تماماً عن P0-5.
...
| 5 — تنفيذ P0 | 🟨 **جزئية** 2026-08-11 — 6 بنود من 8 | `newktra` / 6 commits (`eb50e92`،`0c4153c`،`0884e15`،`2ccf769`،`b7ea11f`،`385a173`) | `manage.py test` OK بعد **كل** commit · 1,040 → 1,046 اختباراً · +6 اختبارات · الواجهة: `tsc --noEmit` نظيف و`npm run build` ناجح · **المتبقّي: P0-5 وحده** |
