...
> (المنفّذ مشطوب؛ **التالي = المرحلة 6 — تتطلب بيئة تشبه الإنتاج، لا حاوية الوكيل**. التفاصيل في «الحالة الآن» أدناه.)
>
...
- **المرحلة 6:** العدّة بُنيت كاملةً (`load_tests/` + أمر بذر محروس + `docs/LOAD_TEST_RESULTS.md`) — **لكن لم يُقَس شيء**، ولا رقم واحد مُختلَق.

**المهمة التالية: تشغيل المرحلة 6 فعلياً.** لم يبقَ فيها بناء، بل قياس. شرطها الوحيد بيئةٌ تشبه الإنتاج: **gunicorn لا يعمل على Windows إطلاقاً** (POSIX-only، يستورد `fcntl`)، والـMySQL الوحيدة المهيّأة في `.env` هي **الإنتاج** — وبرومت المرحلة يمنع صراحةً القياس على بيئة فيها بيانات مستخدمين. أوامر التشغيل جاهزة حرفياً في `docs/LOAD_TEST_RESULTS.md` §1. لا تستبدل gunicorn بـwaitress ولا MySQL بـSQLite: بديلٌ كهذا يُنتج أرقاماً كاذبة، وكذبة موثّقة أسوأ من فجوة معروفة.

**بعد القياس:** يُحسم قرار Celery ومعه P1-12/P1-13، ويُملأ جدول النتائج.
...
**⚠️ يحتاج تحقق دخان على MySQL قبل الاعتماد:** **P2-4** — فرض المفاتيح الأجنبية صار الافتراضي بعد أن كان معطّلاً على كل اتصال إنتاجي طوال عمر النظام. اختبارات المشروع على SQLite لا تكشف قيود FK. **أداة الفحص جاهزة (قراءة فقط):** `python manage.py audit_fk_orphans` على قاعدة الإنتاج — تطبع نسخة الخادم وتعدّ الصفوف اليتيمة لكل علاقة FK؛ صفر يتيم = الفرض آمن. (النسخة مهمة أيضاً: فهارس DESC حقيقية على MySQL 8+ ومتجاهَلة على 5.7 — قراءة عكسية أبطأ قليلاً لكن صحيحة.) مخرج الطوارئ الفوري بلا نشر جديد: `MYSQL_DISABLE_FK_CHECKS=1`.
...
| **P0-12** شاشة الأصناف | `385a173` | ترقيم خادمي فعلي: طلب واحد لكل دورة بحث بدل 8 طلبات متسلسلة. |
...
| 6 | اختبار حمل 500 مستخدم + قرار Celery | **لم تُنفَّذ** — تتطلب بيئة تشبه الإنتاج (gunicorn multi-worker + MySQL)؛ حاوية الوكيل SQLite بلا خادم MySQL، وبرومت المرحلة نفسه يمنع تشغيلها على غير ذلك | ⬜ **المرحلة الوحيدة المتبقية** |

**بنود ما زالت مفتوحة (بعد جلسة «كل النواقص» 2026-08-11 — كلها بقرار موثّق لا بإهمال):**
- **P1-11** (virtualization) — مؤجَّل: `AseelDenseTable` هدف طباعة، والنافذة المرئية تعني طباعة ~20 صفاً بدل الجدول؛ وقيمته استُهلكت أصلاً بـP0-5/P0-12.
- **P1-12 / P1-13** — قرارهما Celery، وهو معلَّق على قياس المرحلة 6 بنصّ البرومت.
...
**حكم المراجعة:** الخطة حقّقت أهدافها المعلنة في المراحل 0–5 بما فيها معالجة كل الديون الجانبية المسجّلة. المتبقي الوحيد من الخطة نفسها هو **المرحلة 6** (قياس لا بناء)، وشرطها المسبق — بنود P0 منفّذة — صار متحققاً.
