...
### ✅ تأكيد بالقياس (2026-08-12) — وتصحيحٌ لموضع الاختناق

قِيسَ التدرّج 50→150→300→500 فعلياً (`docs/LOAD_TEST_RESULTS.md`). **حكم التدقيق
أعلاه صحّ:** فشل عند 500 (p95 = 29 ثانية مقابل عتبة ثانيتين). لكن **بعد** إصلاح
بنود P0 صار الاختناق **واحداً لا خمسة**: البند 1 وحده (سعة الـworkers).

- **السعة العملية ≈ 50 مستخدماً** على العتاد المقيس (نواة واحدة، 3 sync workers).
- **الإنتاجية ثابتة 13.6–16.9 ط/ث في المستويات الأربعة** — بصمة إشباعٍ كامل:
  السقف = `workers ÷ زمن الخدمة`، والمستخدم الإضافي يُنتج طابوراً لا عملاً.
- **صفر أخطاء عند 150 و300 و500** — النظام يبطئ ولا ينهار.
- البنود 2 و3 و4 و5 (الترقيم، الكاش، الـthrottle، تقارير آلاف الاستعلامات)
  **لم تعد تظهر في القياس**: أثقل نقطة عند 50 هي `payables-aging` بـ2.1 ثانية على
  قاعدة **أكبر من الإنتاج**.
- ⚠️ **التحفّظ ملزم:** locust زاحم gunicorn وMySQL على نواة واحدة، والإنتاج كان
  يخدم مستخدمين حقيقيين بالتوازي. الأرقام تصف **هذا النشر بهذا العتاد** لا سقف
  الكود على عتاد مخصّص.
...
>
> **تحديث 2026-08-12 (بعد قياس المرحلة 6):** أُضيف **P0-14** — وهو أول بند في هذا
> الجدول محورُه **جاهزية التعافي** لا الحمل ولا الأمن: النظام يعمل تماماً تحت
> الضغط، لكنه كان **لا يُبنى من الهجرات**. صُنِّف P0 لأن أثره كليّ لا تدريجي (خادم
> جديد ⇒ مخطط ناقص)، ولأنه لا يُكتشَف إلا لحظة الحاجة إليه — أي أثناء كارثة.
> **أُصلح في اليوم نفسه، فجدول P0 كامل ✅ من جديد.**
...
| ✅ P0-14 | **القاعدة لا تُبنى من الهجرات:** `SystemAttachment` كان معرَّفاً بـ`managed = False` — النموذج الوحيد كذلك في المشروع. Django يسجّل `core.0001_initial` مطبَّقةً `[X]` **ولا يُنشئ جدول `system_attachments` إطلاقاً**، فأي عملية تلمسه تنفجر بـ`(1146) Table doesn't exist`. الإنتاج يملك الجدول لأنه أُنشئ يدوياً خارج Django. **أُثبت عملياً 2026-08-12** أثناء تجهيز قاعدة الحمل (حذف تعاقبي انفجر). بقيّة فئة عطل عولجت سابقاً في `chartofaccounts`/`partners`/`tenants` وسقط منها هذا النموذج — *(**أُصلح 2026-08-12.** وكشف الإصلاحُ عطلاً ثانياً أخطر: **مُكتشِف الهجرات يتجاهل حقول النماذج غير المُدارة**، فحقل `tenant` عاش في النموذج ولم يدخل `CreateModel` قطّ — أي أن قلب `managed` وحده كان سيُنشئ جدولاً **بلا عمود `TenantID`**. أُصلح الأمران معاً. **التحقق لم يكن باختبارات SQLite** بل بـ`migrate` كامل على قاعدة MySQL 8 فارغة: الجدول يُبنى بأعمدته السبعة + FK إلى `tenants`. آمن على الإنتاج لأن الهجرة مسجَّلة فلا تُعاد)* | `core/models.py` (`SystemAttachment`) · `core/migrations/0001_initial.py` | تعافٍ |
...
>
> **تحديث 2026-08-12 (بعد القياس):** قياس المرحلة 6 **لم يحسم P1-12/P1-13** —
> سيناريو الحمل لا يستهدفهما أصلاً، فبقي قرارهما معلّقاً بسببٍ جديد أدقّ من
> السابق (التفصيل في صفَّيهما). وأُضيف **P1-16** — عطل في عدّة الاختبار نفسها
> ظهر من أول تشغيل فعلي لها، **وأُصلح في اليوم نفسه**.
...
| ⬜ P1-11 | لا virtualization في أي جدول — **مؤجَّل بقرار:** `KitDenseTable` هدف طباعة (`ktra-print-hidden`)، والنافذة المرئية تعني طباعة ~20 صفاً بدل الجدول كاملاً؛ وقيمته استُهلكت أصلاً بـP0-5/P0-12 (الشاشات صارت 50-200 صفاً خادمياً لا 10 آلاف) | `KitDenseTable.tsx:232` · `GroupedItemsTable.tsx:59` | يوم-يومان |
| ⬜ P1-12 | ترحيل/إلغاء ترحيل دفعي لفواتير الشحنة داخل طلب واحد و`transaction.atomic` طويلة؛ واستلام مخزون الشحنة داخل signal (مئات الحركات لحفظة واحدة) | `logistics/views/` · `logistics/signals.py` → `inventory/services.py` | **القرار ما زال معلّقاً بعد قياس المرحلة 6 — لأن القياس لم يستهدفه:** `load_tests/locustfile.py` يرحّل فاتورة **واحدة** كل مرة، لا دفعة شحنة. يحتاج سيناريو دفعة حقيقية |
| ⬜ P1-13 | `_recompute_product_stock`: `select_for_update()` + إعادة تشغيل كل حركات الصنف — تنازع أقفال لا-خطي على الأصناف الشائعة | `inventory/services.py:268-299,323-324` | **معلّق لنفس السبب:** السيناريو يوزّع الأصناف ولا يُجبر مستخدمَين متزامنَين على **نفس الصنف الشائع** — وهو الشرط الوحيد الذي يُظهر التنازع. يحتاج سيناريو تزاحم مقصود |
| ✅ P1-16 | **حارس البذر يرفض كل قاعدة نظيفة:** الطبقة 3 في `seed_load_test_data` ترفض القاعدة إن وجدت شركة خارج بادئة `LOADTEST` — لكن هجرات Django نفسها تُنشئ «Default Company» (`tenants/migrations/0005_backfill_default_company.py`)، فالقاعدة النظيفة — وهي بالضبط الحالة التي بُني الحارس ليسمح بها — **مرفوضة بالتعريف**. لم يظهر قبل 2026-08-12 لأن العدّة بُنيت ولم تُشغَّل قطّ — *(**أُصلح 2026-08-12** باستثناء `DEFAULT_TENANT_NAME` وحدها. تحقّق موجب وسالب على قاعدة MySQL فعلية: البذر يمرّ على قاعدة نظيفة، ويبقى **يرفض** قاعدةً فيها شركة حقيقية ويسمّيها في الرسالة بلا أن يذكر الشركة الافتراضية — أي أن الحماية لم تُمَسّ)* | `tenants/management/commands/seed_load_test_data.py` | — |
...
| ✅ P2-4 | `init_command` يعطّل `foreign_key_checks` على كل اتصال إنتاجي — *(الفرض صار الافتراضي مع مخرج طوارئ `MYSQL_DISABLE_FK_CHECKS=1`.* **✅ تحقق الدخان أُنجز 2026-08-12 على قاعدة الإنتاج:** MySQL 8.4.11 · 550 قيد FK معرَّف · 550 مفحوصاً · **صفر صفوف يتيمة** ⇒ الفرض آمن. ظهر يتيمان في أول تشغيل (بند صفقة أمُّه محذوفة، وإعدادات مبيعات تشير لعميل محذوف) فأُصلحا ثم عاد الفحص صفراً. التفصيل في `docs/LOAD_TEST_RESULTS.md §5.3`*)* | `core/settings.py` · `accounting/management/commands/audit_fk_orphans.py` |
...
`core/settings.py:172-173`: `CONN_MAX_AGE=60` + `CONN_HEALTH_CHECKS=True`، مع `charset=utf8mb4` ومهلات اتصال/قراءة/كتابة (`:174-180`). **بند المرحلة 5 رقم 3 يُشطب.** الملاحظة المتبقية: `init_command` يعطّل `foreign_key_checks` (P2-4) — **أُغلقت 2026-08-12 بتحقق دخان على الإنتاج: 550 قيداً، صفر يتيم.**
...
- **Virtualization: صفر** — كل الجداول `rows.map()` في DOM (`KitDenseTable.tsx:232`، `GroupedItemsTable.tsx:59`).
...
### مرشّحو Celery — **القرار صدر 2026-08-12: غير مطلوب للحمل المقيس**

> **الحكم كاملاً بأرقامه في `docs/LOAD_TEST_RESULTS.md §4`.** خلاصته: عند 50
> مستخدماً (قبل الإشباع، حيث تُقرأ الكلفة الحقيقية) **أبطأ نقطة 2.1 ثانية** —
> دون عتبة القرار (3–5 ثوانٍ). وعند 500 تتساوى **كل** النقاط عند p95=29 ثانية
> بالضبط، وهو ما يُثبت أن الانتظار في **الطابور** لا في التنفيذ. والسقف
> = `workers ÷ زمن الخدمة` (3 ÷ 0.2 ≈ 15 ط/ث، والمقيس 13.6–16.9) — ونقل العمل
> لمهام خلفية **لا يرفعه**. ما يرفعه: أنوية أكثر، ثم `REDIS_URL`.
>
> **⚠️ استثناءان لم يقسهما السيناريو، فقرارهما باقٍ معلّقاً:** `P1-12`
> (ترحيل دفعي — السيناريو يرحّل فاتورة واحدة لا دفعة شحنة) و`P1-13` (تنازع
> أقفال — السيناريو لا يُجبر مستخدمَين على نفس الصنف الشائع). غياب العَرَض من
> سيناريو لا يستهدفه ليس دليل غياب العلّة.
>
> وأربعة مرشّحين آخرين خارج خليط المستخدم العادي بطبيعتهم فلم يُقاسوا: رفع
> Cloudinary، وLanded Cost، وإقفال السنة، وإعادة مطابقة COGS والجرد.

القائمة الأصلية للمرجع:
