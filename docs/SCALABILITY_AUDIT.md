...
| P1-10 | واجهة: شاشات تجلب جداول كاملة — تقييم المخزون (كل الحركات!)، شاشة القيد (كل القيود لأزرار تنقّل)، فخّ `page_size` بلا `page` (لا يقيّد شيئاً) | `InventoryValuationPage.tsx:130-136` · `AccountingJournalEntryPage.tsx:291` · `activityService.ts:34` · `GoodsReceiptsPage.tsx:167` | يوم |
