...
| GET | `practice/dashboard/` | `practice_views.PracticeDashboardView` (ISSUE #58) — **البوابة الوحيدة المخفَّفة** في سطح المكتب: صاحب ملفٍ مهني يرى `practice_dashboard` (عملاؤه، `practice_deadlines`، والأتعاب غير المحصّلة من دفتره هو وحده)؛ مستخدمٌ بلا `AccountantProfile` (موظف مكتب، القرار 7) يرى `staff_practice_dashboard` — عملاء الدفاتر المُدارة التي هو عضوٌ فيها (`UserCompanyMembership` على `PracticeClient.managed_tenant`، لا حقل إسنادٍ ثالث) فقط، فارغة إن لم يُسنَد له شيء — 200 لا 404. كل مسار آخر في `practice_views.py` يبقى خلف `PracticeView` (يتطلّب ملفاً مهنياً) بلا تخفيف. **ISSUE #87**: نفس النقطة صارت أيضاً مصدر شاشة بداية شركة `accounting_firm` نفسها (`frontend_v2/components/office/OfficeHomeDashboard.tsx`) — لا فرقٌ عن استهلاك `OfficeDashboardPage` على `/office` سوى غياب أزرار التنقّل بين الشركات |
| GET | `/api/accountant/client/documents\|statements\|trend\|summary/` | `ClientDocumentsView` … — `summary`/`trend` (`ClientSummaryView`/`ClientTrendView`) صارتا أيضاً مصدر شاشة بداية دفتر العميل (ISSUE #87، `frontend_v2/components/office/ClientBookFinancialPosition.tsx`) بجانب استهلاكهما القائم في `OfficeClientFilePage`؛ تحت `guard_module_surface` فتردّان 404 بلا ترخيص `accountant_portal` — `client_book` يُرخَّص له تلقائياً عند `create_company` (ISSUE #87 مراجعة، `docs/modules/tenants.md`)، ودفترٌ قائم قبلها يبقى بلا ترخيص حتى يُفعَّل يدوياً من لوحة المنصة |
...
| GET/POST | `/api/accountant/review/queries/` (+ `{id}/answer\|resolve\|withdraw/`) | `ReviewQueryListCreateView` · `ReviewQueryActionView` — أول مستهلكٍ في `frontend_v2/` صار زرّ «ناقِش» في `components/accounting/DocumentCodingPage.tsx` (ISSUE #85، `accountantApi.createReviewQuery`)؛ `entity_type="other"` لأن السندين `expense_voucher`/`revenue_voucher` غير مسجَّلين في `REVIEW_ENTITY_MODELS` بعد — يلزمه ارتباطٌ نشط (`active_engagement`) كسائر هذا المسار |
