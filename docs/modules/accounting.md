...
| `accounting/management/commands/fix_logistics_unpost_cycles.py` | F-1: كشف وتصحيح دورات إلغاء ترحيل دفعات الصفقات القديمة — انظر «تصحيح دورات إلغاء الترحيل القديمة» أدناه | 351 |

### تصحيح دورات إلغاء الترحيل القديمة (`fix_logistics_unpost_cycles`)
نمط إلغاء ترحيل دفعة الصفقة قبل `3358bf7` كان يُلغي ترحيل الأصل (`LOGISTICS_PAYMENT`) ويسِمه
«[ملغى ترحيل — عكس مرحّل #R]» ويُبقي العكس `LOGISTICS_PAYMENT_UNPOST` مرحّلاً ⇒ الدفاتر تحمل
العكس وحده. الأمر `python manage.py fix_logistics_unpost_cycles [--tenant N] [--apply]` يطابق
الأصل بعكسه **بالرقم المكتوب في الوسم** (لا بالمرجع وحده) ويشترط أن تعاكس أسطرُه أسطرَه، ثم:
- **repost** — فترة الأصل مفتوحة بحرّاس `assert_dates_open_for_unpost` (فترة مالية + حرّاس
  الفترة الضريبية + كشف ض.ق.م نهائي): يُرحَّل الأصل نفسه بـ`post_journal_entry` فيتعادل مع عكسه.
- **adjust** — الفترة مقفلة، **أو** أثر الأصل والعكس بالعملة الأساسية لا يتعادل (العكس القديم
  لم ينسخ العملة فسعره 1): قيد تسوية واحد بتاريخ اليوم عبر `post_journal` بأسطر الأصل وبمرجع
  `LOGISTICS_PAYMENT` للدفعة (فيشمله إلغاء ترحيل الصفقة و`purge_deals`)، بعملة العكس وسعره عند
  اختلاف الأساسي، وبـ`mirrors_posted_lines=True` لأنه ينسخ وسمَ الأصل القديم كما هو؛ والأصل يبقى غير مرحّل ويُوسَم «صُحِّح بقيد تسوية #A». **وعكسٌ يعاكس الأصل
  حساباً ومبلغاً ويختلف في وسم الطرف وحده** (العكس القديم وسم سطر النقدية بالمورّد — شكلُ
  دفعات الإنتاج المحذوفة) يُسوّى دائماً بنفي أسطر **العكس** نفسها بعملته وسعره، فيعود كلُّ
  (حساب، طرف) صفراً؛ إعادةُ ترحيل الأصل كانت ستترك الوسمَ الزائد في رصيد المورّد.
- **skip** — أصلٌ مكرر لعكسٍ عُودل (يُصحَّح أدنى معرّف وحده)، وسمٌ لا يطابق عكساً مرحّلاً،
  أسطرٌ لا تتعاكس حساباً ومبلغاً، أو دفعةٌ ما زالت تشير إلى أصلٍ لهذا العكس.
- **report** — الشكل الثاني: دفعة `is_posted=True` وقيدها غائب أو غير مرحّل أو معكوس بوسم — تقرير
  فقط، لأن الصحيح (إعادة ترحيل أم إلغاء) يتوقف على نيّة الدفعة ومبلغها الحالي.

الافتراض تقرير فقط؛ `--apply` ينفّذ كل صفّ في معاملة ذرّية مستقلة مع إعادة التحقق تحت القفل،
ويكتب سطر تدقيق — التشغيل الثاني لا يجد ما يفعله، ولا يُحذف قيد.
...
| `JournalLine` | `debit`، `credit`، `base_debit`، `base_credit`، `description`، `amount_currency` (موقَّع: مدين − دائن بالعملة الأجنبية) و`currency_code` — NULL لسطرٍ بعملة الأساس | `journal` (CASCADE)، `account` (PROTECT)، `partner` (SET_NULL)، `cost_center` |
...
def post_journal(*, tenant_id: int, transaction_date, reference_type: str, reference_id: int | None, description: str, lines_data: list[dict], currency=None, exchange_rate=Decimal("1"), user=None, idempotent: bool = True, branch_id: int | None = None, mirrors_posted_lines: bool = False) -> JournalHeader:  # المسار الوحيد لإنشاء + ترحيل أي قيد؛ يرفض وسمَ طرفٍ على غير ذمّته إلا لقيدٍ يعادل قيداً مرحّلاً سطراً بسطر
def foreign_partner_tag_rows(tenant_id: int, lines_data: list[dict]) -> list[tuple]:  # أسطرٌ تحمل طرفاً على حسابٍ ليس ذمّته — حارسُ post_journal
...
def assert_no_final_vat_statement(tenant_id, transaction_date, document_label="", *, posting=False):  # يرفض فكّ ترحيل مستندٍ **وترحيلَه** داخل فترة كشف ض.ق.م `final` (issue #79 · A2-1) — من unpost_document، و`posting=True` من post_journal/post_journal_entry/api.reverse_journal
...
def record_reconciliation_adjustment(reconciliation, *, kind, amount, account_id, date=None, description="", exchange_rate=None, user=None):  # A2-3: «قيد تسوية» (بنكٌ بعملةٍ أجنبية يشترط سعراً صريحاً) — سند مصروف/إيراد طرفُه حساب البنك المطابَق، ويُؤشَّر سطره مطابَقاً ذرّياً؛ يرفض المطابقة المُقفلة وتاريخاً بعد الكشف وحساباً مقابلاً من نوعٍ آخر
def partner_account_statement(*, tenant_id: int, partner_id: int, is_supplier: bool, limit: int = 50, offset: int = 0, ordering: str = "newest", only_payments: bool = False, anchor_reference_type: str | None = None, anchor_reference_id: int | None = None) -> dict:  # كشف حساب الطرف برصيد قبل الحركة وبعدها؛ only_payments يستثني الفاتورة من المعروض ولا يمسّ الحساب؛ والمرساة تُركّز النافذة على مستندٍ بعينه وتُرجع `anchor` (قبل/بعد/الأثر) بلا أن تمسّ الحساب؛ وكل حركة لوجستية تحمل `shipment_label` الحيّ من مستندها (`logistics.domain.party_accruals.journal_reference_shipment_labels`)؛ و`link_key` يجمع المستند وسنده (`_attach_statement_document_links`): الفاتورة، أو مستحقّ التخليص/الإرسالية/الشحن مع دفعاته المباشرة وسنداته الموزَّعة (`journal_reference_accrual_links`)، ودفعة الصفقة مع فاتورتها الدولية؛ والسند الموزَّع على أكثر من مستند صفٌّ واحد بـ`link_targets` (مفتاح كل مستند ومبلغه) تعرضها الواجهة سطراً معلوماتياً بلا رصيد داخل كل مجموعة
def set_lines_amount_currency(tenant_id: int, updates: dict) -> int:  # تعبئة amount_currency لأسطرٍ قائمة فارغة (أمر التعبئة وحده) — لا تمسّ المبالغ ولا سطراً معبّأً
def statement_reversal_pairs(tenant_id: int, lines) -> dict[int, tuple[int, int]]:  # أزواج «قيد + عكسه» صافيها على الطرف صفر: {journal_id: (الأصل، العكس)} — يقرؤها الكشف وتقريره
...
| GET · POST | `vat-statements/` · `vat-statements/finalize/` (`period_from`/`period_to`) · `vat-statements/{id}/finalize/` · `vat-statements/{id}/reopen/` (`reason` إلزامي) | `VatStatementViewSet` (A2-1) — الاعتماد بـ`accounting.period.manage`، وإعادة الفتح لدور `manager` وحده (لا صلاحية قابلة للمنح)؛ كلاهما في `AccountingAuditLog` |
| GET | `bank-accounts/{id}/statement/` | `BankAccountViewSet` |
| POST | `bank-reconciliations/{id}/toggle-line/` · `close/` · `reopen/` · `adjustment/` | `BankReconciliationViewSet` — `adjustment/` (A2-3): `kind` (`expense`/`revenue`)، `amount`، `date`، `account`، `description` ⇒ `record_reconciliation_adjustment`، بـ`accounting.journal.create` + `finance.expense.create`/`finance.revenue.create` |
...
- **الشريك على سطر الحساب الرقابي وحده (ذمم مدينة/دائنة)** — `partner_posted_balance` وقارئا القوائم يجمعون كل أسطر الشريك بلا فلتر حساب، فمخزونٌ أو ضريبةٌ أو وسيطٌ موسومٌ بالمورد يُلغي دائنَ ذممه ويُظهره بصفر. مطبَّقٌ في كل مسارات الشراء: `logistics/views/invoices.py` (`post_to_accounting`)، `logistics/services.py` (`receive_purchase_invoice`، `post_purchase_return`)، و`accounting/views.py` (`PurchaseReceiptViewSet`)، **ودفعُ المخلّص** (`logistics/views/clearance.py` — `pay_from_cashbox`) **ودفعُ وكيل الشحن بلا FIFO** (`logistics/views/shipments.py` — `post_agent_payment`) اللذان كانا يَسِمان الصندوقَ فلا يُنقص الدفعُ رصيدَهما. القيود القديمة يُصلحها `python manage.py fix_purchase_partner_tags [--tenant N] [--apply]` (`accounting/management/commands/fix_purchase_partner_tags.py`) — ويشمل قيودَ الاستيراد (`LOGISTICS_DEAL` · `LOGISTICS_PAYMENT`/`_UNPOST` · `CLEARANCE_PAYMENT`/`_UNPOST`، و`JOURNAL_REVERSAL` لعكس قيدٍ مشمولٍ وحده: عكسُ بيعٍ يُزال وسمُه ويبقى وسمُ أصله يغيّر رصيدَ الزبون) — عرضٌ افتراضياً، يُزيل الوسم عن غير الرقابي وحده، ويترك للمراجعة قيداً لا سطرَ ذممٍ معروفاً فيه؛ يكتب بـ`QuerySet.update` على `partner` وحده كسابقته `fix_legacy_cash_partner_tags`. **ومفروضٌ في `post_journal` نفسها** (`foreign_partner_tag_rows`) بعد أن تكرّر الخلل ثلاثاً (مشتريات، استيراد، استحقاق النقل المحلي): وسمٌ على حساب الطرف نفسه أو على أي حساب ذمم (`sub_type` مدينة/دائنة) يمرّ؛ على صندوق/بنك/مخزون أو مصروف/إيراد يُرفض دائماً؛ وعلى غير ذلك (ضريبة، GR/IR، فروق صرف) يُرفض إن كان للطرف حسابُه. الرفض قبل إنشاء القيد. معفًى: `accounting/api.py` (`reverse_journal`) لأنه يكتب أسطره مباشرةً، و`mirrors_posted_lines=True` لقيد تسويةٍ يعادل قيداً قديماً سطراً بسطر. **القيد اليدوي** (`JournalHeaderSerializer` ثم `post_journal_entry`) لا يمرّ به.
...
- **الشيك لا يتحرك خارج جدول اتجاهه** — `accounting/services.py` (`INCOMING_TRANSITIONS`) و`accounting/services.py` (`OUTGOING_TRANSITIONS`) هما المصدر الواحد، و`accounting/services.py` (`transfer_cheque`) ترفض الانتقال غير المسموح، وترفض أي حركة على شيكٍ سندُه غير مرحّل، وتكتب `ChequeMovement` مربوطةً بقيدها. والورقة المظهَّرة لها مخرج واحد هو `bounce` (قيدٌ بين ذمّتين: العميل الساحب يعود مديناً والمستفيد تعود ذمّته، بلا مساس بحسابَي الشيكات).
  ...
  **واستثناءٌ واحدٌ محصور: مسارُ تحرير المستند.** إلغاءُ ترحيل مستندٍ يُرجع أوراقَه بالكتابة المباشرة على الحالة لا عبر الجدول — `accounting/services.py` (`record_document_cheque_unposting`) يعيد شيكات السند إلى `Draft`، و`sales/services/flow.py` (`release_auto_sales_return_refund`) يعيد ما أعاده مرتجعُ البيع تلقائياً إلى `Received`. كلاهما **يكتب حركةَ `revert`** فلا يُمحى سجلُّ الورقة. وهذا **ليس فتحاً للانتقال**: `Returned` و`Draft` تبقيان كما هما في الجدول ولا يبلغهما المستخدم بزرّ — التحريرُ عكسُ ترحيلٍ وقع، لا حركةٌ جديدة في حياة الورقة. الوسمُ الذي يحدّد ما يُحرَّر بنيويٌّ (`ChequeMovement.sales_return`) لا نصٌّ في `notes`.

  **وتسويةُ رصيد مرتجع البيع بالورق تقرأ الحركة الفعّالة وحدها**: `sales/services/calc.py` (`linked_return_credit_summary`) يجمع `return_to_customer` الموسومة بالمرتجع حيث `journal_id` موجود؛ حركةٌ فُكّ قيدها أو فُرّغ وسمُها عند التحرير لا تبقى سداداً صامتاً في أعمار الذمم أو سقف ردّ النقد.
...
- **حسابُ السند: نصٌّ حرّ أم ربطٌ بالشجرة (بلاغ المالك)** — `TenantSettings.voucher_account_entry_mode` (`free` الافتراضي · `linked`) يقرأه `_voucher_account_entry_is_linked` (`accounting/services.py`) — **نقطة قراءةٍ واحدة** يستدعيها `create_expense_voucher` و`create_revenue_voucher` معاً فلا تنحرف بوابةٌ عن أخرى. في `linked` يُرفض `expense_account_name`/`revenue_account_name` برسالةٍ عربية صريحة ولا يُنبَت حسابٌ جديد؛ في `free` يبقى سلوك #56/#80 حرفياً. غيابُ صفّ الإعدادات = `free` (فشلٌ **مفتوح** عمداً). إخفاءُ خانة الاسم الحرّ في الواجهة (`utils/voucherAccountEntryMode.ts`) تجميلٌ يتبع الحارس لا يحلّ محلّه.
- **مصدرٌ واحد لأرقام ض.ق.م (issue #79)**: `vat_period_totals` (`accounting/services.py`) تقرأ `JournalLine` وحدها — لا `SalesInvoice.tax_amount` — لأن كل قيدٍ يمرّ بـ`post_journal` فهي وحدها ترى كل مصدر ضريبة (سند مصروف · إيصال استلام · فاتورة شراء · فاتورة بيع). `sales.services.build_vat_statement` (يحفظ)، `VatReportView` (يعرض)، و`accountant_portal.services.client_financial_summary` (ملخص الزبون) يستدعونها وحدها. **مجموعة حسابات الضريبة اتحادٌ لا اشتقاقٌ واحد**: المخرجات = `TaxRate.tax_account` (اتجاه `sales`/`both`) — كل مسار ترحيلٍ للمخرجات من فاتورة يمرّ بها فعلاً (`sales/services/calc.py` — `_build_tax_buckets`) — **∪ سلسلة السقوط الحرفية** (الكود `2104` إن وُجد، وإلا **كل** حساب `Liability` اسمه يحوي «ضريبة» أو «VAT») **issue #80 (مراجعة الالتزام)**: `create_revenue_voucher` يكتب على «2104» بالكود مباشرةً لا عبر `TaxRate`، فشركةٌ بلا `TaxRate` إطلاقاً (حال «دفتر العميل»: محاسبٌ يرمّز سندات إيرادٍ ومصروفٍ ولا يفتح فاتورة بيعٍ واحدة) كانت تُصفِّر مخرجاتها المُرحَّلة فعلاً في الكشف — مرآة سقوط المدخلات حرفياً. المدخلات = `SalesSettings.vat_input_account` ∪ `TaxRate.tax_account` (اتجاه `purchase`/`both`) **∪ سلسلة السقوط الحرفية** (الكود `1105` ثم `Asset` باسمٍ يحوي «ضريبة») لأن مسارات ترحيلٍ فعلية — `logistics/services.py` (`_resolve_vat_input_account`) و`accounting/views.py` (مسار استلام فاتورة شراء قديم) — تجد الحساب بالكود مباشرةً بلا قراءة `SalesSettings`/`TaxRate` إطلاقاً؛ اشتقاقٌ وحده كان يُصفِّر ضريبة مدخلاتٍ مُرحَّلة فعلاً لشركةٍ لم تُعيَّن فيها `SalesSettings.vat_input_account` (الحال الافتراضي) فيُبالَغ في «الصافي المستحق». **لا أثر رجعي**: كشفٌ محفوظ (مسودة أو نهائي) لا تُعاد كتابة أرقامه أبداً؛ `sales.services.vat_statement_diff_report` تقرأ الفرق بين المحفوظ والمحسوب الآن **بلا كتابة**. وفكّ ترحيل مستندٍ مؤرَّخ داخل فترة كشف `final` مرفوض من `unpost_document` (`assert_no_final_vat_statement`) — الإقرار المقدَّم يُصحَّح بإقرار معدَّل لا بتعديل صامت. **وترحيلُه كذلك (A2-1، قفلٌ ضريبي بنمط Tax Lock Date)**: `post_journal` و`post_journal_entry` و`accounting/api.py` (`reverse_journal`) تستدعي الحارس نفسه بـ`posting=True` — قاعدةٌ واحدة لا نسختان. يضع `final` حصراً `sales.services.finalize_vat_statement` (يحدّث الأرقام من الدفتر لحظتها)، والمخرج الوحيد `reopen_vat_statement` بسببٍ مسجَّل وللمدير وحده (`VatStatementViewSet.reopen`).

- **القيد المعكوس وعكسه في كشف الطرف زوجٌ يُسمّى ولا يُحذف** — `services.py` (`statement_reversal_pairs`): العكس يُعرف بـ`JOURNAL_REVERSAL` ومرجعه، أو «عكس (ال)قيد #N»/«عكس #N» في وصف الرأس أو السطر، أو نوعٍ ينتهي بـ`_UNPOST`/`_REVERSAL` فيُطابَق بأحدث قيدٍ قبله من النوع الأصلي على المرجع نفسه. **يُقرن فقط إن كان صافيهما على الطرف صفراً** — العكس الجزئي حركةٌ حقيقية. `partner_account_statement` يرسل كل الأسطر ومعها `reversal_pair_id`/`reversal_pair` و`balance_before_folded`/`running_balance_folded` (الرصيد بلا الأزواج)؛ والختامي لا يتغيّر. الطيّ نفسه قرار العرض: الشاشة (`frontend_v2/utils/entityLinks.ts` — `foldStatementReversals`) وتقرير «كشف حساب طرف» (`core/reports/ledger_import.py` — `_partner_statement`، فلتر `reversals`).

## الاختبارات المهمة
| الملف | ما يغطيه |
|---|---|
| `accounting/tests/test_fix_logistics_unpost_cycles.py` (F-1) | فترة مفتوحة ← إعادة ترحيل الأصل وصافي صفر وإعادة التشغيل بلا أثر · فترة مقفلة أو كشف ض.ق.م نهائي ← قيد تسوية واحد بتاريخ اليوم والأصل غير مرحّل · اختلاف السعر ← تسوية بسعر العكس · أصلان لعكسٍ واحد ← يُصحَّح واحد · فلتر الشركة · الشكل الثاني تقرير فقط |
...
| `accounting/tests/test_fix_duplicate_deal_journals.py` | قيدُ شراء الصفقة المُرحَّل مرّتين: المتطابقان يبقى أوّلهما، والمختلفان يبقى المطابقُ لإجمالي الصفقة (ولو كان الأقدم)، وما لا يطابق شيئاً يُترك للمراجعة، والتقرير لا يغيّر والتكرار بلا أثر وفلترُ الشركة يحصر |
| `accounting/tests/test_fix_purchase_partner_tags.py` | إصلاح وسم المورد على قيود الشراء والاستيراد القديمة: العرض لا يغيّر، `--apply` يُبقي الذمم وحدها موسومة، الملتبس يُترك، العكسُ العامّ لقيدٍ غير مشمول يُترك، التكرار بلا أثر، و`--tenant` يحصر |
| `accounting/tests/test_unpost_document.py` | حذف كل قيود مستند بحدوده وحده + إعادة حركات المخزون + الذرّية (324 سطر) |
| `accounting/tests/test_banks_and_reconciliation.py` | البنوك والفروع والحسابات والمطابقة البنكية، و«قيد التسوية» (A2-3): سند مصروف/إيراد على حساب البنك يُؤشَّر مطابَقاً ويغيّر الفرق، والمطابقة المُقفلة ترفض، والحساب المقابل من نوعٍ أو شركةٍ أخرى مرفوض |
| `accounting/tests/test_vat_final_lock.py` | A2-1: الاعتماد النهائي يحفظ أرقام الدفتر ويضع `final`، ويُرفض ترحيل قيد/فاتورة بيع/قيد يدوي داخل الفترة ويُقبل خارجها، وفكّ الترحيل يُرفض، وإعادة الفتح بسببٍ تُعيد الترحيل، والمحاسب لا يعيد الفتح، وعزل الشركات |
...
| `logistics/tests/test_partner_statement_usd.py` | كشف الدولار: دفعة 3.24 واستحقاق 3.6 بالدولار نفسه = صفر وبالشيكل 360، سطرٌ بلا دولار يُنبَّه ولا يدخل الرصيد، دفعة صندوق FIFO تحمل دولارها، حارس إشارة `post_journal`، العكس اليدوي بعملة الأصل وسعره، وأمر التعبئة يعطي yoyo 3,384.59$ والشيكل بلا تغيير |
| `accounting/tests/test_statement_reversal_pairs.py` | «القيد + عكسه» بصافي صفر يُعلَّم زوجاً والختامي واحد مطويّاً وخاماً؛ العكس الجزئي ليس زوجاً؛ العكس اليدوي بمرجعه و`_UNPOST` بلا نصّ بمستنده؛ لا زوج عبر الشركات؛ التقرير المطبوع يطوي افتراضياً ولا يطوي زوجاً تقسمه الفترة |
...
| `accounting/tests/test_voucher_account_entry_mode.py` (بلاغ المالك) | `voucher_account_entry_mode`: الافتراضي يقبل النصّ الحرّ وينبت الحساب (تراجعٌ صريح للسندين)، و`linked` يردّ 400 ولا ينبت حساباً ولا يخلق سنداً، ويقبل حساباً من الشجرة كما كان؛ والقيمة تصل الواجهة على `settings/current/` |
