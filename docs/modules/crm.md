...
7. **قاعدةُ «من يبتّ في التحويل» في `can_decide_lead_transfer` وحدَها** —
   ...
   مالكُ يومِ الطلب، والعميلُ قد ينتقل بعدَه. ولذلك `LeadTransferViewSet.get_queryset`
   يضمّ `lead__assigned_to` إلى `from_employee | to_employee` (D-1): بدونه لا يرى
   صاحبُ العميل **الآن** طلباً كُتب قبل انتقاله، و`decide` (عبر `get_object`) يردّه ٤٠٤.
