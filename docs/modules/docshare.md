...
**السطح العام مسجَّل مرتين عمداً:** nginx على الإنتاج يخدم الـSPA من القرص ويمرّر `/api/` وحده إلى جانغو، فالمسار القصير `/s/<token>` يحتاج سطر `location` جديداً هناك (خطوة نشر). حتى تُضاف، يعمل `/api/share/<token>/` فوراً — فلا ميزة معطَّلة بانتظار إعداد خادم.
...
وقبلها يعمل `https://ktra-pro.tech/api/share/<token>/` بلا لمس السيرفر.
أساس الرابط المنسوخ يُقرأ من `DOCSHARE_PUBLIC_BASE_URL` (‏`core/settings.py`) لا من ترويسة `Host` الواردة: الرابط يُلصق في واتساب ويعيش شهراً، فلا يجوز أن يتحدّد بما يرسله العميل.
