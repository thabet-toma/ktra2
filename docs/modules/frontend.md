...
| `frontend_v2/components/accounting/DocumentCodingPage.tsx` | **ISSUE #85 — الترميز الدفعي**: شبكةٌ واحدة كثيفة (`KitGrid`) لا نمط عمودين، فوق `POST vouchers/batch-save/` و`coding-rules/` اللتين بنتهما #84؛ الحساب يُقترح من الطرف فور كتابته (قابلٌ للتجاوز بالكتابة فوقه بلا سؤال)، و«ناقِش» على الصفّ يفتح `ReviewQuery` القائم (`accountantApi.createReviewQuery`) لا محادثة جديدة |
