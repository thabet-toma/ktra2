...
- **`utils/tabLink.ts`** — مناولةٌ وحضور. عند الفتح يُكتب سجلّ محلي بمُعرّف
  ...
  التبويب الجديد يلتقطه في `captureTabHandoffOnBoot()` **قبل إقلاع React**
  (`index.tsx`، بجانب `applySkinOnBoot`)، يستهلك السجلّ مرّةً واحدة، وينظّف
  الرابط بـ`replaceState` — فلا يرى الموجّه المعاملَ العابر ولا يتسرّب إلى رابطٍ
  يُنسخ. `sessionStorage` و`window.name` كلاهما مرفوض هنا: يسقط مع
  `rel="noopener"` الذي نفتح به. والحضور عبر `BroadcastChannel` باسم `ktra-tabs`،
