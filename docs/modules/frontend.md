...
- **«مدفوعة» تعبّئ ولا تُرسل (T-PAYFULL)**: التسديد التام هو الحالة الغالبة،
  وكتابةُ المبلغ يدوياً كانت ضريبةً عليها. زرّ **«مدفوعة»** في شريط المحرّرين
  (`fillCollectFull` بيعاً · `fillPayFull` شراءً) وزرّ **«المتبقي كاملاً»**
  في اللوحة (`onFillFull`) يضعان كامل المتبقّي في خانة النقد وينزلان باللوحة
  — والإرسال يبقى بزرّ «تسجيل دفعة» وحده. الفصل مقصود بقرار المالك: لا سندَ
  يُرحَّل بنقرةٍ واحدة قبل مراجعة الصندوق والمبلغ. وعلى **المسودة** الأساسُ
  `remainingAfterIntent` لا `remaining` — النيّة المحفوظة تتجسّد سنداً عند
  الترحيل، فالتعبئة بالخام تدفع مرّتين.
