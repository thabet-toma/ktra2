...
و`avg_cost`. والكلفةُ منذ #137 **بطبقات FIFO** لا بمتوسّطٍ مرجَّح: `StockLayer` لكلّ ورود،
و`StockLayerConsumption` تسجّل أيَّ طبقةٍ أكل منها أيُّ صرفٍ وكم — وهي ما يجعل إلغاء الترحيل
يردّ البضاعة إلى طبقتها **وموقعِها في الرتل** بالضبط لا بالتقريب. و`StockLayerReconciliation` نظيرُها للسدّ: أيُّ وارِدٍ سدّ أيَّ طبقةٍ مؤقّتة وكم — فإلغاءُ ترحيل الوارد يفتح الطبقةَ التي سدّها بعينها. كل الأبواب — البيع، الشراء، الاستلام، التحويل بين المستودعات، الجرد —
...
def restore_returned_sales_serials(return_invoice, lines) -> int:  # مرجع البيع يُعيد وحدات الفاتورة الأصلية FIFO ويسجّل return_line (613)
...
- **الكلفة بطبقات FIFO لا بمتوسّطٍ مرجَّح (#137)**: كلُّ واردٍ يُنشئ `StockLayer` بكلفته،
  وكلُّ صادرٍ يستهلك الطبقات المفتوحة **بترتيب الورود** (`inventory/fifo.py` — `consume`)
  فيحمل `total_cost` مجموعَ ما استهلكه فعلاً. و**`Product.avg_cost` صار رقماً مشتقّاً
  للعرض** (`derived_avg_cost`: قيمة الطبقات المفتوحة ÷ كميّتها) — **لا مصدرَ كلفةٍ يُبنى
  عليه قيدٌ أو تقرير**. من قرأه ليحسب ت.ب.م أعاد عطبَ «الدفتر يقول رقماً والتقرير يقول
  غيره»: تحت المتوسّط كان الرقمان يتطابقان حكماً، وتحت FIFO يفترقان كلّما عبرت البيعةُ
  طبقتين بسعرين.
- **المرتجعُ يُفكّ بعكس ترتيب الاستهلاك** (`fifo.restore_partial`): بيعُ ٦٠ استهلك ٥٠ من
  طبقةِ العشرة وعشراً من طبقةِ الستّين؛ ومرتجعُ ١٥ يفكّ العشرَ الغالية أوّلاً ثمّ خمساً
  رخيصة — فتصير حالةُ الرتل **مطابقةً حرفاً بحرف** لما لو بيعت ٤٥ ابتداءً. الردُّ
  بترتيب الاستهلاك نفسِه يترك وحداتٍ من الطبقة الأحدث مستهلَكةً بلا سبب ويقلب كلفةَ
  ما تبقّى. وصفُّ استهلاكٍ يُفكّ جزئياً **تُنقَص كميّتُه ولا يُحذف**، فمرتجعٌ ثانٍ يجد
  ما يفكّه. و`restores_movement` في `record_stock_movement` يقبل حركةً أو قائمةَ حركات
  (مستندٌ واحدٌ قد يُنتج أكثر من حركةٍ للمنتج نفسه).
- **الثابت: Σ`remaining_qty` − المعلَّقُ المؤقّت = `quantity_on_hand`.** الطبقة لا تحمل
  كميّةً سالبة، فالمخزونُ السالب يُمثَّل بطبقةٍ `is_provisional=True` معلَّقة
  (`original_qty − reconciled_qty`) لا بكميّةٍ سالبةٍ في الرتل.
- **بضاعةٌ قائمةٌ بلا طبقات تُرأب كسولاً وبحيادٍ تامّ** (`backfill_opening_layer`): كلُّ
  صنفٍ في قاعدةٍ سابقةٍ لـ#137 يحمل رصيداً وكلفةً وصفرَ طبقات. فقبل أيّ استهلاك تُنشأ
  طبقةٌ افتتاحيّةٌ بالفرق بكلفة `avg_cost` الجارية وبتاريخ `OPENING_LAYER_DATE` (أقدمِ
  الرتل). **بدونها تُصفَّر كلفةُ الصنف من أوّل بيعة** — تُصنَّف بضاعتُه «غيرَ مغطّاة»
  فتصير طبقةً مؤقّتة تُستهلَك فوراً، فيقسم `derived_avg_cost` صفراً على صفر. وهي محايدةٌ
  على الميزانية بالبناء (كميّة × متوسّط = القيمة الدفتريّة نفسُها)، فلا يلزم تشغيلُ
  `rebuild_fifo_layers` قبل النشر.
- **وإلغاءُ الترحيل يرأب أيضاً لا الترحيلُ وحده** (`_recompute_product_stock`): بعد حذف
  حركات مستند، رصيدٌ لا تغطّيه الطبقات كان يُشتقّ من رتلٍ فارغٍ **صفراً** فيُعاد ترحيلُ
  مبيعاته بكلفة صفر (شركة 6، 16/09/2026). فالرأبُ يمرّ بـ`backfill_opening_layer` نفسِها
  على مرحلتين: اليتيمُ القائمُ بـ`avg_cost` الملتقطة قبل الكتابة فوقه، ثمّ وحداتُ صرفٍ
  أُلغي بلا صفوف استهلاك (سبق FIFO) **بكلفة حركتها** (`_restore_outbound_layers`) —
  عينُ ما يعود لحساب المخزون بحذف قيدها، لا المتوسّطُ الجاري. ويقيس الفجوةَ بعد
  `fifo.restore_partial` فلا يُضاعف ما أُعيد. وإلغاءُ إرساليّة بيع (`void_delivery_note`)
  يردّ إلى الطبقات قبل الحذف كذلك — كان يحذف بلا ردٍّ فيعود الرصيدُ بلا كلفة.
- **والعكس: حذفُ وارِدٍ سبق FIFO يقصّ الافتتاحيّة** (`fifo.trim_opening_layers`): بضاعتُه
  بلا طبقة ذابت في الطبقة الافتتاحيّة، فحذفُه كان ينقص الرصيدَ ويترك الطبقاتِ أكبرَ منه.
  يُقصّ الافتتاحيُّ وحدَه وبحدِّ كميّة ذلك الوارد (`_unlayered_inbound_quantities` — و`RETURN_IN`
  مستثنًى لأن المرتجعَ العائدَ لطبقته الأصليّة بلا طبقةٍ خاصّة أيضاً)، ويُنقص `original_qty`
  معه فيبقى الفرقُ = المستهلَك. **وتنقص قيمتُها بكلفة الوارد المحذوف لا بمتوسّطها** — عينُ ما
  يخرج من حساب المخزون بحذف قيده — فتُعاد كلفةُ الباقي منها؛ إلّا أن تحمل صفوفَ استهلاك أو لا
  يبقى منها شيء أو تكون الكلفةُ صفراً/الناتجُ سالباً، فتبقى كلفتُها. وفائضٌ يبقى بعد القصّ
  (سببُه غيرُ وارِدٍ محذوف) يُسجَّل تحذيراً (`FIFO LAYERS EXCEED STOCK`) ولا يُقصّ.
- **الثابتُ يُفحص بأمر** (`manage.py check_fifo_layers [--tenant N] [--apply]`، على
  `fifo.layer_balance_gaps`): يطبع كلَّ صنفٍ طبقاتُه **أكثرُ** من max(الرصيد، 0) (فائض — لا يُصلَح
  آليّاً أبداً: أيُّ طبقةٍ هي الوهميّة يُعرف من الحركات لا من سكربت) أو **أقلُّ** منه (غيرُ مغطّى).
  معاينةٌ افتراضياً؛ و`--apply` (يلزمه `--tenant`) يرأب غيرَ المغطّى وحده بـ`backfill_opening_layer`
  — بلا قيدٍ ولا تغييرٍ في الرصيد أو `avg_cost`. يُشغَّل بعد أيّ تصحيحٍ كبير. وهو غيرُ
  `rebuild_fifo_layers` الذي يُعيد تقييم الطبقات من دفتر الحركات فيُحرّك القيمة.
- **حارسُ «طبقةٌ استُهلكت من خارج المستند» واحد** (`_assert_layers_not_consumed_elsewhere`):
  يمرّ به `reverse_stock_movements` و`logistics.services.void_goods_receipt` معاً.
- **وإلغاءُ وارِدٍ سدّ طبقاتٍ مؤقّتة يردّ السدَّ ويحذف قيدَ فرقه** (`_reverse_provisional_reconciliations`
  ← `fifo.unreconcile`، قبل حذف الحركة): `reconciled_qty` ينقص على الطبقة التي سدّها بعينها
  (من سجلّ `StockLayerReconciliation`)، وقيدُ `STOCK_PROVISIONAL_RECONCILE` بمرجع الحركة يُحذف —
  كان يبقى يتيماً لأن `unpost_document` يحذف قيودَ المستند بأنواعه وحدها، وإعادةُ الترحيل تُطلع
  قيداً ثانياً للفرق نفسه. وسدٌّ جرى قبل السجلّ تُشتقّ كميّتُه من طبقة الوارد (الأصل − المتبقّي −
  المستهلَك) وتُفتح من المسدودات السابقة لطبقته، الأحدثُ أوّلاً — تقريبٌ للبيانات القديمة وحدها.
- **الطبقةُ على مستوى الشركة والمنتج لا المستودع**: `quantity_on_hand` رقمٌ واحدٌ على
  مستوى الشركة، والمستودعُ يُسجَّل على الطبقة **للأثر لا للتقسيم**. طبقاتٌ لكلّ مستودع
  تُنشئ فكرةً ثانيةً للمخزون تناقضه.
- **المخزونُ السالب يُصحَّح بتاريخ وصول البضاعة دائماً** (#136): البيعُ على السالب يأخذ
  آخرَ كلفةٍ معروفة في طبقةٍ مؤقّتة، وحين تصل البضاعة الحقيقيّة تُسدّ الحفرةُ ويُطلَع
  **قيدُ فرقٍ بتاريخ الورود بلا استثناء** — لا فحصَ لفترة البيعة ولا طلبَ فتحِ فترةٍ
  مقفولة، وفرقٌ صفريٌّ ⟵ لا قيد. و`WAREHOUSE_TRANSFER` مستثنىً من التسوية: صافي أثره
  صفرٌ وبلا قيد، فتسويةُ استلامِه لحفرةٍ خلقها صرفُه نفسُه تُصفّر طبقتَه الجديدة.
- **`sales_cogs_map` (`sales/services/pricing.py`) لا تُلمس**: تجميعتُها على
  `StockMovement.total_cost` صحيحةٌ تحت FIFO بلا تعديل — سيمُ تقارير الربح كلّها.
- **المخزون السالب سياسةُ شركةٍ واحدة**: يُرفض الصرف إن `qty_before < quantity` إلا إذا سمح `SalesSettings.allow_negative_stock_default` (`inventory/services.py` (`record_stock_movement`)) — أي أن قرار مخزون يعيش في `sales` لا هنا. **ولا علامةَ تجاوزٍ على المنتج**: كانت `Product.allow_negative_stock`/`ProductFamily.allow_negative_stock` تغلب الإعداد بلا زرٍّ في أيّ شاشة يكشفها — شركةٌ تقول «ممنوع» وأصنافٌ تبيع بالسالب بلا سببٍ ظاهر (قيس على الإنتاج: 1,490 صنفاً من 1,556 وضعها سكربتُ استيراد على الكلّ). حُذف الحقلان (هجرة `0037`).
...
- **مرجع البيع لا يمحو أثر البيع، وإلغاءُ ترحيله يعكسه** (`serials.py` — `restore_returned_sales_serials`/`revert_returned_sales_serials`): المرجع يعيد الوحدة `in_stock` ويُبقي `sales_line` على بند البيع الأصلي ويسجّل بندَه في `return_line`. إلغاءُ ترحيل المرجع (`sales/views.py` — `unpost_invoice`) يعيد تلك الوحدات «مُباعة» على بيعها الأصلي؛ ووحدةٌ تحرّكت بعده — بِيعت ثانيةً أو انفصلت عن بند بيعها الأصلي — تُسمّى ويُرفض الإلغاء بـ400 (مرآة حارس `release_purchase_serials`). **الحالة هي ما يُسمّي الزبون لا الرابط**: `_serial_row` و`after_sales/services.py` (`warranty_coverage`) لا يعرضان بيعاً ولا زبوناً لوحدةٍ `in_stock`. **حدودٌ معلومة:** مرجعٌ رُحِّل قبل وجود `return_line` بلا أثر، فإلغاءُ ترحيله يترك وحداته في المخزن ولا يُعيدها تخميناً؛ ومرجعٌ ثانٍ على بيعٍ لاحقٍ للوحدة نفسها يكتب `return_line` فوق الأوّل، فإلغاءُ ترحيل الأوّل بعدها لا يرى تلك الوحدة.
...
| `inventory/tests/test_fifo_engine.py` | محرّك الطبقات: مثال المالك (٥٠ بـ١٠ ثمّ ٢٠ بـ٦٠)، عبورُ طبقتين، الردُّ الدقيق، حارسُ التآكل بعشر دورات، نفادُ الرتل بلا استثناء، عزلُ الشركات، واستعلامٌ واحد للقيمة |
| `inventory/tests/test_fifo_movements.py` | الالتحام بـ`record_stock_movement`: الكلفةُ من الطبقات لا من المتوسّط، الردُّ عبر `reverse_stock_movements`، حارسُ حذفِ طبقةٍ استُهلكت، الطبقةُ المؤقّتة، و`restores_movement` |
| `inventory/tests/test_legacy_stock_backfill.py` | **بضاعةٌ قائمةٌ بلا طبقات**: أوّلُ بيعةٍ لا تُصفّر الكلفة، الطبقةُ الافتتاحيّة محايدةٌ وأقدمُ الرتل وغيرُ مؤقّتة (فلا قيدَ فرقٍ وهميّ)، والثابتُ يصمد عند السالب الحقيقيّ |
| `inventory/tests/test_unpost_provisional_reconciliation.py` | إلغاءُ وارِدٍ سدّ مؤقّتة: الطبقةُ تعود معلَّقةً وقيدُ الفرق يُحذف، إعادةُ الترحيل تُقيّد الفرقَ مرّةً لا مرّتين، مع واردَين بكلفتين يُفتح ما سدّه الملغى بعينه، وسدٌّ سبق السجلّ يُردّ اشتقاقاً |
| `inventory/tests/test_reversal_opening_layer_trim.py` | حذفُ وارِدٍ سبق FIFO: الافتتاحيّةُ تُقصّ حتى تطابق الطبقاتُ الرصيد، لا يُمسّ غيرُ الافتتاحيّ ولا ما استُهلك منها، والوارِدُ ذو الطبقة لا يُقصّ له شيء؛ **سلسلةُ الإنتاج كاملةً** (بيع ← إلغاؤه ← إلغاءُ الشراء ← إعادتُهما) تُبقي الطبقاتِ = الرصيد؛ القصُّ بكلفة الوارد لا بالمتوسّط (وتبقى الكلفةُ إن كانت الطبقةُ مستهلَكة)؛ وفائضٌ باقٍ يُحذَّر منه |
| `inventory/tests/test_check_fifo_layers.py` | `check_fifo_layers`: المعاينةُ تُبلغ الفائضَ وغيرَ المغطّى ولا تكتب؛ `--apply` يرأب غيرَ المغطّى بكلفته الدفتريّة بلا قيد ولا يمسّ الفائض؛ محصورٌ بالشركة؛ و`--apply` بلا `--tenant` يُرفض |
| `sales/tests/test_unpost_orphan_stock_cost.py` | **إلغاءُ الترحيل على رصيدٍ بلا طبقات**: حالةُ الإنتاج (7 بـ190) لا تُصفَّر وإعادةُ الترحيل تُقيّد 760، تغطيةٌ جزئيّة لا تنحرف قيمتُها، لا طبقةَ زائدة لبيعةٍ من عصر FIFO، وإلغاءُ إرساليّة البيع يعيد البضاعةَ بكلفتها |
| `inventory/tests/test_provisional_reconciliation.py` | تسويةُ المؤقّت عند الوصول: الثابت، اتجاها قيد الفرق، صفرٌ ⟵ لا قيد، النجاحُ رغم فترةٍ مقفولة، تسويةٌ جزئيّة، وعزلُ الشركات |
| `inventory/tests/test_rebuild_fifo_layers.py` | أمرُ «قِس أوّلاً»: الحالةُ المطابقة، **الفرقُ البنيويّ بلا كميّةٍ يتيمة**، الكميّةُ اليتيمة، وحارسُ «لا يكتب صفّاً» |
