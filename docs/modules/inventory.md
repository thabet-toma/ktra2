...
`search`/`stock_status` تختار **أيّ البراندات** فلا إكمال في الخادم ولا تجميع
في الواجهة (`GroupedItemsTable.tsx` — `brandFilterActive`) — صفوف براندٍ صريحة،
وإلا لأظهر فلتر «نفد» إخوةً متوفّرين في صفٍّ يدّعي أنه المنتج.
...
وما **لم** يُحدَّث بعد فعلاً: فلتر `?stock_status=` في الجدول (`filter_by_stock_status`)
والداشبورد — لا يزالان يقرآن رصيد البراند وحده، فيمكن أن يعود الفلتر بصفٍّ شارتُه
تقول غير ذلك على منتجٍ متعدّد البراندات.
