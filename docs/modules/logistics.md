...
def party_unallocated_notes(tenant_id, partner_id) -> list[tuple]        # [(إشعار مدين، غير الموزَّع، بالأساس)] — يدخل «تحت الحساب» في party_on_account_summary
...
def release_adjusted_overpayment(kind, obj, *, user=None) -> dict | None  # بعد تعديل الاستحقاق: توزيعاتٌ تُقلَّص ثم دفعةٌ تُفصل ← «تحت الحساب»
