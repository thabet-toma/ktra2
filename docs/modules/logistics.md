...
| `LogisticsAccrualAllocation` (`logistics/models.py`) | `amount` (بعملة المصدر)، `amount_base` (بالأساس) | مصدرٌ واحدٌ (CheckConstraint `accrual_allocation_one_source`): `payment`→`sales.SupplierPayment` (`logistics_allocations`) **أو** `note`→`sales.CreditDebitNote` (`accrual_allocations` — إشعارٌ مدينٌ على الطرف)؛ وهدفٌ واحدٌ (CheckConstraint) من `clearance` / `shipment` (استحقاق الشحن) / `local_shipment` |
...
def purchase_invoice_note_totals(invoice) -> tuple[Decimal, Decimal]:    # (مدينة موزَّعة `CreditDebitNoteAllocation`، دائنة مربوطة) مرحّلة — المدين مع المدفوع، والدائن على المستحق
...
def accrual_status(kind, obj, *, exclude_payment_journal_id=None) -> dict:  # {due, paid, allocated, remaining, overpaid, surplus} بالأساس — allocated: السندات والإشعارات المدينة الموزَّعة، والدائنة المربوطة تزيد due
...
def journal_reference_accrual_links(tenant_id, refs) -> dict             # مرساة كل مستحق (LOGISTICS_CLEARANCE:<id> · LOCAL_SHIPMENT:<id> · SHIPMENT_FREIGHT_ACCRUAL:<shipment>) لقيده ودفعاته، والسندات الموزَّعة بمبالغها، ودفعة الصفقة ← فاتورتها — «ربط الفاتورة بسندها»؛ ولكل مرساة `open` (المستحق وشحنته) تفتحه الواجهة، و`details` تفاصيل الحركة: رقم مطالبة المخلّص/البيان أو رقم الإرسالية والشحنة (`_doc_facts`)، وللدفعة المباشرة رقمها وغرضها وتاريخ دفعها (`_payment_facts`)، وللسند والإشعار ما وُزِّع عليه
...
def note_journal_ids(kind, obj) -> list[int]                             # قيود الإشعارات الدائنة المرحّلة المربوطة بالمستند — ترفع due
def party_unallocated_notes(tenant_id, partner_id) -> list[tuple]        # [(إشعار مدين، غير الموزَّع، بالأساس)] — يدخل «فائض تحت الحساب» في party_on_account_summary
def allocate_note_to_accrual(note, kind, pk, amount, *, base) -> str      # داخل sales.services.note_allocation.allocate_note
def party_on_account_summary(tenant_id, partner_id) -> dict              # {on_account, surplus, on_account_items, surplus_items} — خانتا رأس كشف الدائن ومستنداتهما
...
def split_posted_overpayment(kind, obj, *, apply=False, user=None, include_surplus=False)  # للموجود: عكسٌ + إعادة ترحيل + سند (الأمر)
def release_adjusted_overpayment(kind, obj, *, user=None) -> dict | None  # بعد تعديل الاستحقاق: توزيعاتٌ تُقلَّص ثم دفعةٌ تُفصل ← «فائض تحت الحساب» (`SupplierPayment.adjust_surplus`)
...
- **التخفيض تحت المدفوع يعود «فائضاً تحت الحساب»** — `accrual_status`: `due_original` (القيد الأصلي وحده) و`surplus = min(overpaid, max(due_original − due, 0))` — ما دُفع زائداً **لأن الاستحقاق نزل**. «تعديل الاستحقاق» المرحَّل يُتبعه في المعاملة نفسها `overpayment_split.release_adjusted_overpayment`: الزائد كلّه (الفائض معه) يُحرَّر — توزيعات السندات والإشعارات على المستند تُقلَّص الأحدث أولاً (بلا قيد)، ثم ما بقي من دفعات المستند المباشرة بعكسٍ وإعادة ترحيلٍ وسند صرفٍ «تحت الحساب» — فيظهر في فائض الطرف ويُوزَّع أو يُستردّ. وما كان منه فائضاً (لأن المستحق نزل) يُحفظ على السند العائد إليه في `SupplierPayment.adjust_surplus` (بعملته) فيبقى «فائضاً» لا «دفعة». ما تعذّر (عملة أجنبية، قيدٌ مركّب، دفعات الشحن الدولي) يبقى «فائضاً» على المستند ويُذكر سببه في `on_account` من الردّ. وأمر `split_logistics_overpayments` يفصل `overpaid − surplus` وحده كما كان. `party_on_account_summary` يجمع لكل دائن بتعريف المالك: «دفعات تحت الحساب» = ما دُفع والمستخدم يعلم لحظتها أنه ليس لمستحقٍّ بعينه أو يزيد عليه (غير الموزَّع من سندات الصرف — يدويّةً وسندات فصل الزائد — عدا ما حمله `adjust_surplus`، + زائدٌ لحظة الدفع بقي على مستنده)؛ «فائض تحت الحساب» = ما صار لنا لاحقاً لا بدفع (غير الموزَّع من الإشعارات المدينة + `adjust_surplus` من غير الموزَّع + فائضٌ بقي على مستنده). المتاح من كل مصدر = مبلغه − الموزَّع − المسترَدّ؛ ومعهما قائمة مستنداتهما (`on_account_items`/`surplus_items`: سند/إشعار/مستحقّ) — يعرضهما كرت الطرف (`partners/views.py` — `on_account_payments` · `accrual_surplus`) مربّعين فوق كشف الحساب، والرصيد الكلّي لا يتغيّر.
...
- **الإشعار المدين على الدائن رصيدٌ يُوزَّع كالسند، والدائن المربوط يرفع المستحق** (`sales.CreditDebitNote`، القاعدة في `docs/modules/sales.md`): المدين (خصمٌ منه) يُطفئ التخليص/الإرسالية/استحقاق الشحن بتوزيعه (`LogisticsAccrualAllocation.note`) فيدخل `allocated` في `accrual_status` مع سندات الصرف — الترحيل يوزّعه على مستنده المربوط بحدّ متبقّيه، وما لم يُوزَّع «تحت الحساب» (`party_unallocated_notes`). والدائن المربوط (مبلغٌ إضافيّ له) يزيد `due` بقيوده (`note_journal_ids`). الكرت والتوزيع وFIFO والأعمار وفصل الزائد وتبويب «الدفعات» (`document_voucher_rows` — صفّ الإشعار بـ`note_number`) تقرؤه من الموضع الواحد، وكشف الحساب يُرسيه على ما وُزِّع عليه (`journal_reference_accrual_links` — `notes`). وعلى فاتورة الشراء: `purchase_invoice_note_totals` في الملخّص وتوأمه SQL (`note_debit`/`note_credit` في `annotate_purchase_invoice_payment_summary`) — المدين الموزَّع مع المدفوع والدائن المربوط على المستحق، والقاعدتان لا تفترقان.
...
| `tests/test_accrual_adjust.py` | قيد الفرق للتخليص وتعديلٌ ثانٍ فوقه، رفض «لا فرق» والمعاينة لا تكتب، الإرسالية وإلغاء ترحيلها يحذف تعديلاتها، الشحن بسعر صرفٍ معاكس وقفل `freight/`، الفاتورة المرحّلة: مخزون/تكلفة مبيعات والطبقة وحركة البيع بلا انجراف، التحويل بين المستودعات، غير المستلَم على الوسيط ثم يُستلم بالجديدة، الفاتورة المتأخّرة تمنع، إعادة المسودة، والتخفيض تحت المدفوع يعود «تحت الحساب» (دفعةٌ تُفصل سنداً، وتوزيع سندٍ يُقلَّص) |
