...
## دين تقني معروف (موثَّق عمداً)
`logistics/views.py:1223-1265` داخل `unpost_payment_from_accounting`: يُنشئ القيدَ العكسي **يدوياً** عبر `JournalHeader.objects.create(...)` (:1223)
ثم `JournalLine.objects.create(...)` لكل سطر (:1235) بقلب المدين/الدائن، ثم يُنزل ترحيل القيد الأصلي بـ`JournalHeader.objects.filter(pk=…).update(is_posted=False)` (:1264)
لتجاوز حارس `save()`. هذا يتجاوز `accounting.services.post_journal`/`unpost_document` بالكامل — فلا يمرّ بتحقّقاتها ولا بترقيمها؛ التوازن يُتحقَّق منه محلياً هنا
(:1213-1219 قبل، و:1247-1257 بعد)، أي أن **أي تحقّق جديد يُضاف في `accounting.services` لن يسري على هذا المسار**. نفس النمط مكرَّر في `views.py:2633-2660`
(إلغاء ترحيل دفعة تخليص). بقية المسارات تستخدم `post_journal` (views.py:1013, 1871, 2024, 2556, 3744, 3873, 4430).
