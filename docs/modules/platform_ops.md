# platform_ops — مركز قيادة شؤون الموظفين والمهام وعمليات المنصة
...
| `frontend_v2/components/platform-hiring/PlatformPublicJobPage.tsx` (#215 · #217) | صفحةُ الرابط العامّ ببابين: التقديمُ والمتابعة. **البابان بطاقتان مؤطَّرتان متساويتان** لا شريطٌ مختارُه ممتلئٌ وآخرُه نصٌّ عارٍ — ذاك كان يُقرأ عنواناً لا زرّاً فلا يعرف من قدّم سابقاً أنّ له مدخلاً حتى يضغط الآخر. **وشرحُ الوظيفة في بابِ التقديم وحدَه**: من ضغط «قدّمتُ سابقاً» قرأه قبل أن يقدّم، وإعادتُه تدفن حقلَي الرمز والهاتف في آخر الصفحة؛ واسمُ الوظيفة يبقى سطراً في المتابعة فيعرف على أيّ طلبٍ يدخل |
| `frontend_v2/components/platform-hiring/PublicApplicantTrackingPanel.tsx` · `ApplicantUpdateThread.tsx` (#215) | لوحةُ متابعة المتقدّم (الحالة · اجتماعاتُه · «عن الوظيفة» · المحادثة · صندوقُ الردّ)، والخيطُ الزمنيُّ المشتركُ بينها وبين لوحة مسؤول التوظيف — يميّز الأطرافَ الثلاثة، ويصيّر `link` زرّاً و`phone` اتّصالاً. و`location` الحرُّ يصير زرَّ انضمامٍ **إن بدأ بـ`http`** وإلّا فنصَّ مكان |
...
| `platform_ops/public_hiring/` | **كل كود `AllowAny` للتوظيف في حزمة واحدة** ليراجعها الأمن دفعة واحدة: `views.py` (ثماني نقاط عامة بخانق `ClientIpScopedThrottle` ومصادقة معلنة صراحة — والثلاثةُ الأخيرةُ متابعةُ المتقدّم #215)، و`cv_validation.py` (سقف 5 م.ب وفحص النوع والبنية **بالبايتات**)، و`serializers.py` (بلا `cv_url` بأي حال)، و`urls.py` تحت `/api/careers/`، و`urls_page.py` (#214-أ) لصفحة الإعلان المُصيَّرة من الخادم — مركَّبةٌ **مرّتين** في `core/urls.py` |
...
| `frontend_v2/components/platform/PilotAxesTable.tsx` (210-E) | جدولُ محاور الـpilot — **مصدرٌ واحد** لبطاقة الموظف الذاتيّة ولوحة المدير معاً (كان نسختين متطابقتين فأيُّ تعديلِ عمودٍ يُنتج جدولين متناقضين). يعرض `weight_original_pct` بصيغة «الأصلي ← الفعلي»، و`raw_percent` فائضاً فوق الطاقة تحت الدرجة المقصوصة، و`uncatalogued_document_links` تحذيراً، و`exclusions` بأسمائها العربيّة من `utils/pilotExclusions.ts` (`pilotExclusionLabel`: الرمزُ المجهولُ يُعرَض كما هو) — والخريطةُ نفسُها مصدرُ أسماء تصنيف الردّ في `WorkOrdersPanel`، ويحرسها `platform_ops/tests/test_pilot_exclusion_labels.py` على choices الخادم |
...
| `frontend_v2/components/platform/EmployeeProfileDrawer.tsx` (211-J) | ملفُّ الموظّف الـ360 على هيئة اللوحة 4 من صورة المالك: رأسُه وجهُ الموظّف وحالتُه **وحلقةُ تقييمه العامّ**، ثمّ تبويباتُه الستّة (عام · الأداء · المهامّ · المحفظة · الملاحظات · النشاط) بـ`CcTabs`، والملاحظاتُ فقاعاتٌ تُميَّز بالرؤية. **كلُّ تبويبٍ يحمّل بياناته عند فتحه لا قبله — باستثناءٍ واحدٍ مقصود**: درجةُ الأداء تُطلب عند فتح اللوح لأنّ حلقتَها في الرأس، وبلا ذلك كانت تقول «—» لموظّفٍ درجتُه معلومة؛ وخطؤها يُبتلع في الرأس ويُعرض كاملاً في تبويب «الأداء». ولا رقمَ يُشتقّ فيه: ما لا تُرجعه نقطةٌ لا يُعرَض. ويُفتح من نقرةٍ على وجه الموظّف في البطاقة أو على مقعده في الغرفة |
...
| `JobApplicantUpdate` (#215) | سطرٌ واحدٌ يقرؤه المتقدّم في صفحة متابعة طلبه، **بلا `tenant`** كإخوته. جدولٌ واحدٌ لا جدولان (مراحلُ ورسائل) لأنّ المتقدّم لا يراهما شيئين: `kind` يفرّق `status` (يكتبه النظامُ مع الانتقال) عن `notice` (يكتبه الفريقُ أو المتقدّم)، و`author_kind` يفرّق الأطرافَ الثلاثة. و`link` و`phone` **حقلان مستقلّان عن `body`** عمداً: رابطٌ داخل جملةٍ لا يصير زرّاً ورقمٌ داخلها لا يصير اتّصالاً بضغطة. و`is_public` يحجب ما لا يُراد نشرُه، و`read_at` يحمل متى قرأ الفريقُ ردَّ المتقدّم. **ولا يُقرأ منه `JobApplicant.notes` أبداً** — تلك «ملاحظات مسؤول التوظيف»: ما يُكتب **عن** الشخص لا **له**. |
...
| POST | `/api/careers/jobs/{token}/track/` · `track/refresh/` · `track/reply/` (#215) | `AllowAny` + `ClientIpScopedThrottle` بنطاقين: `platform_ops_track` للفتح (سطحُ التخمين) و`platform_ops_track_session` لما بعده. **الدخولُ بعاملين**: رقمُ التتبّع **ورقمُ الهاتف** — رابطُ الإعلان يُنشَر على فيسبوك عمداً (#214-أ) فرمزُ الوظيفة ليس سرّاً، و`reference_code` ثمانيةُ محارفَ وحدَها لا تكفي حارساً على بياناتٍ شخصيّة. **و`POST` لا `GET`**: الهاتفُ يُكتب في سجلّات الخادم ويُرسَل في `Referer` لو مرّ في سلسلة الاستعلام. والفشلُ **نصٌّ واحدٌ لكلّ الأسباب** (`services.TRACKING_DENIED_DETAIL`) وإلّا صارت الصفحةُ تؤكّد وجودَ الرموز لمن يجرّبها. والجلسةُ **موقَّعةٌ بلا صفٍّ في القاعدة** (نصفُ ساعة) لا مهشَّرةٌ كـ`docshare`: ليست رابطاً يُرسَل ويُبطَل. |
| POST | `/api/platform/ops/job-applicants/broadcast/` (#216) | `IsPlatformRecruiter` — رسالةٌ واحدةٌ إلى كلّ متقدّمي **إعلانٍ واحد** (`job` إلزاميّ: «أرسل للكلّ» بلا حدٍّ تعني متقدّمي كلّ إعلانٍ في المنصّة)، مع اختيارِ حالاتٍ و`include_closed`. **صفٌّ لكلّ شخصٍ لا صفٌّ مشترك** فيبقى دفترُ كلٍّ منهم دفترَه ويردّ عليها وحدَه، و`bulk_create` واحدةٌ لا كتابةٌ لكلّ صفّ، وسقفُ ٥٠٠ مستقبِلٍ في الإرسالة. **والمغلقةُ مستثناةٌ افتراضاً** — «تأجّلت المقابلات» تصل مرفوضاً جرحاً بلا سبب. **ولا تُعلَّم ردودٌ مقروءةً هنا** خلافاً لـ`notice/`: تلك جوابٌ على سؤالٍ بعينه، وهذه إعلانٌ عامٌّ وتعليمُها يبتلع ردودَ مئةِ شخصٍ لم يقرأها أحد. |
| GET · POST | `/api/platform/ops/job-applicants/{id}/updates/` · `{id}/notice/` (#215) | `IsPlatformRecruiter` — دفترُ المتقدّم كاملاً (المنشورُ والمحجوب) وكتابةُ رسالةٍ إليه. **وفتحُ `updates/` يُعلّم ردودَه مقروءةً**: أثرٌ جانبيٌّ على `GET` مقصودٌ، لأنّ زرَّ «علّم كمقروء» يُنسى فتبقى الشارةُ تصرخ على رسالةٍ قُرئت. و`unread_reply_count` على صفّ المتقدّم **مُحسَّبٌ بـ`annotate`** في `get_queryset` لا بنداءٍ لكلّ صفّ. |
...
**وطبقةُ مكوّناتٍ أوّليّةٍ داكنةٍ بالمولد لا بالتجاوز** (`components/platform/ui/`):
أحدَ عشرَ مكوّنَ عرضٍ خالصٍ (`CcCard` · `CcStatTile` · `CcGauge` · `CcPill` ·
`CcAvatar` · `CcProgress` · `CcTabs` · `CcSectionTitle` · `CcTable` · `CcEmpty` ·
`CcSkeleton`) ونغماتُها في `frontend_v2/utils/ccTone.ts` (`ccScoreTone` ·
`ccPresenceTone` · `ccPillClasses` · `ccSurfaceClasses`) يختبرها `npm test`.
تلبس رموزَ `--color-cc-*` المعرَّفةَ في `@theme`، **وكلٌّ منها `var(--staff-…)`
أو `var(--pf-…)` بقيمةٍ حرفيّةٍ احتياطيّة** — فليست لوحةً ثانيةً تتباعد عن لوحة
212-T، وتغييرُ اللون يبقى في `--staff-*` وحدَها. ولذلك **لا يمسحها حارسُ الجلد**
(مَسْحُه غيرُ متراجعٍ فلا يبلغ `ui/`) ولا يحتاج إليه: لا صنفَ فاتحاً فيها يُصلَح.
وأضواءُ الحضور الثلاثةُ في `CcAvatar` هي `--pf-online/-meeting/-offline` نفسُها،
كي لا ينقسم معنى اللون بين البطاقة والطاولة.

**والأحرفُ الأولى دالّةٌ واحدةٌ مختبَرة** (`frontend_v2/utils/ccInitials.ts`):
كان المنطقُ مكرّراً خمسَ مرّاتٍ في الموديول (`CcAvatar` · `EmployeeProfileDrawer`
· `MyProfileCard` · `WorkspaceRoom` · `staff/StaffSidebar`) وكلُّها تأخذ أوّلَ
حرفٍ من أوّل كلمتين، فتقرأ أداةَ التعريف حرفاً من الاسم: «عمر الشريف» ← **عا**.
الدالّةُ الآن تنزع «ال»، وتدمج «عبد»/«أبو»/«بن» بما بعدها فيبقى حرفُها، ولا
...
عشرَ زرّاً مكتوباً بيده — ولذلك تغيّر حارسان نصّيّان: `test_tasks_ui_contract`
كان يؤكّد وجودَ `setActiveTab("staff_tasks")` حرفيّاً فصار يقيس الوصلَ (المفتاحُ
في القائمة، والقائمةُ موصولةٌ بمبدِّل الحالة)، و`test_employee_profile_frontend_contract`
كان يؤكّد `employee.photo_url ? … <img>` في البطاقة فصار يقيس الطرفين (البطاقةُ
تمرّر الرابطَ إلى `CcAvatar`، والمكوّنُ يُصيّر `<img>`). **والسببُ ليس تجميلاً:**
مرّ كلا التأكيدين على **تعليقٍ** و**فرعٍ ميّتٍ** كتبهما الوكيلُ ليُرضيَهما — تأكيدٌ
على نصِّ ملفٍّ يُرضى بنصٍّ لا بسلوك. ومرساةُ `test_presence_frontend_contract`
تبعتها (`photoUrl={employee.photo_url}` في البطاقة، و`occupant.photoUrl ?` في
المقعد — الموضعان يرسمان الوجهَ بطريقتين).
...
- الاستقبالُ **لا يُفوتِر** (D-4، قرار المالك): `receive_channel_work_order` لا يمسّ `consumed_quota`؛ الزبونُ يُفوتَر بوحدات المستندات بعد اعتماد المُسلَّم وحدَها (`_apply_usage_event_to_quota`). وقفلُ الاشتراك عند الاستقبال باقٍ لتسلسل فحص الفرادة، ولا حدَّ يُنفَذ هناك بالعدّاد — الأهليّةُ حالةُ الاشتراك لا استهلاكُه.
...
- للإشعارات **منتِجان حقيقيّان** لا دالة إنشاء بلا مستدعٍ: `notify_quota_exceeded`
  تُطلق مرة واحدة لحظة عبور الحد داخل `_apply_usage_event_to_quota` (حيث يتحرّك العدّاد وحدَه منذ D-4)، و`notify_sla_breach`
...
- **بابُ موظّف المنصّة: أربعُ عُقَدٍ لا واحدة** (211 م١). موظّفٌ تُقبَل دعوتُه لم يكن يصل إلى `PlatformEmployeeWorkspace` أبداً، والعُقَدُ مستقلّةٌ وكلٌّ منها تكفي وحدَها لقطع الطريق:
  ...
  - **(ج) `ApplicationBoundary` كان يبتلع من لا شركةَ له**: `accept_applicant_invitation` لا يُنشئ `UserCompanyMembership` — بخلاف نظيرتها في `employee_ops` — فمن عُيِّن ولم يُسنَد بعدُ يهبط على **«أنشئ شركتك الأولى»**. صار `frontend_v2/index.tsx` يستثني مساراتِ المنصّة (`platformTenantlessPath`) كما يستثني مساراتِ المحاسب وبالحجّة نفسِها: هذه الشاشاتُ لا تقرأ شركةَ جلسةٍ إطلاقاً. **ومن دخل من الباب العادي بلا شركة** (D-2) يُسأل الخادمُ عن قدراته قبل رسم نموذج التأسيس (`usePlatformStaffCapabilitiesState` — المصدرُ نفسُه لـ`StaffLoginPage` و`StaffShell`)، وقرارُ «انتظار · `/staff/home` · نموذج التأسيس» دالّةٌ خالصة `frontend_v2/utils/staffAccess.ts` (`firstCompanyGate`) فلا يومض النموذجُ قبل الجواب. وزرُّ «ادخل إلى مساحتك» في `PlatformInvitationPage.tsx` يقود إلى `/staff/home` لا `/platform/employee-space`.
...
- **صفحةُ المتابعة لا تحمل بياناتِ المتقدّم إلى المتقدّم (#215):** الاسمُ والهاتفُ والبريدُ و`about` و`notes` و`rating` و`cv_url` **لا يخرج شيءٌ منها** من `platform_ops/public_hiring/`؛ كلُّها معروفةٌ لصاحبها أصلاً، وإعادتُها تحوّل تخمينَ رمزٍ إلى تسريبِ بياناتٍ شخصيّة. ومفاتيحُ صفّ التحديث العامّ **محصورةٌ بمجموعةٍ في الحارس** فلا يمرّ حقلٌ يُضاف غداً.
- **نصُّ حالة المتقدّم يُشتقّ من مصدرٍ واحدٍ في الخادم (#215):** `platform_ops/services.py` (`APPLICANT_PUBLIC_STATUS_LABELS`) — والواجهةُ تعرض `status_display` كما جاء ولا تبني قاموساً ثانياً يتباعد عن `choices`. و«مرفوض» تُكتب للمتقدّم «لم يقع الاختيارُ على ملفّك هذه المرّة»: الحقيقةُ نفسُها بلا كلمةٍ تبقى على شاشته.
- **رابطُ الرسالة يُفحَص في الخدمة لا يُترَك لنوع العمود (#215):** `objects.create()` **لا يشغّل مدقّقاتِ الحقول**، فـ`URLField` لا يمنع `javascript:` من الاستقرار في العمود — والقيمةُ تُصيَّر `href` في صفحةٍ عامّة. الفحصُ في `publish_applicant_notice`.
- **شروطُ الرسالة في موضعٍ واحد (#216):** `platform_ops/services.py` (`_clean_notice_payload`) تقرؤها الفرديّةُ والجماعيّةُ معاً — ونسخةٌ ثانيةٌ منها تعني باباً يقبل غداً ما يرفضه الآخر. و**الإرسالُ الجماعيُّ مقصورٌ على إعلانٍ واحد**، وحالةٌ مكتوبةٌ غلطاً تُردّ **باسمها** لا تُترَك تطابق صفراً بصمت (فالرفضُ حينها «لا يوجد متقدّم» — جوابٌ صحيحٌ عن سؤالٍ خاطئ يخفي الغلطةَ الإملائيّة).
- **الاجتماعُ يبلغ المتقدّمَ من دفتر الاجتماعات القائم (#215):** `applicant_upcoming_meetings` قراءةٌ فوق `ApplicantMeetingAttendee` — لا جدولٌ ثانٍ يُملأ باليد فيتباعد عن موعده. **وإضافتُه إلى قائمة الحاضرين هي الدعوةُ نفسُها**، ولذلك لا علَمَ نشرٍ عليها؛ ومَن لا يُراد إخبارُه لا يوضَع فيها. ولا تخرج **حالةُ الحضور** إليه: «لم تحضر» تقديرٌ داخليٌّ يفتح جدلاً لا تديره صفحةٌ عامّة.
...
- **`consumed_quota` مغذًّى من الدفتر وحدَه (D-4، قرار المالك):** كان `receive_channel_work_order` (210-B) يزيد العدّادَ واحداً لحظةَ الاستقبال بلا حدث دفتر، ثمّ يزيده الدفترُ بوحداته عند الاعتماد — فأمرُ القناة يُفوتَر مرّتين. أُزيلت زيادةُ الاستقبال، وانتقل معها إشعارُ «تجاوز الباقة» إلى `_apply_usage_event_to_quota`. ويحرسه `test_usage_ledger.py` (`ChannelWorkOrderIsBilledOnlyByTheLedgerTest`). يبقى العدّادُ مسقَطاً تزايديّاً (فرقُ تقريبَي المجموع) لا `SUM` يُعاد حسابُه، ويُصفَّر يدوياً بالتدوير.
...
- **العدّادُ بلا تاريخ:** `consumed_quota` يُحسب على الدورة الجارية لحظةَ اعتماد المُسلَّم، فمُسلَّمٌ يُعتمَد بين `period_end` وتشغيل الأمر يُفوتَر على الدورة المنتهية.
...
| `platform_ops/tests/test_hiring_portal.py` | بوابة التوظيف والسطح الإداري (`PlatformHiringAdminSurfaceTests`): النماذج بلا `tenant`، فحص السيرة بالبايتات والبنية، التقديم العام، 410 للمغلق، تهشير رمز الدعوة، إنشاء الحساب عند القبول لا قبله، منع تحريك الحالة بكتابة مباشرة، رفض حذف إعلان له متقدمون، تمرير بايتات السيرة بلا تسليم رابط التخزين، عزل دور التوظيف عن كل مسارات المنصة، وجرد السطح العام بخانقه ومصادقته، ورفض المتقدم يبطل الدعوة، و`PATCH is_open` متجاهل، واسم السيرة العربي `filename*=`، وقوة كلمة المرور وسباق اسم المستخدم |
| `platform_ops/tests/test_applicant_meetings.py` | اجتماعاتُ المتقدّمين (212-S1): نافذةُ الاجتماع على القاعدة وفي الخدمة معاً، وهويّةُ الحاضر الواحدة (لا الاثنان ولا لا شيء)، ومنعُ تكراره، ومنعُ إضافة من وُظِّف **لحظةَ الإضافة وحدَها** (فالفحصُ الرجعيُّ يمحو تاريخاً)، واستقلالُ الملاحظة عن الحضور، و«مدعوّ» افتراضاً لا «غائب»، ومنعُ الكتابة على حاضرِ اجتماعٍ آخر بمعرّفٍ مخمَّن، وعدمُ تسريب `cv_url` من الباب الجديد. وحرّاسٌ ساكنةٌ على ما لا يقرؤه `tsc`: نافذةُ الاجتماع تُحوَّل قبل أن تُقارَن (طرفٌ نصٌّ من الشبكة وطرفٌ `datetime` من الصفّ ⇒ خمسمئة)، ومنتقي المتقدّمين يخفي **الحالةَ نفسَها** التي ترفضها `add_meeting_attendee` |
| `platform_ops/tests/test_applicant_tracking.py` (#215) | صفحةُ متابعة المتقدّم: الدفترُ يُفتَح بأوّل سطرٍ لحظةَ التقديم، وكلُّ انتقالٍ يخلّف صفّاً **يُقرأ من القاعدة** لا من الكائن العائد، و**الفشلُ موحَّدٌ بايتاً ببايت** لأربعة أسبابٍ مختلفة، والحمولةُ العامّةُ بلا اسمٍ ولا هاتفٍ ولا بريدٍ ولا `notes` (مفتاحاً **وقيمةً**)، ومفاتيحُ صفّ التحديث **محصورةٌ بمجموعةٍ** فيسقط أيُّ حقلٍ يُضاف غداً قبل أن يصل قارئاً، والاجتماعُ لا يبلغ غيرَ حاضره، والردُّ يُقفل عند الرفض/التعيين واللوحةُ تبقى مقروءة، و`javascript:` في الرابط يُردّ (`objects.create()` لا يشغّل مدقّقاتِ `URLField`)، **والشارةُ بلا استعلامٍ إضافيّ** (عددُ الاستعلامات ثابتٌ مهما زادت الصفوف). وعدّادُ الفشل يُختبَر بذاكرةٍ حقيقيّةٍ صراحةً لأنّ `core/test_settings.py` تستعمل `DummyCache` |
| `platform_ops/tests/test_hiring_frontend_contract.py` | حارس العقد بين واجهات التوظيف والفوترة بالـ frontend وحمولات الخادم الحقيقية للـ 15 واجهة، ومطابقة `APPLICANT_STATUS_OPTIONS`/`EMPLOYMENT_TYPE_OPTIONS` لـ`choices` الخادم |
