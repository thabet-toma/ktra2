...
- ترتيب الأقفال الصارم: `Tenant -> ServiceSubscriptionPolicy -> IntegrationKey -> ServiceSubscription -> CompanyHealthCheck -> CompanyHealthCheckItem -> CustomerAcquisition -> PlatformEmployee -> Engagement -> WorkOrder -> WorkOrderDeliverable -> UserCompanyMembership -> DailyRating -> JobPosting -> JobApplicantInvitation -> JobApplicant` (محروسٌ ساكناً بـ`platform_ops/tests/test_engagement_lifecycle.py` (`LockOrderSourceGuardTest`)).
