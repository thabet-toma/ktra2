...
| `PartyRefundAllocation` | `amount` (بعملة السند والمصدر)، `amount_base` | `refund→CustomerPayment` (`refund_sources`) — سند الاسترداد، وواحدٌ فقط (CheckConstraint `refund_allocation_one_source`) من `source_note` / `source_supplier_payment` / `source_customer_payment` (`refunds`). يُحتسب ما دام سند الاسترداد مرحَّلاً، ويُطرح من «غير الموزَّع» لمصدره في كل قارئ |
| `CreditDebitNoteAllocation` | `amount` (بعملة الإشعار)، `amount_in_invoice_currency`، `amount_base` | `note→CreditDebitNote` (`allocations`)، وواحدٌ فقط (CheckConstraint `note_allocation_one_target`) من `sales_invoice` / `purchase_invoice` (`note_allocations`). المستحقّات اللوجستية في `logistics.LogisticsAccrualAllocation.note`. الهجرة 0046 تحوّل كل مدينٍ مرحَّلٍ مربوطٍ لدائنٍ توزيعاً بمبلغه (`backfill_linked_debit_notes`) فلا يتغيّر متبقٍّ |
...
def credit_debit_counter_account_error(account, tenant_id) -> str | None:  # لا ذمم ولا مخزون ولا مجمِّع ولا نقد/بنك/شيكات
# توزيع الإشعار المسوّي كالسند — `sales/services/note_allocation.py`
def note_settles(note) -> bool:  # مدينٌ على دائن أو دائنٌ على عميل — يُنقص ما يُستحق فيُوزَّع
def note_unallocated(note) -> Decimal:  # غير الموزَّع من المسوّي المرحَّل بعملته — «تحت الحساب»
def allocate_note(note, allocations, *, user=None):  # [{kind: sales_invoice|purchase_invoice|clearance|freight|local, id, amount}] — ربطٌ بلا قيد؛ فاتورة البيع يزيد amount_paid
def deallocate_note(note, *, allocation_kind, allocation_id, user=None):  # invoice|accrual — يعود تحت الحساب
def auto_allocate_linked(note, *, user=None) -> Decimal:  # الترحيل: على المستند المربوط بحدّ متبقّيه، والزائد يبقى تحت الحساب
def release_note_allocations(note) -> list[str]:  # إلغاء الترحيل: يفكّ الكل في المعاملة نفسها
def note_open_targets(note) -> list[dict]:  # مستندات الطرف المفتوحة بعملة الإشعار — FIFO؛ suggest_note_fifo يملؤها
def posted_invoice_settled_total(invoice_id) -> Decimal:  # السندات + الإشعارات المرحّلة الموزَّعة — عقد amount_paid (posted_allocations_total نقدٌ وحده: سقف ردّ المرتجع)
# فائض الطرف واستردادُه نقداً — `sales/services/party_surplus.py`
def party_surplus_rows(tenant_id, partner) -> list[dict]:  # الدائن: سندات صرف + إشعارات مدينة غير موزَّعة؛ العميل: سندات قبض + إشعارات دائنة — الأقدم أولاً
def attach_refund_sources(payment, sources, *, user=None):  # [{source: note|supplier_payment|customer_payment, id, amount}] — قبضٌ من الدائن، صرفٌ (refund) للعميل، بعملة السند وبحدّ الفائض
def guard_refund_sources(payment):  # post_customer_payment: ما يُطفئه لا يزال فائضاً عند الترحيل
```
...
| GET/POST · PATCH/DELETE (مسودة) | `credit-debit-notes/` · POST `{id}/post/` · `{id}/unpost/` (يعيد `notice` بما فُكّ) · `{id}/cancel/` · GET `default-account/?partner=&related_…=` | `CreditDebitNoteViewSet` (`views.py`) — صلاحيات دفتر اليومية: `accounting.journal.view` / `.create` / `.post` / `.unpost` |
| GET · POST · POST | `credit-debit-notes/{id}/allocation-targets/` · `{id}/allocate/` (`allocations` أو `fifo: true`) · `{id}/deallocate/` (`allocation_kind`, `allocation_id`) | `CreditDebitNoteViewSet` — غير الموزَّع وتوزيعاته ومستندات الطرف المفتوحة واقتراح FIFO؛ العرض `.view` والتوزيع `.post` |
...
## قواعد لا يجوز كسرها
- **الإشعار المدين/الدائن لأيّ طرف، والدلالة من منظور ذمّته** (`sales/services/orders.py` — `post_credit_debit_note`): المدين Dr ذمّة الطرف (يزيد ما على العميل أو ينقص ما للدائن)، والدائن Cr ذمّته — للعميل هذا سلوك M4-T4 حرفياً. الذمّة `_resolve_ap_account` للدائن (`is_creditor_party`) و`_resolve_ar_account_for_partner` (أو ذمّة فاتورة البيع المربوطة) للعميل. **المقابل** يختاره المستخدم؛ الفارغ يُحلّ بـ`default_note_counter_account`: مصروف المستند المربوط (أكبر مصروفٍ في قيد استحقاق التخليص، `expense_account` للإرسالية، مصروف بنود فاتورة الشراء، إيراد فاتورة البيع) ثم حسب النوع — عميل ← الإيرادات · مخلّص 5307 · وكيل شحن 5301 · ناقل 5305 · مورد ← تكلفة المبيعات (لا المخزون: قيدٌ عليه بلا طبقة يكسر تطابق الدفتر والطبقات). **حسابات الذمم والمخزون والمجمِّعة، والنقدُ والبنوك والشيكات (`accounting.api.money_account_kind` — «الإشعار ليس دفعاً») مرفوضةٌ مقابلاً**؛ وما رُحِّل قبل الحارس يسرده `python manage.py list_notes_with_cash_counter --tenant N` بلا تعديل. الضريبة جزءٌ من `amount`: مخرجاتٌ للعميل (`accounting.services.resolve_output_vat_account`) ومدخلاتٌ 1105 للدائن. العملة الأجنبية تملأ `amount_currency` من `post_journal`. **الربط** بمستندٍ واحدٍ لنفس الطرف، مرحَّلٍ استحقاقه (فاتورة الشراء بعملتها، لا مرتجع). **الإشعار المسوّي رصيدٌ للطرف يُوزَّع كالسند** (`sales/services/note_allocation.py`): المدين على دائن (خصمٌ منه) يُطفئ فواتير شرائه ومستحقّاته اللوجستية، والدائن على عميل يُطفئ فواتير مبيعه (ويزيد `amount_paid` — شريك السندات في عقده). يُوزَّع على مستنده المربوط عند الترحيل بحدّ متبقّيه، والزائد وغير المربوط «تحت الحساب» في بطاقة الطرف والأعمار حتى يُوزَّع يدوياً أو FIFO؛ وإلغاء ترحيله يفكّ توزيعاته في المعاملة نفسها وينبّه. **المعاكس** (دائنٌ على دائن، مدينٌ على عميل) يزيد ما يُستحق فلا يُوزَّع: ربطه يرفع مستحقّ التخليص/الإرسالية/الشحن/فاتورة الشراء، وربط فاتورة البيع له مرجعٌ فقط. فاتورة بيعٍ عليها توزيع إشعارٍ لا يُلغى ترحيلها قبل فكّه (`guard_invoice_payments_before_unpost`). المرحَّل لا يُعدَّل ولا يُحذف؛ إلغاء الترحيل عبر `unpost_document`، والإلغاء للمسودة.
...
- **استرداد الفائض نقداً بالسند القائم** (`sales/services/party_surplus.py`): «سند قبض (استرداد)» في بطاقة الدائن (Dr صندوق / Cr ذمّته) و«سند صرف (ردّ فائض)» في بطاقة العميل (`kind='refund'`: Dr ذمّته / Cr صندوق) — نافذة `NewPaymentModal` نفسها بـ`refundMode`، تعرض الفائض أعلاها (`partners/{id}/surplus/`) ويُختار ما يُطفئه السند منه (`refund_sources` في `payments/` POST). بلا توزيعٍ على فواتير، وردُّ العميل بلا شيكات. إلغاء ترحيل الإشعار يحذف روابط استرداده ويسمّيها في `notice`.
- **`amount_paid` = مجموع توزيعات السندات والإشعارات الدائنة المرحّلة**: كل زيادة عليه تأتي من `post_customer_payment` مقرونةً بصفّ `PaymentAllocation`، أو من `allocate_note` مقرونةً بصفّ `CreditDebitNoteAllocation`، ويُثبته `posted_invoice_settled_total` (`sales/services/note_allocation.py`). و`posted_allocations_total` (`sales/services/flow.py`) يبقى السندات وحدها: منه يُبنى سقف الردّ النقدي للمرتجع، والإشعار ليس نقداً. الحقل مخزنٌ مُشتقّ عمداً — تقرأه التقارير وكشف الحساب والأعمار بجمع SQL — فثمنُه أن يُبرهَن لا أن يُفترض: `python manage.py audit_ar_integrity` (`sales/management/commands/audit_ar_integrity.py`) يمسح كل فاتورة مرحّلة عليها «مدفوع» أو توزيع، ويصنّف كل فرق إلى **قديم** (قيد الفاتورة نفسه يحمل التسوية — دائنُ ذمم داخله بدلالة `invoice_journal_settlement_credit`، أو نقديّةُ ما قبل الميزة 2 بقيدٍ بلا مدين ذمم: متوازنٌ وصحيح، يُبلَّغ ولا يُصلَح أبداً) أو **يتيم** (لا توزيع ولا تسوية داخل القيد — وحده ما يُعيده `--fix`/`--apply` إلى مجموع التوزيعات). الأمر عرضٌ فقط افتراضياً ولا يلمس قيداً إطلاقاً؛ والمنتج القديم يحرسه الحارسان معاً (المتبقي على `amount_paid` + `guard_invoice_allocation_total`) فلا يُحصَّل مرّتين.
- **لا إلغاء ترحيل (ولا حذف) لفاتورة عليها سند قبض مرحّل**: `guard_invoice_payments_before_unpost` (`services.py`) يُستدعى من `views.py` و`:314`. والاستثناءُ سندٌ أنتجه الترحيل نفسه ويُحرَّر قبل الحارس: الموسومُ بـ`auto_settled_invoice` (التسويةُ النقدية التلقائية أو تحصيلُ الشيكات المرفقة) عبر `release_auto_cash_settlement`، والموسومُ بـ`refund_for_invoice` (ردُّ الدفعة) عبر `release_auto_sales_return_refund`. **وسندُ الردّ الذي أنشأه المستخدم لا يُمَسّ** — يحرسه الحارسُ نفسُه.
- **ردُّ دفعة الزبون عند ترحيل مرتجع البيع (#167)**: `_process_sales_return_refund` (`sales/services/flow.py`) يُنفَّذ داخل `post_sales_invoice` لمرتجعٍ له `original_invoice`. **الورقُ أوّلاً ثمّ النقد** — وإلّا استُنفد المرتجعُ بالنقد وبقيت ورقةُ الزبون في محفظتنا بلا مقابل. والسقفان في `calculate_sales_return_refund_caps`: **سقفُ النقد** = التوزيعاتُ المرحّلة ناقصاً **كلَّ ورقةٍ لم تصر مالاً في الصندوق** (أي ما حالتُها ليست `Collected` ولا `Settled`) وناقصاً كلَّ توزيع ردّ مرحّل على المراجيع السابقة للأصل نفسه — سواء كان السند تلقائياً موسوماً بـ`refund_for_invoice` أم يدوياً بلا الوسم. والطرحُ بحالة **الوصول** لا بحالة الانتظار عمداً: القراءةُ المعكوسة («اطرح ما هو في المحفظة أو برسم التحصيل») تُسقط الورقةَ من الطرح متى غادرت الحالتين — رُدَّت لصاحبها أو ارتدّت أو ظُهِّرت — بينما تبقى داخل التوزيعات المرحّلة، فتتحوّل قيمتُها إلى نقدٍ قابلٍ للصرف: ورقةٌ أُعيدت لصاحبها في مرتجعٍ تفتح سقفَ نقدٍ في المرتجع الذي يليه؛ **وسقفُ الورق** = شيكاتُ الأصل بحالة `Received` وحدَها، أقدمُ استحقاقاً أوّلاً، **والورقةُ كاملةٌ أو لا تُردّ**. من دون هذين السقفين يخرج نقدٌ حقيقيٌّ من الصندوق مقابل ورقةٍ لم تُقبض بعد. كل مسارٍ مُعدِّل يقفل المرتجع وأصله معاً بترتيب `pk` ثابت ويعيد جلبهما قبل قراءة السقف، فلا يستهلك شقيقان قدرة النقد نفسها. الورقُ يمرّ من `transfer_cheque(..., 'return_to_customer', sales_return_id=…)` القائمة، والنقدُ سندُ `CustomerPayment` بـ`kind='refund'` (مدين ذمم ÷ دائن صندوق) موسومٌ بـ`refund_for_invoice`. **بلا صندوقٍ افتراضيٍّ والنقدُ لازم ⟵ يُرفض الترحيل** لا يُتخطّى صامتاً. ويقودُه `SalesSettings.auto_refund_on_sales_return` (مطفأٌ افتراضيّاً فلا تبدأ شركةٌ قائمةٌ بالصرف لأنّ نسخةً نُشرت)؛ ومطفأً لا يقع شيءٌ ما لم يصل `{"refund": {...}}` صريحاً في جسم طلب `invoices/{id}/post/` — **والواجهةُ لا تُلفّق الحقل، فغيابُه يعني «اقرأ الإعداد»**.
- **ذمم الأصل تُقرأ بصافي رصيد المرتجع المرتبط**: `linked_return_credit_summary` (`sales/services/calc.py`) هو المصدر الواحد لـ`reports/aging/` و`receivables-aging` واقتراح FIFO وحراسات التوزيع. المرتجع المرحّل وبعملة الأصل لا يظهر ديناً مستقلاً؛ رصيده المفتوح = إجماليه ناقص توزيعات ردّ الدفعة المرحّلة وناقص ورق `return_to_customer` ذي قيد فعّال، وصافي الأصل لا ينزل عن صفر. سند القبض لا يُوزَّع على مرتجع، وسند الرد لا يتجاوز رصيده المفتوح أو سقف النقد المشترك للأصل، وكلا الفحصين تحت أقفال الصفوف نفسها. `amount_paid` على الأصل يبقى تحصيلاً نقدياً فقط ولا يُعاد كتابته بسبب المرتجع. والمرتجع الجديد المرتبط يطابق أصلَه في الشركة والعميل والعملة في الـserializer والخدمة؛ الصفوف التاريخية لا تُعدَّل. رصيد كل مرتجع — مرتبطاً أم لا — مصدره `sales_return_open_credit` (`sales/services/calc.py`)، فصفٌّ تاريخيّ بعملة تخالف أصله لا يُطرح من الأصل ولا يستهلك سقف نقده لكن رصيده يبقى قابلاً للردّ. **والرصيد الدائن غير المطبَّق** (مرتجعٌ بلا أصل أو ذلك الصفّ التاريخي) يجمعه `unapplied_sales_return_credits` ويُعرض **سالباً** في خانة عمره في `receivables-aging` فيُصفّي إجمالي الزبون — كإشعار الدائن غير المطبَّق في QuickBooks وOdoo. أمّا `reports/aging/` فقائمةُ فواتير مفتوحة للتوزيع لا ملخّص أرصدة، فلا تحمل المراجيع.
- **وسمُ إعادة الورقة بنيويٌّ لا نصّيّ**: `ChequeMovement.sales_return` هو ما يعرف به فكُّ الترحيل أيَّ ورقةٍ أعادها **هذا** المرتجع. وسمٌ نصّيٌّ في `notes` يُطابَق بـ`contains` يَعِد بما لا يضمن — تكفي جملةٌ يكتبها مستخدم. وعند التحرير **تُعاد الورقةُ `Received` وتُكتب حركةُ `revert`** (كما يفعل `record_document_cheque_unposting`) **ويُفرَّغ الوسمُ** — فلا يُمحى سجلُّ الورقة ولا تلتقط إعادةُ الترحيل حركةً قديمة. وحالةُ `Returned` **تبقى نهائيّةً** في `INCOMING_TRANSITIONS`: الاستعادةُ من مسار التحرير وحدَه، لا انتقالٌ عامٌّ بزرّ.
- **قيدُ تكلفة المبيعات مصدرُه حركةُ المخزون لا `Product.avg_cost` (#137)**: `_build_cogs_journal_line_dicts` (`services/calc.py`) يأخذ `costs_by_product` — مجموعَ `StockMovement.total_cost` الفعليّ لهذا المستند — **وسيطاً إلزامياً بلا احتياطٍ إلى `avg_cost`**. ولهذا **تُنشأ حركاتُ المخزون قبل بناء القيد** في `post_sales_invoice` (كان العكس)، وفي `deliver_invoice_lines` و`create_standalone_delivery_note` تُجمَع الكلفةُ من الحركات التي أنشأها المسارُ نفسُه. تحت المتوسّط المرجَّح كان الرقمان يتطابقان حكماً؛ تحت FIFO يفترقان كلّما عبرت البيعةُ طبقتين بسعرين — فيصير الدفترُ يقول رقماً و`sales_cogs_map` تقول غيره. الوسيطُ إلزاميٌّ عمداً: مستدعٍ جديدٌ ينساه يُكسَر بصوتٍ بدل أن يرث العطب صامتاً.
- **مرتجعُ البيع يعود إلى طبقاتِ فاتورته الأصليّة (#137)**: ترحيلُ فاتورةٍ من نوع `sale_return` تحمل `original_invoice` يجلب حركاتِ صرفِ الأصل **دفعةً واحدة** ويمرّرها `restores_movement` — فتُفكّ الطبقاتُ بعكس ترتيب استهلاكها بمقدار الكميّة الراجعة وحدها، وتعود البضاعةُ إلى مواقعها في الرتل. مرتجعٌ كاملٌ ⟵ كلفتُه = كلفةُ البيعة بالضبط (الربحُ يصفّر)؛ ومرتجعٌ بلا `original_invoice` أو أكبرُ من البيعة يسقط إلى طبقةٍ جديدةٍ للفائض وحده بلا كسر.
...
- **اسمُ صنف السطر في الواجهة قاعدةٌ واحدة**: `frontend_v2/utils/productDisplayName.ts` (`documentLineProductName`) — اللقطةُ المجمَّدة أوّلاً، وإلّا `formatProductPrimaryName` (أي `display_name` بالبراند). خليّةُ المحرِّر ووجهُ المستند والطباعة يمرّون بها. كانت مكتوبةً ثلاثاً بمآلين: المحرِّرُ يعرض البراند ووجهُ المستند والطباعةُ يسقطان إلى `name_ar` الخامّ، فالمسودّةُ (بلا لقطة) تُفتح بلا براند والمرحَّلةُ به. والطلبيّةُ وعرضُ السعر وسندُ التسليم يسمّون الصنفَ بـ`formatProductPrimaryName` كذلك، ومُطابِقا الطلبيّة والعرض **يحملان `display_name`** (كانا يُسقطانه فتُسمّى القائمةُ وكلُّ سطرٍ يُختار منها بلا براند). يحرس ذلك `frontend_v2/utils/productDisplayName.test.ts`: لا سقوطَ إلى `name_ar ||` في الملفّات الخمسة، ولا مُطابِقٌ بلا `display_name`.
...
- **اسمُ المرتجع في الوجه المالي مشتقٌّ من نوعه لا من مرجع قيده (#214-ب)**:
  قيدُ مرتجع البيع يُكتب بـ`reference_type="SALES_INVOICE"` — **كالبيعة حرفاً**،
  لأنّ المرتجع فعلاً صفُّ `SalesInvoice` بنوعٍ آخر. وكشفُ الحساب وعنوانُ نافذة
  التفاصيل يشتقّان اسمَ الحركة من ذلك الحقل وحدَه، فكانا يكتبان اسمَ فاتورة
  المبيعات على المرتجع (بلاغُ المالك: «لمّا أضغط عليه بتبيّن كلمة فاتورة مبيعات
  بالعنوان»). **ولم يُلمَس القيد**: تغييرُ `reference_type` يمسّ مفتاحَ
  الـidempotency الذي يبني عليه فكُّ الترحيل وسلّةُ المحذوفات — ثمنٌ فادحٌ
  لتصحيح كلمة. فصار `partner_account_statement` يرسل `reference_kind` مع كلّ
  صفّ (`accounting/services.py` ← `_attach_statement_document_links`، **عمودٌ
  ثالثٌ في استعلامٍ قائمٍ لا استعلامٌ ثانٍ**)، وتقرؤه
  `utils/entityLinks.ts` (`referenceTypeLabel`) بوسيطٍ **اختياريّ** فلا ينكسر
  مستدعٍ لا يملك النوع. والاسمُ «مرتجع بيع» واحدٌ في المحرّر والطباعة وكشف
  الحساب — مصدرُه `utils/documentTypeLabels.ts` (`invoiceKindLabel`). ودفترُ
  اليومية كذلك: `reference_kind` على صفّ القيد، واسمُ المصدر وملخّصُ المرجع
  يقرآن نوعَ المستند (`accounting/serializers.py` (`RETURN_KIND_LABELS`)).
- **مرتجعُ البيع يُرحَّل من شاشته هو (#214-ب)**: شكوى المالك أنّ المرتجع
  «احفظ كمسودة وبعدها ارجع لفواتير المبيعات رحّلو — مش عملي إطلاقاً»، وهي دقيقة:
  `SalesReturnEditor.tsx` أفعالُه أربعةٌ ليس فيها ترحيل، و`postSalesInvoice`
  **غيرُ مستورَدٍ فيه أصلاً**، ورسالةُ النجاح نفسُها تأمر بالمغادرة. **والخادمُ لم
  يكن يمنع شيئاً**: `invoices/{id}/post/` يقبل المرتجعَ كما هو منذ #167 — فالنقصُ
  في الشاشة وحدَها. وهو ما يفعله Odoo («Reverse and Create Invoice» تُصادق فوراً)
  وZoho Books ودفترة: الإشعارُ الدائن يُرحَّل من موضعه. فصار للشاشة فعلٌ رئيسٌ
  «حفظ وترحيل»، ومنطقُ خيار وجهة الردّ (`getSalesReturnRefundOptions` ⟵
  `SalesSettings.auto_refund_on_sales_return`) يُسأل **قبل** الترحيل من هنا كما
  يُسأل من قائمة الفواتير — نفسُ النقطة لا نسخةٌ ثانية. **ولا مسارَ ينتهي بمسودّةٍ
  بلا بابِ ترحيل**: إلغاءُ اختيارِ الردّ وفشلُ الترحيل والحفظُ المقصود ثلاثتُها
  تعرض الزرَّ نفسَه في مكان الرسالة (`pendingPostMessage`) — لأنّ المرتجعَ
  **محفوظٌ فعلاً** حينها، وتركُه بلا مخرجٍ هو الشكوى الأصليّة بعينها.
...
- **حسابا إيراد المنتج والخدمة لا يُثبَّتان على رأس الشجرة أبداً**: `default_revenue_account_product`/`_service` (`SalesSettings`) يُتركان فارغين عند إنشاء الشركة (`get_or_create_sales_settings`) — `resolve_product_revenue_account`/`resolve_service_revenue_account` (`sales/services/calc.py`) وحدهما يملآنهما: تحلّان `4101`/`4102` من الشجرة أو تُنشئانهما وتُثبّتانهما. العيب (ISSUE #59، وقبله #53 على جانب الخدمة): أوّل حساب إيراد **بالكود** هو رأس الشجرة «4» (`'4' < '41' < '4101'`)، حسابٌ أب لا يصلح هدفاً للترحيل. `python manage.py fix_product_revenue_account_default` يُصلح صفوف `SalesSettings` القائمة **حيث كان المُثبَّت حساباً أباً فقط** (idempotent، `--dry-run`) — بلا مساس بحسابٍ أو قيدٍ مُرحَّل.
  **وعادت العلّةُ من بابٍ ثانٍ ثم أُغلقت**: `resolve_default_account` كانت تطابق
  البادئةَ بـ`code__startswith` فتُرجع الأبَ نفسَه قبل فروعه (`"41"` يسبق `"4102"`
  نصّياً لأنّ الأقصر يسبق ما هو امتدادٌ له)، فشركةٌ فيها `41` بلا `4101` تُرحِّل
  إيرادَ بضاعتها على رأس شجرة الإيرادات. وكان أمرُ الإصلاح **يهزم نفسه** عليها:
  يصفّر المثبَّت ثم يعيد الحلّ فيعود بالأبِ ذاته ويطبع أنّه أصلحه. الآن
  `allow_parent=False` تستبعد كلَّ حسابٍ له أبناء (استعلامٌ فرعيٌّ واحد) وتُمرَّر من
  كلّ مستدعٍ يقصد **هدفَ ترحيل**، وسقطت البادئاتُ العريضة `"41"`/`"42"`/`"51"` —
  شبكةُ النوع+الاسم أوسعُ منها وأصدق. **والذمّةُ `1103` استثناءٌ مقصود**: أبُ
...
| `sales/tests/test_default_account_is_never_a_parent.py` | حسابٌ له أبناء لا يُعاد هدفَ ترحيل مهما طابقت بادئتُه · إيرادُ المنتجات يُنشأ ورقةً ولا يُخطَف حسابُ الخدمات `4102` · أمرُ الإصلاح يزحزح فعلاً ولا يعود بالأب · الذمّةُ `1103` تبقى الأبَ المقصود · ورقةٌ لا يطابق اسمُها الكلمةَ المفتاحية تبقى تُحَلّ (`1101` «النقدية» لا «صندوق») · والسلوكُ الافتراضيّ بلا `allow_parent` لم يتغيّر |
