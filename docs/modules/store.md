...
**حدود الكتابة على الصنف المخزني** — `store/serializers.py`
(`STORE_EDITABLE_ON_INVENTORY`): اللوحة تُعدّل على صنفٍ **ليس** `is_store_only`
...
الأصناف. السبب أن `store.manage` صلاحية تسويقية: تنشر الصنف وتصف واجهته ولا
تُعيد تعريفه، و`sale_price`/`sku`/`name_ar` تقرؤها الفوترة والتقارير.
صنف المتجر الخالص ملكُ اللوحة كاملاً فلا يخضع للحصر.

**الطلب المسبق قرارُ صاحب المتجر** — لا يُفرض على الأصناف المُنشأة من اللوحة.
«طلب مسبق» وعدٌ تجاري بتوفير الصنف عند الطلب، وافتراضُه في الكود كان يجعل كل
صنف جديد يقطعه نيابةً عن صاحبه.

**رمز الصنف تسلسلي لا عشوائي** — `ST-{n:06d}` عبر
...
على كرت الصنف ليس إذناً بإعلانه للعموم — متاجر الجملة تعرض الكتالوج وتترك السعر
لمحادثة.
...
· صفحة الصنف · صفحة الحملة) كلها تمرّ من هذه الدالة، يغطّيها الحجب بالبناء لا
...
تمثيلاتهما. إخفاء السعر **لا يُخفي الأصناف**: الكتالوج يبقى كاملاً بحالة توفّره.
