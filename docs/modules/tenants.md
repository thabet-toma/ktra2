...
| `tenants/company_templates.py` | سِجلّ قوالب الشركة `COMPANY_TEMPLATES` (`general` · `accounting_firm` · `client_book`) — بذرة كل قالب وأنواع دفاتره | 121 |
...
| `tests/test_client_book_template_api.py` (ISSUE #81) | `client_book`: البذرة (لا مخزون، 13 بند مصاريف تشغيلية) وخمسة دفاتر فقط، القناع يرث `accounting_firm` ويزيد فواتير البيع وأخواتها، سند القبض/الصرف يبقيان مفتوحين، سند إيراد وسند مصروف فعليان يُرحَّلان ويتوازنان، و`general`/`accounting_firm` بلا تغيير |
