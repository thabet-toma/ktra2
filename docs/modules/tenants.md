...
def create_company(name: str, creator_user, *, template: str = 'general', managed_by: Tenant | None = None, fiscal_year: int | None = None, fiscal_granularity: str = 'monthly') -> Tenant:  # إقلاع شركة كاملة: إعدادات + دفاتر + COA حسب القالب + خدماته (ISSUE #78) + فرع + مستودع + فترات السنة المالية (#213-أ) + عضوية مدير
...
- **الشركة الجديدة تولد وفتراتها المالية مفتوحة (#213-أ)**: `create_company` ينادي `accounting.services.create_fiscal_year` داخل معاملته — افتراضه سنة اليوم بتفصيل شهريّ (اثنتا عشرة فترة)، و`fiscal_year`/`fiscal_granularity` كلمتان مفتاحيتان تغيّران **السنة وتفصيلها لا وجودهما**: لا خيار تخطٍّ، لأن التخطّي هو العطب نفسه. قبل ذلك كانت `create_fiscal_year` **بلا مستدعٍ واحد في كود الإنتاج** (كل مستدعيها اختبارات)، فكل شركة تُنشأ من الواجهة تولد عمياء ولا يظهر ذلك حتى يرحّل صاحبها أول فاتورة فيُردّ بـ«لا توجد فترة مالية مفتوحة تغطي التاريخ …». الباب واحد لا ثلاثة: مسار الواجهة (`TenantViewSet.create`) ومسار الدفتر المُدار (`managed_books`) ومسار مكتب المحاسبة (`core/platform_admin_api.py`) كلها تمرّ بهذه الدالة. السنة تُحَدّ بين `MIN_FISCAL_YEAR`/`MAX_FISCAL_YEAR` (2000–2100) **قبل فتح المعاملة** لمنع الغلط المطبعيّ لا لفرض سياسة — الفترات التاريخية تُنشأ من شاشة إدارة الفترات المالية بلا هذا الحدّ. والشركات التي وُلدت قبل هذا التغيير تُصلَح بـ`heal_company_seed` (خطوة 5): يزرع سنة اليوم لمن لا فترة تغطّي يومه، ولا يلمس فترة مقفلة — القفل قرار محاسب لا عطب تأسيس.
