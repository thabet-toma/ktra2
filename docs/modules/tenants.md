...
| `tenants/company_templates.py` | سِجلّ قوالب الشركة `COMPANY_TEMPLATES` (`general` · `accounting_firm` · `client_book`) — بذرة كل قالب وأنواع دفاتره، وتصنيفُ بابه (`BOOK_ONLY_TEMPLATES`/`SELF_SERVE_TEMPLATES` + `assert_self_serve_template`/`assert_book_template`) | 121 |
