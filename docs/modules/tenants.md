...
كما يملك `create_company` الذي يُقلع شركة جديدة كاملة (إعدادات + دفاتر أنواع مستنداتها + شجرة حسابات + فرع رئيسي + مستودع + **فترات السنة المالية** + عضوية مدير) — **على الخطة التجريبية بأربعة عشر يوماً** (`core/plans.py` — `trial_end_date`)، لا `Enterprise` بلا حدود ولا انتهاء.
...
def create_company(name: str, creator_user, *, template: str = 'general', managed_by: Tenant | None = None, fiscal_start: str | datetime.date | None = None, fiscal_granularity: str = 'monthly') -> Tenant:  # إقلاع شركة كاملة: إعدادات + دفاتر + COA حسب القالب + خدماته (ISSUE #78) + فرع + مستودع + فترات السنة المالية من تاريخ بدئها (#213-أ) + عضوية مدير
...
- **الشركة الجديدة تولد وفتراتها المالية مفتوحة (#213-أ)**: `create_company` ينادي `accounting.services.create_fiscal_year` داخل معاملته — افتراضه **أول كانون الثاني من السنة الجارية** بتفصيل شهريّ (اثنتا عشرة فترة)، و`fiscal_start`/`fiscal_granularity` كلمتان مفتاحيتان تغيّران **تاريخ البدء وتفصيله لا وجودهما**: لا خيار تخطٍّ، لأن التخطّي هو العطب نفسه. و**البدء من أيّ تاريخ** لا من كانون الثاني وحده: سنةٌ مالية تبدأ في تموز خيارٌ قياسي (Odoo · Xero · Zoho Books)، والاثنتا عشرة فترةً تُحسب من تاريخ البدء بإزاحةٍ **من المرساة الأصلية** لا من الفترة السابقة (بدايةٌ في 31 كانون الثاني تُقصّ إلى 28 شباط ثم تعود 31 آذار، فلا ينزلق اليوم شهراً بشهر). قبل ذلك كانت `create_fiscal_year` **بلا مستدعٍ واحد في كود الإنتاج** (كل مستدعيها اختبارات)، فكل شركة تُنشأ من الواجهة تولد عمياء ولا يظهر ذلك حتى يرحّل صاحبها أول فاتورة فيُردّ بـ«لا توجد فترة مالية مفتوحة تغطي التاريخ …». الباب واحد لا ثلاثة: مسار الواجهة (`TenantViewSet.create`) ومسار الدفتر المُدار (`managed_books`) ومسار مكتب المحاسبة (`core/platform_admin_api.py`) كلها تمرّ بهذه الدالة. سنة تاريخ البدء تُحَدّ بين `MIN_FISCAL_YEAR`/`MAX_FISCAL_YEAR` (2000–2100) **قبل فتح المعاملة** لمنع الغلط المطبعيّ لا لفرض سياسة، وتاريخٌ يطابق الشكل ويستحيل تقويمياً (2026-02-31) يُردّ برسالة عربية — `parse_date` **ترفع** `ValueError` هنا ولا تعيد `None` — الفترات التاريخية تُنشأ من شاشة إدارة الفترات المالية بلا هذا الحدّ. والشركات التي وُلدت قبل هذا التغيير تُصلَح بـ`heal_company_seed` (خطوة 5): يزرع سنة اليوم لمن لا فترة تغطّي يومه، ولا يلمس فترة مقفلة — القفل قرار محاسب لا عطب تأسيس.
...
| `tests/test_company_seeding.py` | `create_company`: COA + دفاتر + بداية فارغة + عضوية مدير + عدم المساس بشركات قائمة + **فترات السنة المالية وترحيل أول قيد يوم الإنشاء وإصلاح الشركات العمياء (#213-أ)** |
...
| `core/tests/test_header_role_label.py` | **اسمُ دور العضويّة في ترويسة الواجهة مطابقٌ لـ`UserCompanyMembership.ROLE_CHOICES`:** كلُّ دورٍ يمنحه الخادمُ له اسمٌ عربيّ، **ولا اسمَ واحدٌ لدورَين** (خريطةٌ كاملةُ المفاتيح تسمّي أربعةً «موظف» فتمرّ الحارسَ الأوّلَ وهي تُخفي من أمامك)، والملصقُ يقرأ الخريطةَ ولا يعود سلسلةً شرطيّة. حارسٌ ساكنٌ بالضرورة: `npm test` لا يُصيّر مكوّناً و`tsc` لا يفحص نصَّ واجهة |
