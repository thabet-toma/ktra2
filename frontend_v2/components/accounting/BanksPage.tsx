...
import type { KitTab, KitToolbarAction, DenseColumn } from "../kit";
...
export const BanksPage: React.FC = () => {
  ...
  const bankColumns: DenseColumn<BankDto>[] = [
    ...
    { key: "active", header: "الحالة", render: (b) => (b.is_active ? "فعّال" : "معطّل") },
    {
      key: "del", header: "حذف",
      render: (b) => (
        <button type="button" className="ktra-toolbtn ktra-toolbtn--danger" disabled={busy}
    ...
    <>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", alignItems: "flex-end", marginBottom: "10px" }}>
        <div className="ktra-field">
          <label className="ktra-field-label">اسم البنك</label>
          <input className="ktra-input" value={bankName} placeholder="بنك فلسطين"
            onChange={(e) => setBankName(e.target.value)} />
        </div>
        <div className="ktra-field">
          <label className="ktra-field-label">SWIFT</label>
          <input className="ktra-input" value={bankSwift} placeholder="PALSPS22"
            onChange={(e) => setBankSwift(e.target.value)} />
        </div>
        <button type="button" className="ktra-toolbtn" disabled={busy || !bankName.trim()}
          onClick={() => void addBank()}>
          <Plus className="w-4 h-4" /> إضافة بنك
        </button>
      </div>
      <KitDenseTable<BankDto>
        columns={bankColumns}
        rows={banks}
        getRowKey={(b) => b.id}
        loading={loading}
  ...
  const branchesTab = (
    <>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", alignItems: "flex-end", marginBottom: "10px" }}>
        <div className="ktra-field">
          <label className="ktra-field-label">البنك</label>
          <select className="ktra-input" value={branchBank} onChange={(e) => setBranchBank(e.target.value)}>
            <option value="">— اختر —</option>
            {banks.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}
          </select>
        </div>
        <div className="ktra-field">
          <label className="ktra-field-label">اسم الفرع</label>
          <input className="ktra-input" value={branchName} placeholder="فرع رام الله"
            onChange={(e) => setBranchName(e.target.value)} />
        </div>
        <div className="ktra-field">
          <label className="ktra-field-label">رمز الفرع</label>
          <input className="ktra-input" value={branchCode} style={{ width: "110px" }}
  ...
  const accountsTab = (
    <>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", alignItems: "flex-end", marginBottom: "10px" }}>
        <div className="ktra-field">
          <label className="ktra-field-label">البنك</label>
          <select className="ktra-input" value={accBank}
        ...
        <div className="ktra-field">
          <label className="ktra-field-label">الفرع</label>
          <select className="ktra-input" value={accBranch} disabled={!accBank}
        ...
        <div className="ktra-field">
          <label className="ktra-field-label">تسمية الحساب</label>
          <input className="ktra-input" value={accName} placeholder="جاري شيكل"
            onChange={(e) => setAccName(e.target.value)} />
        </div>
        <div className="ktra-field">
          <label className="ktra-field-label">رقم الحساب</label>
          <input className="ktra-input" value={accNumber} onChange={(e) => setAccNumber(e.target.value)} />
        </div>
        <div className="ktra-field">
          <label className="ktra-field-label">IBAN</label>
          <input className="ktra-input" value={accIban} onChange={(e) => setAccIban(e.target.value)} />
        </div>
        <div className="ktra-field">
          <label className="ktra-field-label">العملة</label>
          <select className="ktra-input" value={accCurrency} onChange={(e) => setAccCurrency(e.target.value)}>
            <option value="">— اختر —</option>
            {currencies.map((c) => (
  ...
  return (
    <div>
      <KitDocumentShell
        ...
        status={
          <>
            <span className="ktra-status-item"><Landmark className="w-3 h-3" /> بنوك <b>{banks.length}</b></span>
            <span className="ktra-status-item"><Building2 className="w-3 h-3" /> فروع <b>{branches.length}</b></span>
            <span className="ktra-status-item"><Wallet className="w-3 h-3" /> حسابات <b>{accounts.length}</b></span>
          </>
        }
      >
        <></>
