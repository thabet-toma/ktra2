...
export interface AseelFloatWindowProps {
  open: boolean;
  onClose: () => void;
...
export const AseelFloatWindow: React.FC<AseelFloatWindowProps> = ({
  open,
  onClose,
  title,
  name,
    ...
    >
      ...
      >
        <div className="aseel-float-win__bar" onPointerDown={startDrag}>
          <span className="aseel-float-win__title" id={titleId}>{title}</span>
          {barExtras}
          <button
            type="button"
        ...
        <div className="aseel-float-win__body">{children}</div>
        {footer ? <div className="aseel-float-win__foot">{footer}</div> : null}

        {!isMobile && GRIPS.map(({ key, dir }) => (
          <div
            key={key}
            className={`aseel-float-win__grip aseel-float-win__grip--${key}`}
            data-grip={key}
