...
type Props = {
  ...
  /** #24: وضع تحديد الضمّ — قاعدةٌ واحدة بلا استثناء: المربّع على صفّ
   *  **البراند** دائماً (براندٌ مكشوفٌ من منتج، أو منتجٌ قديمٌ بلا أبٍ فوقه)،
   *  ولا يحمله صفّ المنتج مهما كان عدد برانداته — معرّفه مرجعيٌّ لا يمثّل
   *  الكلّ، فتحديده يوهم بتحديد كل البراندات بينما يرسل واحداً. ولا صفوف
   *  التصنيف. غيابه = الجدول كما كان تماماً. */
  selection?: {
...
export const GroupedItemsTable: React.FC<Props> = ({
  ...
  // سطر منتج (ورقة) — إزاحة الاسم حسب العمق. `reveal` حاضرةٌ لكل صفّ منتجٍ
  // له أبٌ (ولو ببراندٍ واحد): تضيف عنصر الكشف الصغير داخل عمود الاسم — لا صفٌّ
  ...
  const renderGroupNodes = (group: ProductGroup, depth: number): React.ReactNode[] => {
    ...
    // تريستار)». والحالة ليست نادرة — أكثر من نصف كتالوج شركة النور منتجاتٌ
    // ببراندٍ واحد. وهي حالةٌ عابرة أيضاً: يضيف المستخدم براندًا ثانياً غداً
