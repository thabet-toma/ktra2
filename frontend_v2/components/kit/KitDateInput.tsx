/**
 * N1-T5 — KitDateInput
 * Wrapper حول <input type="date"> يُطبّق ميزات Kit للحقول الزمنية:
 *
 *   - data-ktra-field="date" مُطبَّق تلقائياً
...
export interface KitDateInputProps
...
const YearCalendar: React.FC<YearCalendarProps> = ({
  ...
  return (
    <div
      ...
      className="ktra-date-cal__backdrop"
      onClick={onClose}
    >
      <div
        className="ktra-date-cal"
        onClick={(e) => e.stopPropagation()}
        dir="rtl"
      >
        <div className="ktra-date-cal__header">
          <button
            type="button"
          ...
          <input
            type="number"
            className="ktra-input ktra-date-cal__year"
            value={year}
            onChange={(e) => {
        ...
        <div className="ktra-date-cal__grid">
          {AR_MONTHS.map((monthName, mIdx) => {
            ...
            return (
              <div key={mIdx} className="ktra-date-cal__month">
                <div className="ktra-date-cal__month-title">{monthName}</div>
                <div className="ktra-date-cal__weekdays">
                  {AR_WEEKDAYS_SHORT.map((w) => (
                    <span key={w}>{w}</span>
                  ))}
                </div>
                <div className="ktra-date-cal__days">
                  {cells.map((d, i) => {
                    ...
                    return (
                      <button
                        ...
                        className={[
                          'ktra-date-cal__day',
                          isSelected ? 'ktra-date-cal__day--sel' : '',
                          isToday ? 'ktra-date-cal__day--today' : '',
                        ].filter(Boolean).join(' ')}
                        onClick={() => {
                          onPick(iso);
                          onClose();
...
export const KitDateInput: React.FC<KitDateInputProps> = ({
  value,
  onChange,
  ...
  const inputClass = useMemo(
    () => ['ktra-input', className].filter(Boolean).join(' '),
    [className],
  );
      ...
      {!disableCalendar && (
        <button
          ...
          style={{
            ...
            padding: 0, cursor: disabled ? 'default' : 'pointer', color: 'var(--ktra-ink-soft, #888)',
            display: 'inline-flex',
          }}
        >
