...
import { KitSpinner, KitEmptyState } from './KitStates';
import {
...
export type KitDenseTableProps<T> = {
...
export function KitDenseTable<T extends Record<string, any>>({
  columns,
  rows,
  getRowKey,
  onRowClick,
  ...
  // الخلية الخطأ في كل صفحة قائمة. الأرقام تبقى يميناً (عرف الأصيل + `.ktra-num`).
  ...
  return (
    <div className={`ktra-dense-table ${className}`}>
      {exportable && rows.length > 0 && (
        <div className="ktra-dense-toolbar">
          <button className="ktra-btn" onClick={handleExport} title="تصدير CSV" style={{ marginRight: 'auto' }}>
            تصدير
          </button>
        </div>
      )}
      <table className="ktra-grid" data-variant="list">
        <thead>
          <tr>
            {selectable && <th className="ktra-print-hidden" style={{ width: '32px' }}></th>}
            {columns.map((col) => (
              <th
                key={col.key}
                style={{
                ...
                className={[
                  col.sortable && onSort ? 'ktra-sortable' : '',
                  col.key === 'actions' ? 'ktra-print-hidden' : '',
                ].filter(Boolean).join(' ')}
                /* حالة الفرز تُعلَن هنا لا بالمثلّث المخفيّ عن قارئ الشاشة. */
                aria-sort={
                  col.sortable && onSort
                    ? sortKey === col.key
              ...
              >
                {col.header}
                {sortKey === col.key && (
                  /* زخرفة لا معلومة: حالة الفرز تصل قارئ الشاشة من `aria-sort`
                     لا من مثلّث. وبلا `aria-hidden` يدخل المثلّث في نصّ الرأس
                     فيتغيّر النصّ بتغيّر عمود الفرز — وهو ما جعل حارس التكافؤ
                     يبلّغ عن «عمود مفقود» وهو حاضر (شاشة الشيكات). */
        ...
        <tbody>
          {loading && rows.length === 0 && (
            <tr className="ktra-row--empty">
              <td colSpan={totalCols}><KitSpinner /></td>
            </tr>
          )}
          {!loading && rows.length === 0 && (
            <tr className="ktra-row--empty">
              <td colSpan={totalCols}><KitEmptyState hint={typeof emptyHint === 'string' ? emptyHint : undefined} /></td>
            </tr>
          )}
          {rows.map((row, idx) => {
            ...
            return (
              ...
              >
                {selectable && (
                  <td className="ktra-print-hidden">
                    <input
                      type="radio"
                      checked={isSelected}
