...
export type KitViewTone = "ok" | "warn" | "danger" | "info" | "muted";

export interface KitViewStatus {
  label: string;
  tone?: KitViewTone;
}

export interface KitViewMetric {
  label: string;
  value: React.ReactNode;
  tone?: KitViewTone;
}

export interface KitViewField {
  label: string;
  value: React.ReactNode;
...
export interface KitViewParty {
  title: string;
  icon?: React.ReactNode;
  fields: KitViewField[];
}

export interface KitViewColumn<T> {
  key: string;
  header: string;
...
export interface KitViewSection {
  key: string;
  title: string;
...
export interface KitDocumentViewProps<T> {
  ...
  status?: KitViewStatus;
  metrics?: KitViewMetric[];
  parties?: KitViewParty[];
  meta?: KitViewField[];
  columns: KitViewColumn<T>[];
  rows: T[];
  rowKey?: (row: T, index: number) => React.Key;
  totals?: KitViewTotal[];
  sections?: KitViewSection[];
  emptyRowsHint?: string;
  /** يُعرض أسفل المستند (شروط، توقيع) */
  footer?: React.ReactNode;
}

const TONE_CLASS: Record<KitViewTone, string> = {
...
const METRIC_VALUE_TONE: Record<KitViewTone, string> = {
  ok: "text-emerald-700",
  warn: "text-amber-700",
...
function FieldRow({ field }: { field: KitViewField; key?: React.Key }) {
  return (
    <div
      className={
        field.wide
 ...
 */
export function KitViewTable<T>({
  columns,
  rows,
  rowKey,
  ...
  const alignClass = (align?: KitViewColumn<T>["align"]) =>
...
export function KitDocumentView<T>({
  title,
  subtitle,
  documentNumber,
  status,
  ...
  footer,
}: KitDocumentViewProps<T>) {
  const { identity } = useTenantSettings();

  return (
    <div dir="rtl" className="ktra-doc-view flex flex-col gap-3 p-3 overflow-auto">
      {/* ── الترويسة ─────────────────────────────────────────────── */}
