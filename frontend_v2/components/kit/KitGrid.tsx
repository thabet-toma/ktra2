...
export interface KitGridColumn<T> {
  key: string;
  header: string;
  width?: string;
...
export interface KitGridProps<T> {
  columns: KitGridColumn<T>[];
  rows: T[];
...
export function KitGrid<T>({
  ...
  return (
    <div ref={containerRef} className="relative flex-1 overflow-y-auto min-h-0" style={{ zIndex: 0 }}>
      <table className="ktra-grid" data-variant={variant}>
        <thead>
          <tr>
            {columns.map((c) => (
          ...
          {rows.length === 0 ? (
            <tr>
              <td
                ...
                style={{ textAlign: 'center', color: 'var(--ktra-ink-soft)', padding: '14px' }}
              >
                {emptyHint}
              </td>
            </tr>
            ...
            rows.map((row, ri) => (
              <tr
                ...
                className={ri === selectedIndex ? 'ktra-row--sel' : undefined}
              ...
              >
                {columns.map((c) => {
                  ...
                  const alignClass =
                    c.align === 'left' || c.type === 'number' ? 'ktra-num' : undefined;
                  return (
