/**
 * N0-T7 — useKitIndexKeymap
 * Kit keyboard model for LIST/INDEX pages (not forms).
...
export type KitIndexKey =
  | 'F2' | 'F3' | 'F4' | 'F5' | 'F6'
  | 'Enter' | 'Escape' | 'AltF4'
  | 'CtrlHome' | 'CtrlEnd' | 'CtrlPageUp' | 'CtrlPageDown'
  | 'CtrlIns' | 'CtrlDel';

export type KitIndexKeymapHandlers = Partial<Record<KitIndexKey, () => void>>;
...
export function useKitIndexKeymap(
  handlers: KitIndexKeymapHandlers,
  { enabled = true }: Options = {},
): void {
  const ref = useRef<KitIndexKeymapHandlers>(handlers);
