...
import { claimCrmLead, createCrmActivity, createCrmLead, changeCrmLeadStatus, getCrmLead, listCrmActivities, listCrmColleagues, listCrmLeads, lookupCrmPhone, releaseCrmLead, requestCrmLeadTransfer, transferCrmLead, type CrmActivity, type CrmActivityInput, type CrmColleague, type CrmLead, type CrmLeadStatus, type CrmLookup } from '../../../../services/platformCrmApi';
...
export const CrmPanel: React.FC<CrmPanelProps> = ({ isManager, myEmployeeId }) => {
  ...
  const [colleagues, setColleagues] = useState<CrmColleague[]>([]); const [selected, setSelected] = useState<CrmLead | null>(null); const [activities, setActivities] = useState<CrmActivity[]>([]); const [detailLoading, setDetailLoading] = useState(false); const [detailError, setDetailError] = useState('');
  ...
  const loadLeads = useCallback(async (nextScope = scope, q = '', status: CrmLeadStatus | '' = '') => { setListLoading(true); setListError(''); try { const page = await listCrmLeads({ scope: nextScope, q: q || undefined, status: status || undefined }); setLeads(page.results); } catch (caught: unknown) { setLeads([]); setListError(messageOf(caught, 'تعذر تحميل العملاء.')); } finally { setListLoading(false); } }, [scope]);
  ...
  const openLead = async (id: number) => { setDetailLoading(true); setDetailError(''); try { const [lead, page] = await Promise.all([getCrmLead(id), listCrmActivities(id)]); setSelected(lead); setActivities(page.results); } catch (caught: unknown) { setDetailError(messageOf(caught, 'لا يمكن فتح ملف هذا العميل.')); setNotice(messageOf(caught, 'لا يمكن فتح ملف هذا العميل.')); setSelected(null); } finally { setDetailLoading(false); } };
  ...
  if (selected) return <CrmLeadProfile key={selected.id} lead={selected} activities={activities} loading={detailLoading} error={detailError} isManager={isManager} isOwner={Boolean(myEmployeeId && selected.assigned_to?.id === myEmployeeId)} colleagues={colleagues} onBack={() => { setSelected(null); setDetailError(''); }} onStatus={saveStatus} onActivity={saveActivity} onTransfer={transfer} onRelease={release} />;
  return <div className="space-y-6">{myEmployeeId !== null && <CrmMyStats refreshKey={refreshKey} />}<CrmLeadList isManager={isManager} leads={leads} scope={scope} loading={listLoading} error={listError} onScope={(nextScope) => { setScope(nextScope); void loadLeads(nextScope); }} onFilters={(q, status) => void loadLeads(scope, q, status)} onSelect={(lead) => void openLead(lead.id)} onClaim={(lead) => void claim(lead)} onLookup={(value) => void performLookup(value)} lookup={lookup} lookupLoading={lookupLoading} onOpenLookup={(id) => void openLead(id)} onRequestLookupTransfer={(id) => setRequestLeadId(id)} onClaimLookup={(id) => void claimFromLookup(id)} />
