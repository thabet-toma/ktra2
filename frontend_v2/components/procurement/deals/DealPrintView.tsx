            ...
            >
                ...
                <div className="flex justify-between items-center border-b-2 border-gray-800 pb-4 mb-4">
                    <div className="flex gap-4 items-center">
                        ...
                        <div>
                            <h1 className="text-2xl font-black text-gray-900 leading-none">تقرير صفقة شامل</h1>
                            <p className="text-xs font-bold text-gray-500 mt-1">INTERNAL DEAL REPORT</p>
                        </div>
                    </div>

                    <div className="text-left text-xs space-y-1">
                        <div className="flex gap-2 justify-end"><span className="font-bold text-gray-900 text-sm">{deal.dealNumber}</span> <span className="text-gray-500">REF:</span></div>
                        <div className="flex gap-2 justify-end"><span className="font-medium text-gray-800">{formatDate(new Date().toISOString())}</span> <span className="text-gray-500">PRINTED:</span></div>
                ...
                <div className="grid grid-cols-4 gap-3 mb-4">
                    {[
                        { label: 'الحالة', value: deal.status, color: 'bg-blue-100 text-blue-800' },
                ...
                <div className="grid grid-cols-2 gap-4 mb-4 text-[11px]">
                    <div className="border border-gray-300 rounded-lg overflow-hidden">
                        <div className="bg-gray-100 px-3 py-2 border-b border-gray-300 flex justify-between items-center">
                            ...
                            <span className="font-mono text-[10px] text-gray-500">ID: {deal.supplierId}</span>
                        </div>
                        <div className="p-3 space-y-2">
                            <div className="flex justify-between"><span className="text-gray-500">الاسم:</span> <span className="font-bold text-sm">{deal.factoryName}</span></div>
                            ...
                            <div className="flex justify-between"><span className="text-gray-500">الهاتف:</span> <span className="dir-ltr font-medium">{supplier?.phone || supplier?.mobile || '-'}</span></div>
                            <div className="flex justify-between"><span className="text-gray-500">الايميل:</span> <span className="dir-ltr">{supplier?.email || '-'}</span></div>
                        </div>
                    </div>
                    ...
                    <div className="border border-gray-300 rounded-lg overflow-hidden">
                        ...
                        <div className="p-3 grid grid-cols-2 gap-x-6 gap-y-2">
                            <div className="flex justify-between"><span className="text-gray-500">الشحن:</span> <span className="font-bold">{deal.shippingMethod || '-'}</span></div>
                            <div className="flex justify-between"><span className="text-gray-500">المدة:</span> <span className="font-bold">{deal.deliveryDays || '-'} يوم</span></div>
                            <div className="flex justify-between"><span className="text-gray-500">الوزن:</span> <span className="dir-ltr font-bold">{deal.totalWeight ? `${deal.totalWeight} KG` : '-'}</span></div>
                            <div className="flex justify-between"><span className="text-gray-500">الحجم:</span> <span className="dir-ltr font-bold">{deal.totalVolume ? `${deal.totalVolume} CBM` : '-'}</span></div>
                                    ...
                                    <td className="py-2 px-2 text-center border-l border-gray-200">
                                        {item.imageUrls?.[0] && (
                                            <a href={item.imageUrls[0]} target="_blank" rel="noreferrer">
                                                <img src={item.imageUrls[0]} className="w-10 h-10 object-cover border border-gray-200 rounded shadow-sm mx-auto hover:scale-110 transition-transform" alt="" />
                                            </a>
                                        )}
                                    </td>
                                    <td className="py-2 px-3 border-l border-gray-200">
                                        <p className="font-bold text-gray-900 text-[12px]">{item.name}</p>
                                        <p className="text-[10px] text-gray-500 leading-relaxed mt-1 line-clamp-2">{item.specifications}</p>
                ...
                <div className="flex gap-4 text-[11px]">
                    <div className="flex-1 border border-gray-300 rounded-lg overflow-hidden">
                        <div className="bg-gray-100 px-3 py-2 border-b border-gray-300 font-bold flex justify-between items-center">
                            ...
                            <span className="text-[10px] bg-white px-2 py-0.5 rounded border border-gray-200">{deal.payments?.length || 0} دفعة</span>
                        </div>
                            ...
                            <tbody>
                                {deal.payments?.length ? (
                                    deal.payments.map((p, i) => (
                                        <tr key={i} className="border-b border-gray-100 last:border-0 hover:bg-blue-50/30">
                                            ...
                                            <td className="py-2 px-3">
                                                {p.confirmedBySupplier ? <span className="text-green-600 font-bold">● مؤكدة</span> : <span className="text-orange-500 font-bold">○ قيد الانتظار</span>}
                                            </td>
                                            <td className="py-2 px-3 text-left font-mono font-bold" dir="ltr">{formatCurrency(p.amount)}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr><td colSpan={4} className="py-4 text-center text-gray-400 italic">لا توجد سجلات دفع</td></tr>
                                )}
                            </tbody>
                            <tfoot className="bg-gray-50 font-bold border-t-2 border-gray-300 text-sm">
                                <tr>
                    ...
                    <div className="w-64 border border-gray-300 rounded-lg overflow-hidden h-fit shadow-sm">
                        ...
                        <div className="p-3 space-y-2.5">
                            ...
                            <div className="flex justify-between pt-1 text-red-600 font-bold text-sm">
                ...
                {/* 6. Footer Notes */}
                <div className="mt-4 border border-gray-200 rounded-lg p-3 bg-gray-50 text-[11px]">
                    <span className="font-bold text-gray-500 block mb-1">ملاحظات التقرير:</span>
                    <p className="font-bold text-gray-700 leading-relaxed">{deal.internalNotes || "لا توجد ملاحظات إضافية مسجلة لهذه الصفقة."}</p>
                </div>
