            ...
            >
                ...
                <div className="flex justify-between items-center border-b-2 ktra-border-soft pb-4 mb-4">
                    <div className="flex gap-4 items-center">
                        ...
                        <div>
                            ...
                            <h1 className={`font-black ktra-text-ink leading-none ${identity?.company_name_primary ? 'text-base mt-1' : 'text-2xl'}`}>فاتورة مشتريات</h1>
                            <p className="text-xs font-bold ktra-text-soft mt-1">PURCHASE INVOICE - {invoice.currency === 'ILS' ? 'NIS' : 'USD'}</p>
                        </div>
                    </div>

                    <div className="text-left text-xs space-y-1">
                        <div className="flex gap-2 justify-end"><span className="font-bold ktra-text-ink text-sm">{invoice.invoiceNumber}</span> <span className="ktra-text-soft">REF:</span></div>
                        <div className="flex gap-2 justify-end"><span className="font-medium ktra-text-ink">{formatDate(invoice.invoiceDate || invoice.createdAt)}</span> <span className="ktra-text-soft">DATE:</span></div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4 text-[11px]">
                    <div className="border ktra-border-soft rounded-lg overflow-hidden">
                        ...
                        <div className="p-3 space-y-2">
                            <div className="flex justify-between"><span className="ktra-text-soft">الاسم:</span> <span className="font-bold text-sm">{getSupplierName()}</span></div>
                            ...
                            <div className="flex justify-between"><span className="ktra-text-soft">مندوب المبيعات:</span> <span className="font-medium">{invoice.supplierSnapshot?.salesRepName || '-'}</span></div>
                            <div className="flex justify-between"><span className="ktra-text-soft">الهاتف:</span> <span className="dir-ltr font-medium">{invoice.supplierSnapshot?.salesRepPhone || supplier?.phone || '-'}</span></div>
                        </div>
                    </div>
                    ...
                    <div className="border ktra-border-soft rounded-lg overflow-hidden">
                        ...
                        <div className="p-3 grid grid-cols-2 gap-x-6 gap-y-2">
                            <div className="flex justify-between"><span className="ktra-text-soft">طريقة الشحن:</span> <span className="font-bold">{invoice.dealInfo?.shippingMethod || '-'}</span></div>
                            <div className="flex justify-between"><span className="ktra-text-soft">الوزن الإجمالي:</span> <span className="dir-ltr font-bold">{invoice.totalWeight ? `${invoice.totalWeight} KG` : '-'}</span></div>
                            <div className="flex justify-between"><span className="ktra-text-soft">الحجم الإجمالي:</span> <span className="dir-ltr font-bold">{invoice.totalVolume ? `${invoice.totalVolume} CBM` : '-'}</span></div>
                            <div className="col-span-2 flex justify-between border-t ktra-border-soft pt-2 mt-1">
                                <span className="ktra-text-soft">ملاحظات الصفقة:</span>
                    ...
                    <table className="w-full text-right border-collapse">
                        <thead className="ktra-bg-panel text-white text-[10px] font-bold">
                            <tr>
                                ...
                                {printLanded && (
                                    <th className="py-2 px-2 w-28 text-left border-r ktra-border-soft ktra-bg-panel/90">نهائي / وحدة</th>
                                )}
                                <th className="py-2 px-2 w-24 text-left border-r ktra-border-soft">الإجمالي</th>
                                {printLanded && (
                                    <th className="py-2 px-2 w-28 text-left ktra-bg-panel/90">إجمالي نهائي</th>
                                )}
                            </tr>
                        </thead>
                                    ...
                                    <td className="py-2 px-2 text-center border-l ktra-border-soft">
                                        {(item.imageUrls?.[0] || item.factoryImageUrl) && (
                                            <a href={item.imageUrls?.[0] || item.factoryImageUrl} target="_blank" rel="noreferrer">
                                                <img src={item.imageUrls?.[0] || item.factoryImageUrl} className="w-10 h-10 object-cover border ktra-border-soft rounded shadow-sm mx-auto" alt="" />
                                            </a>
                                        )}
                                    </td>
                                    <td className="py-2 px-3 border-l ktra-border-soft">
                                        <p className="font-bold ktra-text-ink text-[12px]">{item.name}</p>
                                        <p className="text-[10px] ktra-text-soft leading-relaxed mt-1">{item.specifications}</p>
                                        {item.notes && (
                                            <div className="mt-1 p-1 ktra-bg-panel border ktra-border-soft rounded text-[9px] ktra-text-ink italic">
                                                ملاحظة: {item.notes}
                                            </div>
                                        )}
                ...
                {!!invoice.paymentDetails?.length && (
                    <div className="mb-4 border ktra-border-soft rounded-lg overflow-hidden">
                        <div className="ktra-bg-panel px-3 py-2 border-b ktra-border-soft font-bold">تفاصيل دفعات المورد</div>
                        <table className="w-full text-[10px]">
                            ...
                            <tbody>
                                {invoice.paymentDetails.map((payment) => (
                                    <tr key={`${payment.source}-${payment.id}`} className="border-b ktra-border-soft">
