...
export const GoodsReceiptsPage: React.FC = () => {
  ...
  const columns: CommercialListColumn<GoodsReceiptRow>[] = [
    ...
    },
    {
      ...
      numeric: true,
      render: (r) => (
        <span className="aseel-num font-mono text-xs">{formatQuantity(r.total_quantity)}</span>
      ),
    },
    {
      key: "total_remaining",
      ...
      render: (r) => {
        ...
        return (
          <span
            ...
            style={{
              color: remaining > 0 ? "var(--aseel-warn, #b06800)" : "var(--aseel-ok, #2d7d46)",
            }}
          ...
          <tbody>
            {viewDoc.lines.map((l) => (
              <tr key={l.id}>
                ...
                <td className="aseel-num">{formatQuantity(l.received_total)}</td>
                <td
                  className="aseel-num"
                  style={{
                    color:
