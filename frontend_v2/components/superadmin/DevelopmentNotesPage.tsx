      ...
      <div className="overflow-x-auto rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)]">
        <table className="w-full min-w-[1050px] border-collapse text-sm">
          <thead className="bg-[var(--color-surface-2)] aseel-text-soft"><tr>
            ...
            <th className="w-24 px-2 py-2 text-center">إجراءات</th>
          </tr></thead>
          <tbody>
            {notes.map((note, index) => (
              <tr key={note.id} className="border-t border-[var(--color-border)]">
                ...
                <td className="border-l border-[var(--color-border)]"><textarea aria-label={`وصف الملاحظة ${index + 1}`} rows={1} className={`${cellClass} min-w-64 resize-y`} value={note.description} onChange={(e) => patchLocal(note.id, "description", e.target.value)} /></td>
                ...
                <td className="border-l border-[var(--color-border)]"><input aria-label={`موعد الملاحظة ${index + 1}`} type="date" className={cellClass} value={note.due_date || ""} onChange={(e) => patchLocal(note.id, "due_date", e.target.value || null)} /></td>
                <td className="px-2 py-1"><div className="flex justify-center gap-1"><button type="button" className="aseel-iconbtn" onClick={() => void save(note)} disabled={savingId === note.id} aria-label={`حفظ ${note.title}`} title="حفظ"><Save className="h-4 w-4" /></button><button type="button" className="aseel-iconbtn aseel-iconbtn--danger" onClick={() => void remove(note)} disabled={savingId === note.id} aria-label={`حذف ${note.title}`} title="حذف"><Trash2 className="h-4 w-4" /></button></div></td>
              </tr>
            ))}
            <tr className="border-t-2 border-[var(--color-primary)] bg-[var(--color-surface-2)]">
              ...
              <td className="border-l border-[var(--color-border)]"><input aria-label="عنوان ملاحظة جديدة" className={cellClass} placeholder="ملاحظة تطوير جديدة" value={draft.title} onChange={(e) => setDraft({ ...draft, title: e.target.value })} onKeyDown={(e) => { if (e.key === "Enter") void add(); }} /></td>
              <td className="border-l border-[var(--color-border)]"><textarea aria-label="وصف ملاحظة جديدة" rows={1} className={`${cellClass} min-w-64 resize-y`} value={draft.description} onChange={(e) => setDraft({ ...draft, description: e.target.value })} /></td>
              ...
              <td className="border-l border-[var(--color-border)]"><input aria-label="موعد ملاحظة جديدة" type="date" className={cellClass} value={draft.due_date || ""} onChange={(e) => setDraft({ ...draft, due_date: e.target.value || null })} /></td>
              <td className="px-2 py-1 text-center"><button type="button" onClick={() => void add()} disabled={savingId === "new"} className="aseel-btn aseel-btn--primary"><Plus className="h-4 w-4" /> إضافة</button></td>
            </tr>
          </tbody>
        </table>
      </div>
