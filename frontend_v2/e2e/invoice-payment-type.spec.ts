...
/* T-SIMPL2 — **معيارٌ استُبدل بقرار المالك، لا فشلٌ أُسكت.** كان هذا الاختبار
   يشترط أن الوضع السهل يُظهر «استحقاق» على الآجلة (THA-131 ربطه بنوع الدفع لا
   بالوضع). ثم طلب المالك صراحةً ألّا يظهر الاستحقاق في الواجهة المبسّطة أصلاً،
   فصار مفتاحاً في سِجلّ القناع (`doc.due-date`).

   والوعد الذي كان يحرسه هذا السطر **لم يسقط، بل انتقل**: أن الحقل يتبع نوع
   الدفع في الوضع **المتقدّم** يحرسه الاختبار السابق مباشرةً (بالاتجاهين)، وأن
   موعداً مُدخَلاً فعلاً يعود رغم الوضع السهل تحرسه قاعدة السقوط للظهور في
   `utils/uiMode.test.ts`. فما يُفحَص هنا الآن هو ما تبقّى: الشارة تُقرأ والصندوق
   والاستحقاق مطويّان — وأن قلب الحالة لا يزال يعمل تحت القناع. */
test('الوضع السهل يُبقي الشارة ويطوي الصندوق والاستحقاق معاً', async ({ page }) => {
  ...
  // قلبُ الحالة يعمل تحت القناع — والشارة وحدها تقول ماذا صارت الفاتورة.
  await paidBox(page).uncheck();
  await expect(badge(page)).toHaveText('آجل');
  await expect(page.getByText('استحقاق', { exact: true })).toHaveCount(0);
});
