 ...
 * تُقاد من معرض المكوّنات `/ui-kit` — عام وحتمي، وبيانات الشركات ليست كذلك.
 * (نفس مبرّر `ktra-dense-table-widths.spec.ts`.)
 */
...
test('النافذة العائمة تُسحب من شريط عنوانها', async ({ page }) => {
  ...
  const barBox = (await win.locator('.ktra-float-win__bar').boundingBox())!;
