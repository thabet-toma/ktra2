...
const ALWAYS_VISIBLE = ['العميل', 'مدفوعة', 'تاريخ', 'ملاحظات', 'أرصدة العميل'];
...
test('«؟» تشرح حقول الفاتورة الظاهرة في السهل — ولا تشرح ما أخفاه القناع', async ({ page }) => {
  await withInvoiceScreen(page, 'simple');
  // الحقول الظاهرة وحدها: لا «؟» للعملة ولا للاستحقاق — وكلاهما مخفيّ هنا
  // (عملةٌ افتراضية محلولة، وفاتورةٌ مدفوعة). التلميح يتبع القناع لا يعانده.
  ...
  // «مدفوعة» أدقّ الحالات: «؟» تسكن بجوار خانة اختيار، فضغطُها يجب أن يشرح
