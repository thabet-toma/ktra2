 ...
 *     هنا يُثبَت أن كتابة رقم المورّد تجد المنتج فعلاً، وأن الرقم يُعرض كي يرى
 *     المستخدم **لماذا** طابق.
 *
...
test('منتقي البنود يجد المنتج برقم مورّده، ويُظهر الرقم الذي طابق', async ({ page }) => {
  await openDraftInvoice(page, true);

  // المسودّة تُفتح للعرض؛ «تحرير» يفتح الشبكة ومعها منتقي المنتجات.
  await page.getByRole('button', { name: /تحرير/ }).first().click();

  // فتح فهرس المنتجات من زرّ خليّة «رقم المنتج» في شبكة البنود.
  await page.getByTitle('فهرس المنتجات الكامل (+)').first().click();
  // النطاق نافذة «إضافة منتج» وحدها — شجرة المنتجات في الشريط الجانبي تسرد
  // الكتالوج كلّه، فمطابقةٌ على الصفحة كلّها تقيس الشجرة لا المنتقي.
  const picker = page.locator('div.fixed.inset-0').filter({ hasText: 'إضافة منتج' });
  await picker.getByPlaceholder(/بحث|ابحث/).first().fill('3068.82');

  // المنتج الصحيح ظهر، والذي لا يحمل الرقم اختفى.
