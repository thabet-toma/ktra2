 ...
 * تخصّ الصفقة والتخليص، لا الشاشة (ولا تُستعمل قيمته هنا أصلاً — انظر
 * `getScreenColumns` أدناه، بارامترها `_scope` بادئته الشرطية تقول ذلك).
 * ولا حذف بيانات: حقل HS يبقى حيث كان — **لا وجود له على `PurchaseRFQLine`
 * ولا `SupplierQuotationLine` أصلاً**؛ النموذج الوحيد الذي يحمله في هذا
 * المستودع هو `PurchaseInvoiceItem` (فاتورة الشراء الدولية)، بعد أن تُبنى
 * البنود من التخليص — بعيداً كل البعد عن الطلبية والعرض اللذين تخدمهما هذه
 * المصفوفة. ولا إعداد لكل شركة يتحكّم بالأعمدة — رفضٌ صريح (تعقيدٌ بلا مشترٍ).
 */
