"""يكشف دفعات الاستيراد (LogisticsPayment) المرحّلة بغير قيمتها بالشيكل — ويصحّح (أ) بطلب.
...
المجموعات (لكل دفعة صفقة مرحّلة):
  (أ) صفقتها لها فاتورة دولية مرحّلة — الفاتورة دائنة بالشيكل والدفعة مدينة بالدولار
      كأنه شيكل ⇒ رصيد المورد خاطئ. --apply يصحّحها.
  (ب) صفقة أرشيف بقيد LOGISTICS_DEAL — القيد والدفعات بنفس الوحدة ⇒ تقرير فقط.
  (ج) صفقة بلا فاتورة ولا قيد صفقة — تقرير فقط.
  (وكيل) دفعات وكيل الشحن — تقرير فقط.

    python manage.py audit_deal_payment_currency --tenant 1
    python manage.py audit_deal_payment_currency --tenant 1 --apply

--apply: لكل دفعة (أ) بفرق: قيد عكسي للأصل (reverse_journal، لا حذف) بتاريخه، ثم قيد
جديد بـamount × usd_to_ils على نفس حسابَي الأصل وبتاريخه، وربط الدفعة بالجديد، وسجلّ
تدقيق — كلّ دفعة في transaction.atomic وحدها، فرفضُ واحدة (فترة مغلقة) لا يمسّ غيرها.
...
class Command(BaseCommand):
    help = "كشف دفعات الاستيراد المرحّلة بالدولار كأنه شيكل، وتصحيح المجموعة (أ) بـ--apply."

    def add_arguments(self, parser):
        parser.add_argument("--tenant", type=int, required=True, help="رقم المستأجر")
        parser.add_argument("--apply", action="store_true",
                            help="صحّح المجموعة (أ) فقط: عكس القيد وإعادة ترحيله بالشيكل.")
    ...
    def handle(self, *args, **options):
        ...
        for p in payments:
            if p.deal_id:
                ...
                label = p.deal.ref_number
            else:
                group, party, label = 'agent', p.shipment.shipping_agent, p.shipment.shipment_number
            ...
            rows.append({
                'p': p, 'group': group, 'label': label, 'party': party,
        ...
        self._report(rows)
        if options["apply"]:
            self._apply([r for r in rows if r['group'] == 'a' and r['diff'] != 0])
        else:
            self.stdout.write("\nقراءة فقط. --apply يصحّح المجموعة (أ) وحدها.")
    ...
    def _apply(self, rows):
        ...
        w = self.stdout.write
        w(self.style.MIGRATE_HEADING(f"\n--apply: {len(rows)} دفعة في المجموعة (أ)"))
        done = failed = 0
        for r in rows:
            p = r['p']
            try:
                with transaction.atomic():
                    ...
                    rev = accounting_api.reverse_journal(
                        ...
                        description=(f"[تصحيح عملة دفعة] صفقة {r['label']} — {p.title} — "
                                     f"عكس القيد #{orig.id} (دولار سُجّل كشيكل)")[:500],
                        line_description_prefix=f"عكس قيد #{orig.id}: ",
                        copy_currency=True,
                        copy_project=True,
                    )
                    desc = f"دفعة {p.title} | صفقة: {r['label']}"
                    ...
                    new = post_journal(
                        ...
                        reference_id=p.pk,
                        description=(f"{desc} | المورد: {r['party'].name} "
                    ...
                    create_audit_log(
                        ...
                        change_details=(
                            f"تصحيح عملة دفعة صفقة {r['label']}: القيد #{orig.id} رحّل "
