...
**أين يُرى الفائض** (2026-09-25): دفعة تخليصٍ أو إرساليةٍ بالعملة الأساسية على
استحقاقٍ مرحَّل لا تعلق زائدةً على مستندها بعد الآن — تُسجَّل بالمتبقّي والزائد
سند صرفٍ «تحت الحساب» للطرف (`domain/overpayment_split.py`). الدفتر كما هو.
"""
...
def clearance_broker_posting_cap_check(paid, additional_amount, budget, label="clearance broker") -> Tuple[bool, Optional[str]]:
    ...
    بدل رفض العملية. نسجّل الفائض في اللوج فقط لبقائه ظاهراً للمراجعة؛ والفصل نفسه
    (المتبقّي على المستند والزائد سند «تحت الحساب») في `domain/overpayment_split.py`.
    """
    add = Decimal(additional_amount or 0)
    if add <= 0:
        return True, None
