...
class PurchaseInvoiceItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        # `id` قابلٌ للكتابة عمداً (انظر الحقل أعلاه): به تُطابَق البنود عند
        # التعديل. الباقي مشتقٌّ على الخادم فلا يُكتب.
        read_only_fields = ['received_quantity', 'remaining_quantity']
...
class PurchaseInvoiceSerializer(serializers.ModelSerializer):
    ...
    def _guard_received_items(self, instance, existing, items_data, kept_ids):
        ...
        """
        # «كم استُلم» قاعدةٌ واحدة في النظام (`purchase_item_receipt_quantities`)
        # — لا نسخة سابعة منها هنا.
        errors = []
