...
class JobApplicantInvitation(models.Model):
    ...
    #: ‏#214-د: شرحٌ حرٌّ يقرؤه المدعوُّ **قبل** إنشاء حسابه.
    #: كانت صفحةُ الدعوة أربعَ جملٍ ثابتة (اسمُه، عنوانُ الوظيفة، تاريخُ
    #: الانتهاء) ولا موضعَ في النموذج كلِّه لكلمةٍ يكتبها المدير — فكلُّ شرحٍ
    #: عن الدوام أو الراتب أو موعد المباشرة كان يُرسَل خارج المنصّة أو لا يُرسَل.
    note = models.TextField(
        blank=True,
        default="",
        verbose_name="شرح الدعوة",
        help_text="نصٌّ يظهر للمدعوّ في صفحة قبول الدعوة قبل تسجيل الدخول",
    )
    #: رقمُ تواصلٍ للمدعوّ إن كان له سؤالٌ قبل القبول. منفصلٌ عن `note` عمداً:
    #: رقمٌ داخل نصٍّ حرٍّ لا يصير زرَّ اتصالٍ على الهاتف، وحقلٌ مستقلٌّ يصير.
    contact_phone = models.CharField(
        max_length=40,
        blank=True,
        default="",
        verbose_name="رقم التواصل",
        help_text="يظهر للمدعوّ ليسأل قبل قبول الدعوة",
    )
...
class EmployeeCompensationPolicy(models.Model):
    ...
    #: شرحٌ حرٌّ **يقرؤه الموظّف** مع أرقامه. بلاغُ المالك: رقمُ الأساسيّ وحدَه
    #: يكذب بالسكوت — قد يكون صغيراً لأنّ فوقه عمولةَ تسويقٍ أو حافزَ اكتساب،
    #: فيقرأ صاحبُه «٤٠٠» ويظنّ أنّ هذا كلُّ ما له.
    #:
    #: ومكانُه **على النسخة** لا على الموظّف: النسخةُ مؤرَّخةٌ ولا تُعدَّل بعد
    #: تفعيلها، فالشرحُ يسري ويُحفَظ مع الأرقام التي يشرحها. ولو عُلِّق على
    #: الموظّف لبقي شرحٌ قديمٌ معلَّقاً فوق أرقامٍ جديدةٍ بلا أن يشتكي شيء.
    #:
    #: ويختلف عن `activation_reason` اختلافَ جمهور: ذاك **للمدقّق** (لماذا
    #: فُعِّلت هذه النسخة) ولا يُعرَض على أحد، وهذا **للموظّف** وحدَه المقصود.
    pay_terms_note = models.TextField(
        blank=True,
        default="",
        verbose_name="شرح شروط الصرف",
        help_text="نصٌّ يظهر للموظّف تحت أرقامه في صفحته الشخصيّة — اشرح ما لا يقوله الرقم وحدَه.",
    )
    activation_reason = models.CharField(
        max_length=500, blank=True, default="", verbose_name="سبب التفعيل",
        help_text="إلزامي لحظة تفعيل المسودة؛ يبقى في الصف بعد التفعيل.",
