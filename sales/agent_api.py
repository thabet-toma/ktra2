...
def agent_last_price(request):
    ...
    Response: {"unit_price": "123.45", "invoice_number": ..., "invoice_date": ...}
              أو {"unit_price": null, ...} إن لم يسبق بيع الصنف إطلاقاً.

    يفوّض إلى `sales.services.last_sale_price` — **نفس** الدالة التي تُغذّي
    اقتراح السعر في الواجهة وبطاقة الصنف (`inventory.services`). القاعدة لا
    ...
    يسبق له شراء الصنف رجعت إلى آخر سعر عام — وهذا مقصود لأن البوت يحتاج رقماً
    يقترحه لا فراغاً. ومحصورة ببيع **مرحَّل** (`invoice_kind='sale'`) وبترتيب
    حتمي (تاريخ الفاتورة تنازلياً ثم المعرّف).

    العزل عبر `_resolve_agent_tenant` — الصنف والعميل يُفلتران بشركته لا
