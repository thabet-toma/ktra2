...
def allocate_invoice_discount(net, discount, subtotal):
    ...
    أسطر نفس المنتج: `Σ(n − d·n/s) = Σn − d·Σn/s`. لولا ذلك لاختلف رقم الإيراد
...
def sales_revenue_map(
    *,
    ...
    """مرآة `sales_cogs_map` على جانب الإيراد: `{(فاتورة، منتج): {net, qty, tax}}`.
    ...
    قاعدة التوزيع. هنا يُحسب مرّةً — صافي الأسطر لنفس المنتج، ناقصاً نصيبه من خصم
    الفاتورة عبر `allocate_invoice_discount`.

    الصافي **بلا تقريب**: التقريب في نهاية العرض وحدها، فمجموعُ المنتجات يساوي
...
def invoice_profits(
    *,
    ...
    التاريخية الموحَّدة نفسها التي تقرأها تقارير «حسب المنتج/الماركة»، فلا يوجد
...
def last_sale_price(
    *,
    ...
    """task18 DEF-C2: آخر سعر بيع للوحدة لهذا المنتج (واختيارياً لهذا العميل).
...
def customer_price_list(
    *, tenant_id: int, customer_id: int, exclude_quotation_id: int | None = None,
    ...
    for p in products:
        ...
        # آخر مصدر: «سعر البيع» العام في كرت المنتج — يظهر فقط حين لا شراء سابق
