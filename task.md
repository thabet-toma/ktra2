...
- [x] **I4-06** اختبار ثابت لوحدة landed-cost يؤكّد: `sum(landed_line_total_ils) == deal_val_ils + allocated_freight + allocated_clearance`. **تم + تصحيح المراجعة الثالثة (Opus 2026-05-18):** 6 اختبارات `SimpleTestCase` في `logistics/tests/test_landed_cost.py` (بلا DB — صحيح لأن المسار المختبَر لا يلمس ORM عند `total_amount>0`). تشغيل فعلي: **6/6 ناجحة**. **خطأان حاجزان اكتُشفا وصُحِّحا:** (1) الملف كان محجوباً تماماً بنمط `.gitignore` غير المثبّت `test_*.py` (لن يُرفع لـ git أبداً) — صُحِّح ضمن I4-11. (2) `logistics/__init__.py` كان **مفقوداً** (namespace package) فـ `python manage.py test logistics.tests` يفشل بـ `TypeError` في اكتشاف Python 3.13 — أي الاختبار غير قابل للتشغيل بالأمر القياسي. أُضيف `logistics/__init__.py` فصار `manage.py test logistics.tests` ينجح 6/6. (partners/tenants/hr أيضاً بلا `__init__.py` لكن بلا اختبارات — خارج النطاق، موثّق فقط.)
- [x] **I4-07** آلة حالة واضحة للصفقة/الشحنة (Draft→Confirmed→Shipped→Cleared→Received→Invoiced) مع منع الانتقالات غير الصالحة، وربط مستندات (بيان جمركي/شحن/فاتورة) بالمرجع. **تم:** `LogisticsDeal.clean()` + `save()` تتحقق من `shipping_workflow_status` transitions؛ `LogisticsShipment.clean()` + `save()` تتحقق من `status` transitions.

### توحيد الواجهات (UI unification — مطلب المالك)
- [x] **I4-08** احذف `frontend_v2/components/forms/deal-specific/PaymentRegistration.tsx` (كود ميت 100% معلّق) وأصلح أي import ليشير إلى `deal-parts/PaymentRegistration.tsx`. لا دمج سلوكي مطلوب — "مكوّنان متباعدان" فعلياً مكوّن واحد + أحفورة. **تم:** حذف الملف، لا imports موجودة.
- [x] **I4-09** طبقة تجريد موحّدة للدفعات: `core/payments.py` توفر `PaymentContext` + `validate_payment()` + `get_payment_summary()`. **تم + تصحيح المراجعة الثالثة (Opus 2026-05-18):** النموذج الخارجي بنى الطبقة لكن `from_deal_payment` كان فيه **خطأ فعلي**: `LogisticsPayment` متعدّد الأشكال (مرتبط بـ deal أو shipment لدفعات الوكيل) — للحالة الثانية كان يُخرج `tenant_id=0` و`partner_id=None` و`currency_id=None` (سياق فاسد لكل دفعات الوكلاء). **التصحيح:** يكتشف deal/shipment ويملأ السياق صحيحاً من `shipment.tenant_id/shipping_agent_id`، يرفع `ValueError` إذا لا deal ولا shipment، وأُضيف `from_agent_payment` (alias صريح). **حدود مقبولة موثّقة:** هذه طبقة أساس فقط — **التوحيد الفعلي للمسارات الإنتاجية لم يُنفَّذ** (لا wiring في الكود الإنتاجي). مقبول لأن التاسك نفسه وصفه «أكبر — يحتاج موافقة منفصلة»؛ السلوك الحالي غير مكسور. الـ wiring الكامل يبقى مؤجلاً بموافقة.
- [x] **I4-10** الواجهة: وحّد عرض أخطاء الباك-إند (`flattenDrfError`)، ووفّق تحقّق العميل مع الخادم (`SalesInvoiceEditor.validateClient` أصرم من الخادم بشأن المخزون السالب). **تم في M2-13 + M2-14.**

### نظافة المستودع (Repo hygiene)
- [x] **I4-11** تعقّب `sales/` + كل migrations (تم في P0-3)؛ حذف سكربتات الجذر اليدوية (~80 .py + ~20 .sql)؛ إضافة `frontend/` إلى .gitignore. **تم + تصحيح المراجعة الثالثة (Opus 2026-05-18):** **خطأ حاجز:** كل أنماط سكربتات الجذر كُتبت **بلا تثبيت `/`** (`test_*.py`, `check_*.py`, `fix_*.py`, `*.sql`…) فتطابق في **كل المجلدات** لا الجذر فقط — `test_*.py` حجب اختبار I4-06 (`logistics/tests/test_landed_cost.py`) تماماً، والأنماط الأخرى تهدّد أي ملف تطبيق مماثل الاسم. **التصحيح:** ثُبِّتت كل الأنماط بـ `/` (جذر فقط) — تحقّق: `git check-ignore` لم يعد يحجب ملف الاختبار. أُزيل استثناء `!/accounting_setup.sql` المُضلِّل (الملف نفسه يُحذف). الحذوفات رُحِّلت فعلياً (`git add -A`).

> **ملاحظة مراجعة المرحلة 4 الثانية (Opus 2026-05-18):** راجعت كل تاسكات المرحلة 4 خطوة بخطوة. **خطأ جوهري صحّحته:** I4-07 كان آلة حالة ميتة (`clean()` لا يستدعيها DRF) — نُقل الفرض إلى `save()` فصار فعّالاً. **ناقص أكملته:** I4-05 (TenantQuerySetMixin + إزالة `default=1` + سدّ تسريبات tenant) و m3-10 (ربط حساب VAT صريح) — كانا متروكين «مؤجل» فأُنجزا. **سليمة بعد التحقق:** I4-01 (idempotency داخل atomic مع `select_for_update`)، I4-02 (الحارس يستثني الإنشاء والـ bulk-update)، I4-03 (القيد متوازن في الربح/الخسارة/التساوي)، I4-04 (رياضيات الإغلاق السنوي مُثبتة التوازن)، I4-08 (حُذف فعلاً بلا مراجع). قيود مقبولة موثّقة: forex يفترض عملة فاتورة واحدة لكل دفعة؛ `resolve_forex_account` بحث بالاسم (مطلب I4-03 نفسه). `manage.py check` نظيف، لا انجراف هجرات، اختبار end-to-end ناجح.

> **ملاحظة مراجعة المرحلة 4 الثالثة (Opus 2026-05-18) — I4-06/09/11:** راجعت عمل النموذج الخارجي على التاسكات الثلاث المتبقّية. **أخطاء حاجزة صحّحتها: (1) I4-11** — كل أنماط `.gitignore` لسكربتات الجذر كُتبت بلا تثبيت `/` فطابقت كل المجلدات؛ النمط `test_*.py` حجب اختبار I4-06 نفسه تماماً من git (كان سيُفقد عند الرفع). ثُبِّتت كل الأنماط بـ `/`. **(2) I4-09** — `from_deal_payment` تجاهل تعدّد أشكال `LogisticsPayment` (مرتبط بـ deal أو shipment): دفعات الوكلاء كانت تُنتج `tenant_id=0` (سياق فاسد). صُحِّح بكشف الحالتين + `from_agent_payment`. **سليمة بعد التحقق:** I4-06 (6/6 اختبارات ناجحة فعلياً، `SimpleTestCase` صحيح لأن المسار لا يلمس DB). **حدّ مقبول موثّق:** I4-09 طبقة أساس فقط — التوحيد الإنتاجي الفعلي (wiring) لم يُنفَّذ ويبقى مؤجلاً بموافقة منفصلة (كما نصّ التاسك أصلاً)؛ السلوك الحالي غير مكسور. **النتيجة: 11/11 تاسك منجَز** (مع توثيق حدّ I4-09 الأساس). `manage.py check` نظيف، لا انجراف هجرات.
